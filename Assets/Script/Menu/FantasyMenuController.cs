...
{
    private static FantasyMenuController instance; private enum SettingsTab
    {
        Display,
        Keybind, Audio, Graphics, Distance
    }
    private struct KeybindEntry
    {
        public string KeyId;
        public KeyCode Fallback; public Button Button; public TMP_Text Label;
    }
    [Header("Scene")]
    [SerializeField] private string gameplaySceneName = "SampleScene";
    [SerializeField] private bool randomizeMenuCameraOnStart = false; [Header("Layout")]
    ...
    [SerializeField] private TMP_Text statusText; [Header("Loading")]
    ...
    [Header("Main Buttons")][SerializeField] private Button newGameButton;
    [SerializeField] private Button continueButton;
    [SerializeField] private Button loadGameButton;
    [SerializeField] private Button settingsButton;
    [SerializeField] private Button creditsButton; [SerializeField] private Button exitButton;
    [Header("Settings Navigation")][SerializeField] private Button settingsBackButton;
    ...
    [SerializeField] private TMP_Text settingsStatusText; [Header("Credits")]
    [SerializeField] private Button creditsBackButton; [Header("Display Controls")]
    ...
    [SerializeField] private TMP_Text uiScaleValueText; [Header("Audio Controls")]
    ...
    [SerializeField] private Toggle mutedToggle; [Header("Graphics Controls")]
    ...
    [SerializeField] private Toggle motionBlurToggle; [Header("Keybind Controls")]
    ...
    private int currentResolutionIndex; private bool suppressCallbacks;
    ...
    private Button pendingKeybindButton; private TMP_Text pendingKeybindLabel;
    private bool isWaitingForKeybind; private int keybindCaptureStartFrame;
    ...
    private string[] qualityNames = Array.Empty<string>(); private int qualityOptionIndex;
    private int frameRateOptionIndex; private int antiAliasingOptionIndex;
    private int shadowQualityOptionIndex; private int textureQualityOptionIndex;
    private string menuSceneName = string.Empty; private GameObject menuCanvasRoot;
    private GameObject menuEventSystemRoot; private RectTransform settingsFooterRow;
    private bool gameplaySettingsOpen; private bool isLoadingScene;
    private bool loadSavedGameAfterSceneLoad; private float loadingStartedAt;
    private string loadingBaseMessage = "Loading"; private void Awake()
    {
    ...
    private void OnDestroy()
    {
        ...
        SceneManager.sceneLoaded -= OnSceneLoaded; GameplayUiState.SetExternalMenuOpen(false);
        instance = null;
    }
    private void Update()
    {
        ...
        if (isWaitingForKeybind) { CaptureNextKeybind(); return; }
        if (!IsInGameplayScene() || !Input.GetKeyDown(KeyCode.Escape)) { return; }
        if (gameplaySettingsOpen) { CloseGameplaySettingsOverlay(); return; }
        if (!GameplayUiState.IsMenuOpen) { OpenGameplaySettingsOverlay(); }
    }
    private void BindUi()
    {
        EnsureSettingsFooterButtons(); EnsureDistanceTab();
        ...
        BindSlider(uiScaleSlider, OnUiScaleChanged); BindAudioUi(); BindGraphicsUi();
        BindKeybindUi();
    }
    ...
    private static void PositionFooterButton(Button button, Vector2 anchor, Vector2 anchoredPosition)
    {
        ...
        rectTransform.anchorMax = anchor; rectTransform.pivot = new Vector2(0.5f, 0.5f);
    ...
    private static void SetSettingsRowLabel(Transform row, string labelText)
    {
        ...
        for (int i = 0; i < labels.Length; i++)
        {
            ...
            if (label != null && label.name == "Label Text") { label.text = labelText; return; }
        }
    }
    ...
    private void BindSlider(Slider slider, UnityAction<float> action)
    {
        ...
        slider.onValueChanged.RemoveAllListeners(); slider.onValueChanged.AddListener(action);
    }
    private void BindToggle(Toggle toggle, UnityAction<bool> action)
    {
        if (toggle == null) { return; }
        toggle.onValueChanged.RemoveAllListeners(); toggle.onValueChanged.AddListener(action);
    }
    private static void SetupSlider(Slider slider, float minValue, float maxValue, bool wholeNumbers, float value)
    {
        if (slider == null) { return; }
    ...
    {
        if (resolutionChoices == null || resolutionChoices.Length == 0) { return 0; }
    ...
    {
        suppressCallbacks = true; PopulateResolutionChoices();
        ...
        UpdateBrightnessLabel(GameSettings.Brightness); UpdateUiScaleLabel(GameSettings.UIScale);
        ApplyUiScalePreview(GameSettings.UIScale); LoadAudioSettingsToUi();
        LoadGraphicsSettingsToUi(); UpdateKeybindLabels(); suppressCallbacks = false;
        SetSettingsStatus(string.Empty); UpdateTabVisuals(); ShowTab(currentTab);
    }
    private void ShowMain()
    {
        HideLoadingScreen(); if (IsInGameplayScene())
        {
            CloseGameplaySettingsOverlay(); return;
        }
        gameplaySettingsOpen = false;
        if (menuCanvasRoot != null) { menuCanvasRoot.SetActive(true); }
        SetPanelState(true, false, false);
        SetBackground(mainBackgroundSprite, mainBackgroundTint); SetStatus(string.Empty);
    ...
    {
        if (menuShellRoot != null) { menuShellRoot.SetActive(showMain || showCredits); }
    ...
    {
        if (resolutionChoices == null || resolutionChoices.Length == 0) { return; }
        ...
        currentResolutionIndex = newIndex; ApplyResolutionChoice();
    }
    private void ApplyResolutionChoice()
    {
        ...
        GameSettings.RefreshRate = choice.RefreshRate; UpdateResolutionLabel();
        MarkSettingsDirty("Resolution changed.");
    }
    private void UpdateResolutionLabel()
    {
        if (resolutionValueText == null || resolutionChoices == null || resolutionChoices.Length == 0) { return; }
    ...
    private void OnUiScaleChanged(float value)
    {
        ...
        ApplyUiScalePreview(value); if (suppressCallbacks) { return; }
        GameSettings.UIScale = value; GameSettings.NotifyChanged();
    ...
    private void LoadGraphicsSettingsToUi()
    {
        ...
        UpdateQualityLabel(); UpdateFrameRateLabel(); UpdateAntiAliasingLabel();
        UpdateShadowQualityLabel(); UpdateTextureQualityLabel();
    ...
    {
        UpdateVolumeLabel(label, value); if (suppressCallbacks) { return; }
        setVolume(value);
        GameSettings.ApplyAudioSettings(); GameSettings.NotifyChanged();
        MarkSettingsDirty($"{volumeName} volume {Mathf.RoundToInt(value * 100f)}%.");
    }
    private void OnMutedChanged(bool value)
    {
        if (suppressCallbacks) { return; }
        GameSettings.Muted = value; GameSettings.ApplyAudioSettings();
        GameSettings.NotifyChanged();
    ...
    private void StepQuality(int delta)
    {
        ...
        GameSettings.ApplyGraphicsPreset(qualityOptionIndex); GameSettings.NotifyChanged();
    ...
    private void StepFrameRate(int delta)
    {
        ...
        GameSettings.TargetFrameRate = target; GameSettings.ApplyDisplaySettings();
        GameSettings.NotifyChanged(); UpdateFrameRateLabel();
    ...
    private void StepAntiAliasing(int delta)
    {
        ...
        GameSettings.AntiAliasing = value; GameSettings.ApplyGraphicsSettings();
        GameSettings.NotifyChanged(); UpdateAntiAliasingLabel();
    ...
    private void StepShadowQuality(int delta)
    {
        ...
        GameSettings.ApplyGraphicsSettings(); GameSettings.NotifyChanged();
    ...
    {
        if (suppressCallbacks) { return; }
        GameSettings.AnisotropicFiltering = value; GameSettings.ApplyGraphicsSettings();
        GameSettings.NotifyChanged();
    ...
    {
        if (suppressCallbacks) { return; }
        GameSettings.Bloom = value; GameSettings.ApplyGraphicsSettings();
        GameSettings.NotifyChanged();
    ...
    {
        if (suppressCallbacks) { return; }
        GameSettings.MotionBlur = value; GameSettings.ApplyGraphicsSettings();
        GameSettings.NotifyChanged();
    ...
    {
        isWaitingForKeybind = true; pendingKeybindId = keyId; pendingKeybindFallback = fallback;
        pendingKeybindButton = button; pendingKeybindLabel = label;
    ...
    {
        if (Time.frameCount == keybindCaptureStartFrame) { return; }
        if (Input.GetKeyDown(KeyCode.Escape)) { CancelKeybindCapture(); return; }
        Array values = Enum.GetValues(typeof(KeyCode)); for (int i = 0; i < values.Length; i++)
        {
            KeyCode keyCode = (KeyCode)values.GetValue(i); if (keyCode == KeyCode.None) { continue; }
            if (!Input.GetKeyDown(keyCode)) { continue; }
            ApplyCapturedKeybind(keyCode); return;
        }
    }
    private void CancelKeybindCapture()
    {
        isWaitingForKeybind = false;
        pendingKeybindId = string.Empty; pendingKeybindButton = null; pendingKeybindLabel = null;
        if (keybindInfoText != null) { keybindInfoText.text = string.Empty; }
        UpdateKeybindLabels(); SetSettingsStatus("Keybind capture canceled.");
    }
    private void ApplyCapturedKeybind(KeyCode keyCode)
    {
        isWaitingForKeybind = false;
        GameSettings.SetKey(pendingKeybindId, keyCode, false);
        GameSettings.ApplyInputOverridesToScene(); GameSettings.NotifyChanged();
        UpdateKeybindLabels();
        if (keybindInfoText != null) { keybindInfoText.text = string.Empty; }
        MarkSettingsDirty($"{pendingKeybindId} -> {GameSettings.ToDisplayName(keyCode)}.");
        pendingKeybindId = string.Empty; pendingKeybindButton = null;
    ...
    private void UpdateKeybindLabels()
    {
        for (int i = 0; i < keybindEntries.Count; i++)
        {
            ...
            {
                entry.Label.text = "Press key..."; continue;
            }
    ...
    private void SetGraphicsRowVisible(Component control, bool visible)
    {
        ...
        if (row != null && !IsQualityRow(row)) { row.gameObject.SetActive(visible); return; }
    ...
    private Transform FindGraphicsRow(Transform control)
    {
        ...
        Transform content = graphicsTabContent.transform; Transform current = control;
    ...
    private bool IsQualityRow(Transform row)
    {
        ...
        IsControlInRow(row, qualityNextButton) || IsControlInRow(row, qualityValueText);
    }
    ...
    private static int FindClosestOptionIndex(int[] values, int target)
    {
        ...
        int bestIndex = 0; int bestDelta = int.MaxValue; for (int i = 0; i < values.Length; i++)
        {
            int delta = Mathf.Abs(values[i] - target); if (delta < bestDelta)
            {
                bestDelta = delta;
                bestIndex = i;
            }
    ...
    private static int WrapIndex(int value, int length)
    {
        ...
        int result = value % length; if (result < 0) { result += length; }
        return result;
    }
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        {
            isLoadingScene = false; gameplaySettingsOpen = false;
            if (menuCanvasRoot != null) { menuCanvasRoot.SetActive(true); }
            HideLoadingScreen();
            ShowMain(); return;
        }
    ...
    {
        if (menuCanvasRoot != null) { DontDestroyOnLoad(menuCanvasRoot.transform.root.gameObject); }
    ...
    private void OpenGameplaySettingsOverlay()
    {
        ...
        SetPanelState(false, true, false); UpdateSettingsFooterContextButtons(true);
    ...
    {
        gameplaySettingsOpen = false; SetPanelState(false, false, false); SetStatus(string.Empty);
    ...
    private void ApplySettings()
    {
        ...
        pendingKeybindId = string.Empty; pendingKeybindButton = null; pendingKeybindLabel = null;
        if (keybindInfoText != null) { keybindInfoText.text = string.Empty; }
        GameSettings.SaveAndApply(); ApplyUiScalePreview(GameSettings.UIScale);
        LoadAudioSettingsToUi(); LoadGraphicsSettingsToUi(); UpdateKeybindLabels();
        SetSettingsStatus("Settings applied.");
    }
    private void UpdateSettingsFooterContextButtons(bool inGameplayEscOverlay)
    {
        if (settingsSaveButton != null) { settingsSaveButton.gameObject.SetActive(inGameplayEscOverlay); }
    ...
    {
        if (TryLoadSavedScene("Continuing saved game...")) { return; }
        SetStatus("No saved game. Start a new game.");
    }
    private void LoadSavedGame()
    {
        if (TryLoadSavedScene("Loading saved game...")) { return; }
        SetStatus("No saved game found.");
    }
    private bool TryLoadSavedScene(string loadingMessage)
    {
        if (!GameSaveManager.TryGetSavedSceneName(out string savedScene)) { return false; }
        loadSavedGameAfterSceneLoad = true; LoadSceneByName(savedScene, loadingMessage);
        return true;
    }
    private void LoadGameplayScene(bool newGame)
    {
        if (newGame)
        {
            GameSaveManager.DeleteSave(); loadSavedGameAfterSceneLoad = false;
        }
        LoadSceneByName(gameplaySceneName, newGame ? "Starting new game..." : "Loading game...");
    }
    private void SaveCurrentGameFromSettings()
    {
        ApplySettings(); if (!IsInGameplayScene())
        {
            SetSettingsStatus("Save is only available in game."); return;
        }
    ...
    {
        if (isLoadingScene) { return; }
        string trimmed = sceneName?.Trim(); if (string.IsNullOrWhiteSpace(trimmed))
        {
            SetStatus("Gameplay scene name is empty."); return;
        }
        if (!Application.CanStreamedLevelBeLoaded(trimmed))
        {
            SetStatus($"Scene '{trimmed}' is not in Build Settings."); return;
        }
    ...
    {
        isLoadingScene = true; ShowLoadingScreen(loadingMessage); yield return null;
        GameSettings.SaveAndApply();
        AsyncOperation operation = SceneManager.LoadSceneAsync(sceneName);
        if (operation == null)
        {
            isLoadingScene = false; HideLoadingScreen(); ShowMain();
            SetStatus($"Could not load scene '{sceneName}'."); yield break;
        }
        operation.allowSceneActivation = false; SetLoadingStage("Loading scene data");
        ...
        SetLoadingStage("Activating world"); SetLoadingProgress(0.9f);
        ...
        yield return WarmupLoadedScene(loadedScene); if (loadSavedGameAfterSceneLoad)
        {
            SetLoadingStage("Restoring save"); SetLoadingProgress(0.995f);
        ...
        SetLoadingProgress(1f); isLoadingScene = false; HideLoadingScreen();
        CloseGameplaySettingsOverlay();
    }
    private IEnumerator WarmupLoadedScene(Scene scene)
    {
        ...
        while (Texture.streamingTextureLoadingCount > 0 && Time.unscaledTime < timeoutAt)
        {
            ...
            SetLoadingProgress(0.98f); yield return null;
        }
        Texture.streamingTextureForceLoadAll = previousForceLoadAll; SetLoadingStage("Finishing");
        SetLoadingProgress(0.99f); yield return null;
    }
    private static void TouchSceneRenderers(Scene scene)
    {
        ...
        for (int i = 0; i < renderers.Length; i++)
        {
            ...
            {
                Material material = materials[materialIndex]; if (material == null) { continue; }
                Shader shader = material.shader; if (shader == null) { continue; }
                ...
                for (int textureIndex = 0; textureIndex < textureNames.Length; textureIndex++)
                {
                    ...
                    _ = texture.width; _ = texture.height;
                }
            }
        }
    }
    private void ShowLoadingScreen(string message)
    {
        ...
        SetLoadingProgress(0f); GameplayUiState.SetExternalMenuOpen(true);
    }
    ...
    {
        if (loadingSpinner != null) { loadingSpinner.Rotate(0f, 0f, -180f * Time.unscaledDeltaTime); }
        if (loadingMessageText != null)
        {
...
Application.Quit();
    ...
    private static void RandomizeMenuCameraView()
    {
        ...
        Vector3 position = focusPoint - (orbitDirection * distance); position.y = height;
        Vector3 target = focusPoint + new Vector3(UnityEngine.Random.Range(-0.8f, 0.8f),
        UnityEngine.Random.Range(-0.2f, 0.5f), UnityEngine.Random.Range(-0.8f, 0.8f));
