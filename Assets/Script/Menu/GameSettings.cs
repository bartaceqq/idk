...
{ Key.SwordSpecial2, KeyCode.Alpha4 }, { Key.SwordSpecial3, KeyCode.Alpha5 } };
    ...
    public static event Action SettingsChanged; public static int ResolutionWidth
    {
    ...
    {
        get => GetBool("display.vsync", true); set => SetBool("display.vsync", value);
    }
    ...
    {
        get => Mathf.Clamp(GetInt("graphics.qualityIndex", 1), 0, GraphicsPresetNames.Length - 1);
        set => SetInt("graphics.qualityIndex", Mathf.Clamp(value, 0, GraphicsPresetNames.Length - 1));
    }
    ...
    {
        get => GetFloat("audio.sfx", 0.9f); set => SetFloat("audio.sfx", Mathf.Clamp01(value));
    }
    ...
    {
        get => GetBool("audio.muted", false); set => SetBool("audio.muted", value);
    }
    public static void EnsureDefaults()
    {
        ...
        if (storedVersion > 0)
        {
            MigrateSettings(storedVersion); return;
        }
        ...
        RefreshRate = 0; FullScreenMode = UnityEngine.FullScreenMode.FullScreenWindow;
        VSync = true; Brightness = 1f; UIScale = 1f; TargetFrameRate = 0;
        SetGraphicsPresetValues(1); MasterVolume = 1f; MusicVolume = 0.8f; SfxVolume = 0.9f;
        AmbienceVolume = 0.8f; Muted = false;
...
"audio.ambience", "audio.muted" };
        ...
        foreach (string keyId in DefaultKeys.Keys) { PlayerPrefs.DeleteKey(KeyPrefKey(keyId)); }
        PlayerPrefs.DeleteKey(VersionKey); EnsureDefaults(); ApplyAllSettings(); RaiseChanged();
    }
    public static void ApplyAllSettings()
    {
        EnsureDefaults(); ApplyDisplaySettings();
        ApplyGraphicsSettings(); ApplyAudioSettings(); ApplyInputOverridesToScene();
    }
...
Screen.SetResolution(ResolutionWidth, ResolutionHeight, FullScreenMode, RefreshRate);
    ...
    public static void ApplyGraphicsSettings()
    {
        ...
        if (qualityNames != null && qualityNames.Length > 0)
        {
            ...
            if (QualitySettings.GetQualityLevel() != clampedQuality) { QualitySettings.SetQualityLevel(clampedQuality, true); }
        }
        QualitySettings.antiAliasing = AntiAliasing;
        QualitySettings.shadowDistance = ShadowDistance; QualitySettings.lodBias = ViewDistance;
        QualitySettings.anisotropicFiltering = AnisotropicFiltering
        ? UnityEngine.AnisotropicFiltering.ForceEnable : UnityEngine.AnisotropicFiltering.Disable;
        ...
        SetTextureMipmapLimit(TextureQuality); ApplyRenderPipelineSettings();
        ApplyViewDistanceSettingsToScene(); ApplyCameraSettingsToScene();
        ApplyVolumeComponentState("Bloom", Bloom);
        ApplyVolumeComponentState("MotionBlur", MotionBlur);
    }
    public static void ApplyGraphicsPreset(int presetIndex)
    {
        SetGraphicsPresetValues(presetIndex); ApplyGraphicsSettings();
    }
    public static void ApplyAudioSettings()
    {
        EnsureDefaults();
        AudioListener.volume = Muted ? 0f : 1f;
        AudioSource[] sources = UnitySceneSearch.FindAll<AudioSource>();
        for (int i = 0; i < sources.Length; i++)
        {
            ...
            {
                baseVolume = source.volume; BaseAudioSourceVolumes[id] = baseVolume;
            }
    ...
    {
        UnityEngine.InputSystem.PlayerInput[] inputs = UnitySceneSearch.FindAll<UnityEngine.InputSystem.PlayerInput>();
        for (int i = 0; i < inputs.Length; i++) { ApplyInputOverrides(inputs[i]); }
    }
    ...
    public static KeyCode GetDefaultKey(string keyId, KeyCode fallback = KeyCode.None) { return DefaultKeys.TryGetValue(keyId, out KeyCode key) ? key : fallback; }
    public static KeyCode GetKey(string keyId, KeyCode fallback = KeyCode.None)
    {
        ...
        if (!string.IsNullOrWhiteSpace(stored) && Enum.TryParse(stored, true, out KeyCode parsed)) { return parsed; }
        return GetDefaultKey(keyId, fallback);
    }
    public static void SetKey(string keyId, KeyCode keyCode, bool save = true)
    {
        PlayerPrefs.SetString(KeyPrefKey(keyId), keyCode.ToString()); if (save)
        {
            PlayerPrefs.Save(); ApplyInputOverridesToScene(); RaiseChanged();
        }
    }
    ...
    public static bool GetMouseButtonDown(string keyId, int fallbackButton)
    {
        ...
        if (TryGetMouseButtonIndex(keyCode, out int buttonIndex)) { return Input.GetMouseButtonDown(buttonIndex); }
        return Input.GetKeyDown(keyCode);
    }
    public static bool GetMouseButtonHeld(string keyId, int fallbackButton)
    {
        KeyCode keyCode = GetKey(keyId, fallbackButton == 1 ? KeyCode.Mouse1 : KeyCode.Mouse0);
        if (TryGetMouseButtonIndex(keyCode, out int buttonIndex)) { return Input.GetMouseButton(buttonIndex); }
        return Input.GetKey(keyCode);
    }
    public static void SaveAndApply()
    {
        PlayerPrefs.SetInt(VersionKey, Version); PlayerPrefs.Save(); ApplyAllSettings();
        RaiseChanged();
    }
    public static string GetKeyDisplayName(string keyId, KeyCode fallback = KeyCode.None) { return ToDisplayName(GetKey(keyId, fallback)); }
    public static string ToDisplayName(KeyCode keyCode)
    {
        ...
        if (keyCode >= KeyCode.Keypad0 && keyCode <= KeyCode.Keypad9) { return "Num " + (int)(keyCode - KeyCode.Keypad0); }
    ...
    {
        string[] names = new string[GraphicsPresetNames.Length];
        Array.Copy(GraphicsPresetNames, names, GraphicsPresetNames.Length); return names;
    }
    public static ResolutionChoice[] GetResolutionChoices()
    {
        ...
        if (resolutions != null && resolutions.Length > 0)
        {
            for (int i = 0; i < resolutions.Length; i++)
            {
                ...
                if (resolution.width < 640 || resolution.height < 360) { continue; }
                ResolutionChoice choice = new ResolutionChoice(resolution.width, resolution.height,
                RefreshRateValue(resolution)); if (!choices.Contains(choice)) { choices.Add(choice); }
            }
        }
        if (choices.Count == 0)
        {
    ...
    private static int GetInt(string key, int fallback) { return PlayerPrefs.GetInt(Prefix + key, fallback); }
    private static void SetInt(string key, int value) { PlayerPrefs.SetInt(Prefix + key, value); }
    private static float GetFloat(string key, float fallback) { return PlayerPrefs.GetFloat(Prefix + key, fallback); }
    private static void SetFloat(string key, float value) { PlayerPrefs.SetFloat(Prefix + key, value); }
    private static bool GetBool(string key, bool fallback) { return PlayerPrefs.GetInt(Prefix + key, fallback ? 1 : 0) != 0; }
    private static void SetBool(string key, bool value) { PlayerPrefs.SetInt(Prefix + key, value ? 1 : 0); }
    private static string KeyPrefKey(string keyId) { return Prefix + "key." + keyId; }
    ...
    private static void SetGraphicsPresetValues(int presetIndex)
    {
        int preset = Mathf.Clamp(presetIndex, 0, GraphicsPresetNames.Length - 1);
        QualityIndex = preset; switch (preset)
        {
            case 0:
                RenderScale = 0.6f; AntiAliasing = 0;
                ShadowQuality = 0; ShadowDistance = 25f; TextureQuality = 2; AnisotropicFiltering = false;
                ViewDistance = 0.4f; Bloom = false; MotionBlur = false; break;
            case 1:
                RenderScale = 0.85f; AntiAliasing = 2; ShadowQuality = 1; ShadowDistance = 75f;
                TextureQuality = 1; AnisotropicFiltering = true; ViewDistance = DefaultViewDistance;
                Bloom = true; MotionBlur = false; break;
            case 2:
                RenderScale = 1f; AntiAliasing = 4;
                ShadowQuality = 2; ShadowDistance = 150f; TextureQuality = 0; AnisotropicFiltering = true;
                ViewDistance = 4f; Bloom = true; MotionBlur = false; break;
            default:
                RenderScale = 1.25f;
                AntiAliasing = 8; ShadowQuality = 2; ShadowDistance = 300f; TextureQuality = 0;
                AnisotropicFiltering = true; ViewDistance = ViewDistanceMax; Bloom = true;
                MotionBlur = true; break;
        }
    }
    private static void ApplyMoveBinding(InputActionMap playerMap, string partName, string keyId, KeyCode fallback)
    {
        InputAction action = playerMap.FindAction("Move", false); if (action == null) { return; }
        KeyCode keyCode = GetKey(keyId, fallback); string path = ToKeyboardPath(keyCode);
        if (string.IsNullOrEmpty(path)) { return; }
        for (int i = 0; i < action.bindings.Count; i++)
        {
            InputBinding binding = action.bindings[i]; if (!binding.isPartOfComposite) { continue; }
            if (!string.Equals(binding.name, partName, StringComparison.OrdinalIgnoreCase)) { continue; }
            if (!ContainsGroup(binding.groups, "Keyboard&Mouse")) { continue; }
            action.ApplyBindingOverride(i, new InputBinding { overridePath = path }); return;
        }
    }
    private static void ApplyKeyboardBinding(InputActionMap playerMap, string actionName, string keyId, KeyCode fallback)
    {
        ...
        KeyCode keyCode = GetKey(keyId, fallback);
        string path = ToInputSystemPath(keyCode); if (string.IsNullOrEmpty(path)) { return; }
        for (int i = 0; i < action.bindings.Count; i++)
        {
            ...
            if (bindingIsMouse != pathIsMouse && HasMatchingDeviceBinding(action, pathIsMouse)) { continue; }
            action.ApplyBindingOverride(i, new InputBinding { overridePath = path }); return;
        }
    }
    private static bool HasMatchingDeviceBinding(InputAction action, bool wantsMouse)
    {
        for (int i = 0; i < action.bindings.Count; i++)
        {
            ...
            if (isMouse == wantsMouse) { return true; }
        }
        return false;
    }
    private static bool ContainsGroup(string groups, string group) { return string.IsNullOrEmpty(groups) || groups.IndexOf(group, StringComparison.OrdinalIgnoreCase) >= 0; }
    ...
    {
        if (keyCode >= KeyCode.A && keyCode <= KeyCode.Z) { return "<Keyboard>/" + keyCode.ToString().ToLowerInvariant(); }
        if (keyCode >= KeyCode.Alpha0 && keyCode <= KeyCode.Alpha9) { return "<Keyboard>/" + (int)(keyCode - KeyCode.Alpha0); }
        if (keyCode >= KeyCode.Keypad0 && keyCode <= KeyCode.Keypad9) { return "<Keyboard>/numpad" + (int)(keyCode - KeyCode.Keypad0); }
        return keyCode switch
        {
    ...
    private static bool TryGetMouseButtonIndex(KeyCode keyCode, out int buttonIndex)
    {
        switch (keyCode)
        {
            case KeyCode.Mouse0: buttonIndex = 0; return true;
            case KeyCode.Mouse1:
                buttonIndex = 1; return true;
            case KeyCode.Mouse2: buttonIndex = 2; return true;
            default:
                buttonIndex = -1; return false;
        }
    }
    private static void SetTextureMipmapLimit(int limit)
    {
        ...
        {
            globalLimit.SetValue(null, limit, null); return;
        }
        PropertyInfo masterLimit = typeof(QualitySettings).GetProperty("masterTextureLimit", BindingFlags.Public | BindingFlags.Static);
        if (masterLimit != null && masterLimit.CanWrite) { masterLimit.SetValue(null, limit, null); }
    }
    ...
    {
        float viewDistance = ViewDistance; Terrain[] terrains = Terrain.activeTerrains;
    ...
    {
        Volume[] volumes = UnitySceneSearch.FindAll<Volume>();
...
return resolution.refreshRate;
    ...
    private static void RaiseChanged() { SettingsChanged?.Invoke(); }
    private static float ResolveAudioChannelVolume(AudioSource source)
    {
        string name = source.name;
        if (source.transform.parent != null) { name += " " + source.transform.parent.name; }
        name = name.ToLowerInvariant();
        if (name.Contains("music") || name.Contains("theme") || name.Contains("song")) { return MusicVolume; }
        if (name.Contains("ambient") || name.Contains("ambience") || name.Contains("rain") || name.Contains("wind")) { return AmbienceVolume; }
        return SfxVolume;
    }
    private static string NicifyKeyName(string raw)
    {
        if (string.IsNullOrEmpty(raw)) { return string.Empty; }
        List<char> chars = new List<char>(raw.Length + 8); for (int i = 0; i < raw.Length; i++)
        {
            char current = raw[i];
            if (i > 0 && char.IsUpper(current) && !char.IsUpper(raw[i - 1])) { chars.Add(' '); }
            chars.Add(current);
        }
    ...
    {
        public readonly int Width; public readonly int Height; public readonly int RefreshRate;
        public ResolutionChoice(int width, int height, int refreshRate)
        {
            Width = width;
            Height = height; RefreshRate = refreshRate;
        }
        public bool Equals(ResolutionChoice other) { return Width == other.Width && Height == other.Height && RefreshRate == other.RefreshRate; }
        public override bool Equals(object obj) { return obj is ResolutionChoice other && Equals(other); }
        public override int GetHashCode()
        {
            unchecked
            {
                int hashCode = Width;
                hashCode = (hashCode * 397) ^ Height; hashCode = (hashCode * 397) ^ RefreshRate;
                return hashCode;
            }
        }
        public override string ToString()
        {
            string aspect = GetAspectLabel(Width, Height); return RefreshRate > 0
            ? $"{Width} x {Height} ({aspect}) {RefreshRate}Hz" : $"{Width} x {Height} ({aspect})";
        }
        ...
        {
            a = Mathf.Abs(a);
            b = Mathf.Abs(b); while (b != 0) { int remainder = a % b; a = b; b = remainder; }
