...
public static class GameSaveManager
{
    ...
    public static bool TryGetSavedSceneName(out string sceneName)
    {
        ...
        {
            sceneName = data.sceneName; return true;
        }
    ...
    public static bool SaveCurrentGame(out string message)
    {
        ...
        {
            message = "Save failed: no active gameplay scene."; return false;
        }
        ...
        }; try
        {
            ...
            PlayerPrefs.SetString(SavedScenePlayerPrefsKey, data.sceneName); PlayerPrefs.Save();
            message = $"Game saved. Buildings: {data.buildings.Count}, inventory slots: {data.inventory.Count}.";
            return true;
        }
        catch (Exception exception)
        {
            message = $"Save failed: {exception.Message}"; return false;
        }
    }
    public static bool LoadSavedGameIntoActiveScene(out string message)
    {
        if (!TryReadSave(out GameSaveData data, out message)) { return false; }
        Scene activeScene = SceneManager.GetActiveScene();
    ...
    public static void DeleteSave()
    {
        ...
        PlayerPrefs.DeleteKey(SavedScenePlayerPrefsKey); PlayerPrefs.Save();
    }
    private static bool TryReadSave(out GameSaveData data, out string message)
    {
        data = null;
        message = string.Empty; if (!File.Exists(SavePath))
        {
        ...
        try
        {
            ...
            {
                message = "Save file is empty or invalid."; return false;
            }
        ...
        {
            message = $"Could not read save: {exception.Message}"; return false;
        }
    }
    ...
    private static Transform TryGetActivePlayerTransform(out LookingController lookingController)
    {
        ...
        {
            if (lookingController.buildingcapsule != null && lookingController.buildingcapsule.activeInHierarchy) { return lookingController.buildingcapsule.transform; }
    ...
    private static List<BuildingSaveData> CaptureRuntimeBuildings()
    {
        ...
        for (int i = 0; i < pieces.Count; i++)
        {
            ...
            Transform pieceTransform = piece.transform; buildings.Add(new BuildingSaveData
            {
                kind = piece.kind,
    ...
    {
        ClearRuntimeBuildings(); if (buildings == null || buildings.Count == 0) { return 0; }
        RayCastScriptTest builder = FindBuilder(); int restored = 0;
        for (int i = 0; i < buildings.Count; i++)
        {
            ...
            RuntimeBuildPiece.Mark(created, building.kind); restored++;
        }
        return restored;
    }
    private static int RestoreInventory(List<InventorySlotSaveData> inventory)
    {
        InventoryManager manager = FindInventoryManager(); if (manager == null) { return 0; }
        List<SlotInsideUI> slots = GetInventorySlots(manager);
    ...
    private static void ClearRuntimeBuildings()
    {
        ...
        {
            RuntimeBuildPiece piece = piecesToDestroy[i]; if (piece != null)
            {
                piece.gameObject.SetActive(false); UnityEngine.Object.Destroy(piece.gameObject);
            }
        }
    }
    private static GameObject ResolveBuildPrefab(RayCastScriptTest builder, BuildPieceKind kind)
    {
        ...
        switch (kind)
        {
            case BuildPieceKind.Wall: return builder.wall;
            case BuildPieceKind.Floor:
                return builder.floor;
            case BuildPieceKind.Stair: return builder.stair;
            default:
                return null;
        }
    }
    ...
    {
        if (manager.slotlist == null) { manager.slotlist = new List<SlotInsideUI>(); }
    ...
    private static InventoryItem FindInventoryItem(string itemName)
    {
        ...
        return NpcTradeInventory.FindOrCreateItemDefinition(normalizedName);
    }
    private static void ApplyInventorySlot(SlotInsideUI slot, InventoryItem item,
    string itemName, int count, bool occupied, bool uiShown)
    {
        if (slot == null) { return; }
        slot.Item = item; slot.nameofslot = itemName; slot.count = count;
        slot.occupied = occupied; if (slot.image != null)
        {
        ...
        if (slot.text != null)
        {
            slot.text.text = count > 0 ? count.ToString() : "0"; slot.text.enabled = uiShown;
        }
    }
    [Serializable]
    ...
    {
        public int version;
        public string sceneName; public string savedAtUtc; public PlayerSaveData player;
    ...
    {
        public bool valid; public bool buildMode;
    ...
    private struct BuildingSaveData
    {
        ...
        public Vector3 position; public Quaternion rotation; public Vector3 localScale;
    }
    [Serializable]
