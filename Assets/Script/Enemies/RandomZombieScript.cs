...
{
    [Header("References")] public LookingController lookingController;
    public GameObject PlayerNormal; public GameObject PlayerBuilding;
    public Transform playertransform; public EnemiesHandler enemiesHandler;
    [Header("Behavior")] public float followRange = 15f; public float detectionRange = 15f;
    public float attackRange = 2f; public float attackRangeBuffer = 0.35f;
    public float attackCooldown = 1.2f; public float attackAnimLockSeconds = 0.8f;
    [Header("Player Damage")] public bool damagePlayerOnAttack = true;
    public float playerDamage = 12f; public float playerDamageDelay = 0.25f;
    public float playerDamageRangePadding = 0.75f;
    [Header("Detection")] public bool autoFindPlayerByTag = true;
    public string playerTag = "Player"; public bool debugRangeLogs;
    public float debugLogInterval = 0.25f;
    [Header("Roaming")] public bool enableRoaming = true; public float roamRadius = 25f;
    public float roamRepathInterval = 2f; public float roamMinMoveDistance = 3f;
    public int roamDestinationTries = 8;
    [Header("Custom Movement")] public float moveSpeed = 7f; public float acceleration = 18f;
    public float rotationSpeed = 10f; public float bodyRadius = 0.5f;
    public float bodyHeight = 2f; public float groundProbeHeight = 2.5f;
    public float groundProbeDistance = 6f; public float minimumGroundNormalY = 0.45f;
    public LayerMask groundLayerMask = ~0; public LayerMask obstacleLayerMask = ~0;
    public bool disableAnimatorRootMotion = true; public float obstacleProbeDistance = 1.25f;
    public float obstacleSideProbeDistance = 1.75f; public float obstacleSkinWidth = 0.08f;
    public float destinationReachDistance = 0.45f; public float separationRadius = 1.1f;
    public float separationStrength = 0.75f;
    [Header("Stuck Recovery")] public float stuckCheckInterval = 0.35f;
    public float stuckDistanceThreshold = 0.08f; public float stuckRecoverySeconds = 0.8f;
    public float stuckRecoveryTurnAngle = 80f; [Header("Performance")]
    ...
    private float _nextAttackTime; private float _attackAnimUnlockTime;
    private Vector3 _roamAnchor; private float _nextRoamRepathTime;
    private float _nextThinkTime; private float _nextResolvePlayerTime;
    private Vector3 _lastRequestedDestination; private bool _hasRequestedDestination;
    private Vector3 _currentVelocity; private bool _hasMoveDestination;
    private Vector3 _moveDestination; private float _moveStopDistance;
    private float _nextDebugLogTime; private bool _wasPlayerInRangeLastFrame;
    ...
    {
        if (enemiesHandler != null && !enemiesHandler.enemies.Contains(gameObject)) { enemiesHandler.enemies.Add(gameObject); }
        _roamAnchor = transform.position; _lastStuckCheckPosition = transform.position;
        if (detectionRange <= 0f) { detectionRange = followRange; }
        _nextThinkTime = Time.time + Random.Range(0f, Mathf.Max(0.02f, aiThinkInterval));
        _nextResolvePlayerTime = 0f; _hasRequestedDestination = false;
    }
    protected virtual void Update()
    {
        ...
        MoveAlongDestination(Time.deltaTime); UpdateStuckRecovery();
    }
    ...
    public void LockActions(float seconds)
    {
        ...
        if (lockUntil > _attackAnimUnlockTime) { _attackAnimUnlockTime = lockUntil; }
        StopMoving(); SetWalkAnimation(false);
    }
    ...
    {
        if (_attackSequence <= 0) { _attackSequence++; }
        TryDamagePlayerForAttack(_attackSequence);
    }
    private IEnumerator DamagePlayerAfterDelay(int attackSequence)
    {
        float delay = Mathf.Max(0f, playerDamageDelay);
        if (delay > 0f) { yield return new WaitForSeconds(delay); }
        TryDamagePlayerForAttack(attackSequence);
    }
    ...
    private void Think()
    {
        ResolvePlayerTransformThrottled(); if (playertransform == null)
        {
            StopMoving(); SetWalkAnimation(false); return;
        }
        if (Time.time < _attackAnimUnlockTime)
        {
            StopMoving(); FacePlayer(); SetWalkAnimation(false); return;
        }
        ...
        if (!playerInRange) { TryRoamAroundSpawn(); return; }
        _nextRoamRepathTime = 0f;
        if (IsPlayerInAttackRange(distanceToPlayer))
        {
            StopMoving(); FacePlayer();
            SetWalkAnimation(false); TryAttack(); return;
        }
        ChasePlayer();
    }
    private void ResolvePlayerTransform()
    {
        ...
        if (lookingController != null)
        {
            ...
            {
                if (PlayerNormal != null) { resolved = PlayerNormal.transform; } else if (lookingController.normalcapsule != null) { resolved = lookingController.normalcapsule.transform; }
            }
        }
        if (resolved == null)
        {
            ...
            else if (PlayerNormal != null) { resolved = PlayerNormal.transform; } else if (PlayerBuilding != null) { resolved = PlayerBuilding.transform; }
        }
        if (resolved == null) { resolved = TryFindPlayerByTag(); }
        playertransform = resolved;
    }
    private void ResolvePlayerTransformThrottled()
    {
        if (Time.time < _nextResolvePlayerTime) { return; }
        _nextResolvePlayerTime = Time.time + Mathf.Max(0.1f, playerResolveInterval);
        ResolvePlayerTransform();
    }
    private Transform TryFindPlayerByTag()
    {
        if (!autoFindPlayerByTag || string.IsNullOrWhiteSpace(playerTag)) { return null; }
        try
        {
    ...
    private void ChasePlayer()
    {
        ...
        if (!shouldRefreshDestination && _hasMoveDestination) { return; }
        SetMoveDestination(target, Mathf.Max(0.1f, attackRange * 0.8f));
        _lastRequestedDestination = target; _hasRequestedDestination = true;
    }
    private void StopMoving()
    {
        _hasMoveDestination = false; _hasRequestedDestination = false;
    ...
    private void TryRoamAroundSpawn()
    {
        ...
        if (_hasMoveDestination && Time.time < _nextRoamRepathTime) { return; }
        if (!TryGetRoamDestination(out Vector3 roamDestination))
        {
            StopMoving();
            SetWalkAnimation(false); _nextRoamRepathTime = Time.time + 0.5f; return;
        }
        ...
        _lastRequestedDestination = roamDestination; _hasRequestedDestination = true;
    }
    private bool TryGetRoamDestination(out Vector3 destination)
    {
        destination = Vector3.zero;
        int tries = Mathf.Max(1, roamDestinationTries);
        float minDistance = Mathf.Max(0f, roamMinMoveDistance); for (int i = 0; i < tries; i++)
        {
            Vector2 randomCircle = Random.insideUnitCircle * roamRadius;
            Vector3 candidate = _roamAnchor + new Vector3(randomCircle.x, 0f, randomCircle.y);
            if (!TryProjectToGround(candidate, out Vector3 groundedCandidate)) { continue; }
            if (HorizontalDistance(transform.position, groundedCandidate) < minDistance) { continue; }
            if (IsOwnSpaceBlocked(groundedCandidate)) { continue; }
    ...
    {
        if (TryProjectToGround(destination, out Vector3 groundedDestination)) { destination = groundedDestination; }
        _moveDestination = destination; _moveStopDistance = Mathf.Max(0.05f, stopDistance);
    ...
    private void MoveAlongDestination(float deltaTime)
    {
        if (!_hasMoveDestination || deltaTime <= 0f)
        {
            ...
            SetWalkAnimation(false); SnapToGround(); return;
        }
        Vector3 toDestination = _moveDestination - transform.position; toDestination.y = 0f;
        float distance = toDestination.magnitude;
        if (distance <= Mathf.Max(0.05f, _moveStopDistance))
        {
            StopMoving();
            SetWalkAnimation(false); return;
        }
        Vector3 desiredDirection = toDestination / distance;
        desiredDirection = ApplySeparation(desiredDirection);
        if (Time.time < _stuckRecoveryUntil) { desiredDirection = Quaternion.Euler(0f, _stuckTurnSign * stuckRecoveryTurnAngle, 0f) * desiredDirection; }
        ...
        float moveDistance = _currentVelocity.magnitude * deltaTime; if (moveDistance > 0.0001f)
        {
            ...
            if (TryGetBlockingHit(moveDirection, moveDistance + obstacleSkinWidth, out RaycastHit hit))
            {
                ...
                slideDirection.y = 0f; if (slideDirection.sqrMagnitude > 0.0001f &&
            ...
            else { transform.position += moveDirection * moveDistance; }
            FaceDirection(_currentVelocity.sqrMagnitude > 0.0001f ? _currentVelocity.normalized : openDirection);
        }
        SnapToGround(); SetWalkAnimation(_currentVelocity.sqrMagnitude > 0.05f);
    }
    private Vector3 ApplySeparation(Vector3 desiredDirection)
    {
        ...
        int overlapCount = Physics.OverlapSphereNonAlloc(transform.position, safeRadius,
        SeparationOverlapBuffer, obstacleLayerMask, QueryTriggerInteraction.Ignore);
        Vector3 separation = Vector3.zero; for (int i = 0; i < overlapCount; i++)
        {
            ...
            Vector3 away = transform.position - otherEnemy.transform.position; away.y = 0f;
            float distanceSqr = away.sqrMagnitude; if (distanceSqr <= 0.0001f) { continue; }
    ...
    {
        if (!TryGetBlockingHit(desiredDirection, obstacleProbeDistance, out _)) { return desiredDirection; }
        Vector3 bestDirection = Vector3.zero; float bestScore = float.MinValue;
        ...
        if (bestDirection.sqrMagnitude > 0.0001f) { return bestDirection; }
        BeginStuckRecovery();
    ...
    {
        bestHit = default; direction.y = 0f;
        if (direction.sqrMagnitude <= 0.0001f || distance <= 0f) { return false; }
        Vector3 normalizedDirection = direction.normalized;
        GetCapsulePoints(transform.position, out Vector3 bottom, out Vector3 top);
        float radius = Mathf.Max(0.05f, bodyRadius); int hitCount = Physics.CapsuleCastNonAlloc(
        bottom, top, radius, normalizedDirection, MovementHitBuffer, Mathf.Max(0.01f, distance),
        obstacleLayerMask, QueryTriggerInteraction.Ignore); float bestDistance = float.MaxValue;
        bool found = false; for (int i = 0; i < hitCount; i++)
        {
            RaycastHit hit = MovementHitBuffer[i]; Collider hitCollider = hit.collider;
            if (ShouldIgnoreMovementCollider(hitCollider)) { continue; }
            if (hit.distance < bestDistance)
            {
                bestDistance = hit.distance; bestHit = hit;
                found = true;
            }
        }
        return found;
    ...
    private bool IsOwnSpaceBlocked(Vector3 position)
    {
        ...
        int overlapCount = Physics.OverlapCapsuleNonAlloc(bottom, top,
        Mathf.Max(0.05f, bodyRadius), OwnSpaceOverlapBuffer, obstacleLayerMask,
        QueryTriggerInteraction.Ignore); for (int i = 0; i < overlapCount; i++)
        {
    ...
    private void SnapToGround() { if (TryProjectToGround(transform.position, out Vector3 groundedPosition)) { transform.position = groundedPosition; } }
    private bool TryProjectToGround(Vector3 position, out Vector3 groundedPosition)
    {
        ...
        int hitCount = Physics.RaycastNonAlloc(rayStart, Vector3.down, GroundHitBuffer,
        rayDistance, groundLayerMask, QueryTriggerInteraction.Ignore);
        ...
        for (int i = 0; i < hitCount; i++)
        {
            ...
            {
                bestY = hit.point.y;
                groundedPosition = new Vector3(position.x, hit.point.y, position.z); found = true;
            }
        }
        if (found) { return true; }
        Terrain terrain = FindTerrainAt(position);
        if (terrain == null) { return false; }
        groundedPosition = new Vector3(position.x,
        terrain.SampleHeight(position) + terrain.transform.position.y, position.z); return true;
    }
    private static Terrain FindTerrainAt(Vector3 worldPosition)
    {
        Terrain[] terrains = Terrain.activeTerrains; for (int i = 0; i < terrains.Length; i++)
        {
            Terrain terrain = terrains[i];
            if (terrain == null || terrain.terrainData == null) { continue; }
            Vector3 terrainPosition = terrain.transform.position;
            Vector3 size = terrain.terrainData.size; if (worldPosition.x >= terrainPosition.x &&
            worldPosition.x <= terrainPosition.x + size.x && worldPosition.z >= terrainPosition.z &&
            worldPosition.z <= terrainPosition.z + size.z) { return terrain; }
        }
        return null;
    }
    ...
    {
        if (!_hasMoveDestination || Time.time < _nextStuckCheckTime) { return; }
        ...
        if (movedSqr >= requiredMove * requiredMove) { return; }
        if (HorizontalDistance(transform.position, _moveDestination) <= _moveStopDistance + 0.5f) { return; }
        BeginStuckRecovery();
    }
    private void BeginStuckRecovery()
    {
        _stuckRecoveryUntil = Time.time + Mathf.Max(0.05f, stuckRecoverySeconds);
        _stuckTurnSign = Random.value < 0.5f ? -1f : 1f; _nextRoamRepathTime = 0f;
    }
    private void DebugPlayerDistance(float distanceToPlayer, float effectiveFollowRange, bool playerInRange)
    {
        if (!debugRangeLogs) { _wasPlayerInRangeLastFrame = playerInRange; return; }
        ...
        if (shouldLogNow)
        {
            ...
            this); _nextDebugLogTime = Time.time + Mathf.Max(0.05f, debugLogInterval);
        }
    ...
    private void DisableUnityNavMeshAgentComponent()
    {
        ...
        if (navMeshAgentComponent is Behaviour navMeshAgentBehaviour) { navMeshAgentBehaviour.enabled = false; }
    }
    private void DisableAnimatorRootMotion()
    {
        if (!disableAnimatorRootMotion) { return; }
        Animator[] animators = GetComponentsInChildren<Animator>();
        for (int i = 0; i < animators.Length; i++) { if (animators[i] != null) { animators[i].applyRootMotion = false; } }
    }
    private bool ShouldIgnoreMovementCollider(Collider candidate) { return candidate is TerrainCollider || ShouldIgnoreOwnOrPlayerCollider(candidate); }
    private bool ShouldIgnoreGroundCollider(Collider candidate) { return ShouldIgnoreOwnOrPlayerCollider(candidate); }
    private bool ShouldIgnoreOwnOrPlayerCollider(Collider candidate)
    {
        if (candidate == null) { return true; }
        Transform candidateTransform = candidate.transform;
        if (candidateTransform == null || candidateTransform.IsChildOf(transform)) { return true; }
        if (IsPlayerTransform(candidateTransform)) { return true; }
        return candidate.GetComponentInParent<CustomEnemyAIBase>() != null;
    }
    private bool IsPlayerTransform(Transform candidate)
    {
        if (IsSameOrChild(candidate, playertransform)) { return true; }
        if (PlayerNormal != null && IsSameOrChild(candidate, PlayerNormal.transform)) { return true; }
        if (PlayerBuilding != null && IsSameOrChild(candidate, PlayerBuilding.transform)) { return true; }
        if (lookingController != null)
        {
            if (IsSameOrChild(candidate, lookingController.transform)) { return true; }
            if (lookingController.normalcapsule != null && IsSameOrChild(candidate, lookingController.normalcapsule.transform)) { return true; }
            if (lookingController.buildingcapsule != null && IsSameOrChild(candidate, lookingController.buildingcapsule.transform)) { return true; }
        }
        return false;
    }
    ...
    {
        a.y = 0f;
        b.y = 0f; return Vector3.Distance(a, b);
    }
    private static bool IsSameOrChild(Transform candidate, Transform root) { return candidate != null && root != null && (candidate == root || candidate.IsChildOf(root)); }
    private void OnDrawGizmosSelected()
    {
        float effectiveFollowRange = GetEffectiveFollowRange(); Gizmos.color = Color.yellow;
        Gizmos.DrawWireSphere(transform.position, effectiveFollowRange); Gizmos.color = Color.red;
        Gizmos.DrawWireSphere(transform.position, attackRange); if (_hasMoveDestination)
        {
            Gizmos.color = Color.cyan; Gizmos.DrawLine(transform.position, _moveDestination);
...
{
    [Header("Zombie References")] public ZombieAnimationScript zombieAnimationScript;
    ...
    protected override void OnEnemyAttack()
    {
        ...
        if (thrownItemPrefab != null) { StartCoroutine(ThrowItemRoutine()); }
    }
    ...
    private IEnumerator ThrowItemRoutine()
    {
        ...
        Rigidbody rb = thrownItem.GetComponent<Rigidbody>(); if (rb != null)
        {
    ...
    private IEnumerator MoveProjectileArc(Transform projectile, Vector3 startPos, Vector3 endPos, float travelTime, float arcHeight)
    {
        ...
        Vector3 previousPos = startPos; while (elapsed < safeTravelTime)
        {
            elapsed += Time.deltaTime; float t = Mathf.Clamp01(elapsed / safeTravelTime);
    ...
    private static void EnsureProjectileVfxIsPlaying(GameObject projectileRoot)
    {
        ...
        foreach (VisualEffect effect in effects)
        {
            ...
            effect.enabled = true; effect.Reinit(); effect.Play();
        }
    }
}
