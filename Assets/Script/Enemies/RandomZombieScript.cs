using System.Collections; using UnityEngine; using UnityEngine.VFX;
...
public abstract class CustomEnemyAIBase : MonoBehaviour {
    ...
    [Header("Behavior")] public float followRange = 15f;
    ...
    [Header("Roaming")] public bool enableRoaming = true;
    ...
    [Header("Custom Movement")] public float moveSpeed = 7f;
    ...
    protected virtual void Start() { if (enemiesHandler != null && !enemiesHandler.enemies.Contains(gameObject)) { enemiesHandler.enemies.Add(gameObject); }
    ...
    private void ResolvePlayerTransform() {
        ...
        if (lookingController != null) {
            if (lookingController.switched) {
                if (PlayerBuilding != null) {
                    resolved = PlayerBuilding.transform; } else if (lookingController.buildingcapsule != null) {
                    resolved = lookingController.buildingcapsule.transform; } } else {
        ...
        if (resolved == null) {
            if (PlayerNormal != null && PlayerNormal.activeInHierarchy) {
                resolved = PlayerNormal.transform; } else if (PlayerBuilding != null && PlayerBuilding.activeInHierarchy) {
                resolved = PlayerBuilding.transform; } else if (PlayerNormal != null) { resolved = PlayerNormal.transform; } else if (PlayerBuilding != null) { resolved = PlayerBuilding.transform; } }

        if (resolved == null) { resolved = TryFindPlayerByTag(); }

        playertransform = resolved; }

    private void ResolvePlayerTransformThrottled() { if (Time.time < _nextResolvePlayerTime) { return; }

        _nextResolvePlayerTime = Time.time + Mathf.Max(0.1f, playerResolveInterval);
        ResolvePlayerTransform(); }

    private Transform TryFindPlayerByTag() { if (!autoFindPlayerByTag || string.IsNullOrWhiteSpace(playerTag)) { return null; }

        try {
            GameObject taggedPlayer = GameObject.FindGameObjectWithTag(playerTag);
            return taggedPlayer != null ? taggedPlayer.transform : null; } catch (UnityException) { return null; } }

    private void ChasePlayer() { if (playertransform == null) { return; }
    ...
    private void TryAttack() { if (Time.time < _nextAttackTime) { return; }
    ...
    private void SetMoveDestination(Vector3 destination, float stopDistance) { if (TryProjectToGround(destination, out Vector3 groundedDestination)) { destination = groundedDestination; }
        ...
        if (moveDistance > 0.0001f) {
            ...
            if (TryGetBlockingHit(moveDirection, moveDistance + obstacleSkinWidth, out RaycastHit hit)) {
                ...
                if (slideDirection.sqrMagnitude > 0.0001f &&
                    ...
                    transform.position += slideDirection.normalized * moveDistance * 0.75f; } else {
                    BeginStuckRecovery();
                    _currentVelocity = Vector3.zero; } } else { transform.position += moveDirection * moveDistance; }
    ...
    private void FacePlayer() { if (playertransform == null) { return; }
    ...
    private void UpdateStuckRecovery() { if (!_hasMoveDestination || Time.time < _nextStuckCheckTime) { return; }
    ...
    private void DisableAnimatorRootMotion() { if (!disableAnimatorRootMotion) { return; }
    ...
    private bool ShouldIgnoreOwnOrPlayerCollider(Collider candidate) { if (candidate == null) { return true; }
    ...
    private bool IsPlayerTransform(Transform candidate) { if (IsSameOrChild(candidate, playertransform)) { return true; }
        ...
        if (lookingController != null) { if (IsSameOrChild(candidate, lookingController.transform)) { return true; }
...
public class RandomZombieScript : CustomEnemyAIBase {
    ...
    [Header("Throw Visual")] public GameObject thrownItemPrefab;
    ...
    protected override void OnEnemyAttack() { if (zombieAnimationScript != null) { zombieAnimationScript.ThrowAnim(); }
    ...
    private IEnumerator ThrowItemRoutine() { if (throwSpawnDelay > 0f) { yield return new WaitForSeconds(throwSpawnDelay); }
    ...
    private IEnumerator MoveProjectileArc(Transform projectile, Vector3 startPos, Vector3 endPos, float travelTime, float arcHeight) { if (projectile == null) { yield break; }
    ...
    private static void EnsureProjectileVfxIsPlaying(GameObject projectileRoot) { if (projectileRoot == null) { return; }

        VisualEffect[] effects = projectileRoot.GetComponentsInChildren<VisualEffect>(true);
        foreach (VisualEffect effect in effects) { if (effect == null) { continue; }
