...
public class NPCDemageScript : MonoBehaviour
{
    ...
    public NPCHealthScript npcHealthScript; public Material demagemat;
    public Material origimat; public SkinnedMeshRenderer meshRenderer;
    public float flashSeconds = 0.5f; public float reactionLockSeconds = 0.6f;
    public float defaultDamage = 50f; public string damageStateName = "Damage";
    public string damageTriggerName = "Damage";
    [Header("Sword Hit Feedback")] public bool applyPracticeCapsuleHitFeedback = true;
    public TestHitting practiceCapsuleHitTemplate; public TestHitting hitFeedback;
    private Renderer _targetRenderer; private Coroutine _flashRoutine;
    void Awake()
    {
    ...
    public void TakeDemage(float damage)
    {
        ...
        if (npcHealthScript != null && npcHealthScript.IsDead) { return; }
        PlayDamageReaction();
        LockEnemyActions(); if (_flashRoutine != null) { StopCoroutine(_flashRoutine); }
    ...
    private void PlayDamageReaction()
    {
        ...
        {
            animator.Play(damageStateHash, 0, 0f); return;
        }
        if (!string.IsNullOrEmpty(damageTriggerName) && HasParameter(damageTriggerName, AnimatorControllerParameterType.Trigger))
        {
            animator.ResetTrigger(damageTriggerName); animator.SetTrigger(damageTriggerName);
        }
    }
    ...
    {
        if (!applyPracticeCapsuleHitFeedback) { return; }
        if (practiceCapsuleHitTemplate == null) { practiceCapsuleHitTemplate = TestHitting.FindPracticeTemplate(); }
        hitFeedback = TestHitting.EnsureEnemyHitFeedback(gameObject, practiceCapsuleHitTemplate,
        demagemat);
    }
}
