...
public class SlotManager : MonoBehaviour {
    ...
    private void OnValidate() {
        if (!Application.isPlaying) {
            ResolveInventoryListHandler();
            PrepareSlots(); } }
    public void RegisterSlot(Slot slot) { if (slot == null) { return; }

        if (!slots.Contains(slot)) { slots.Add(slot); }

        if (slot.slotManager != this) { slot.slotManager = this; }
    ...
    public bool CanAddItem(InventoryItem inventoryItem) { if (inventoryItem == null) { return false; }
        ...
        for (int i = 0; i < slots.Count; i++) {
            ...
            if (slot.MatchesItem(inventoryItem) || slot.IsEmpty()) { return true; } }
