...
public class RandomRainCycleController : MonoBehaviour
{
    ...
    [SerializeField] private BaseRainScript rainScript; [Header("State Durations (seconds)")]
    ...
    [Range(0f, 1f)]
    [SerializeField] private float minRainIntensity = 0.2f; [Range(0f, 1f)]
    ...
    [Header("Startup")][SerializeField] private bool startRaining; [Header("Wind")]
    [SerializeField] private bool controlWind = true;
    [SerializeField] private bool windWhenRaining = true;
    [SerializeField] private bool windWhenDry; private Coroutine weatherRoutine;
    ...
    private IEnumerator WeatherLoop()
    {
        ...
        if (!currentlyRaining) { rainScript.RainIntensity = 0f; ApplyWind(false); } while (true)
        {
            if (currentlyRaining) { yield return RainPhase(); } else { yield return DryPhase(); }
    ...
    {
        if (duration <= 0f) { rainScript.RainIntensity = target; yield break; }
    ...
    {
        float min = Mathf.Max(minimum, value.x); float max = Mathf.Max(min, value.y);
