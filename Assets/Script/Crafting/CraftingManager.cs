...
public class CraftingManager : MonoBehaviour
{
    ...
    public bool logActiveStation = false; private bool _checkQueued;
    ...
    private string _activeStationId = string.Empty; void Start()
    {
        ...
        ApplyMenuVisibility(); RefreshLists(); ResetRuntimePlacementState();
    ...
    public void Check()
    {
        EnsureActiveContextInitialized(); RefreshLists(); if (items.Count == 0)
        {
            ...
            UpdateSlotVisibility(); return;
        }
        if (slots.Count == 0)
        {
            Debug.LogWarning("CraftingManager: No craftable slots assigned or found."); return;
        }
        RebuildVisibleCraftables();
    }
    private void RebuildVisibleCraftables()
    {
        ResetRuntimePlacementState(); for (int i = 0; i < items.Count; i++)
        {
            CraftableItem item = items[i]; if (!IsItemVisibleInCurrentContext(item)) { continue; }
            CraftableSlot slot = GetLowestAvailableSlot(); if (slot == null) { break; }
