...
public class SpawnNpcsAtNight : MonoBehaviour
{
    ...
    public float minDistanceFromPlayer = 20f; public Transform spawnedParent;
    [Tooltip("Hard cap for alive enemies spawned by this spawner. Set <= 0 to disable cap.")] public int maxTotalAliveFromThisSpawner = 60;
    public float spawnClearRadius = 0.65f; public float spawnClearHeight = 2f;
    public LayerMask spawnBlockMask = ~0;
    [Header("Night Settings")] public LightingManager lightingManager;
    public bool spawnOnStartIfAlreadyNight = true; public bool spawnEveryNight = true;
    ...
    private void Update()
    {
        ...
        bool isNight = IsNightNow(); if (isNight && !_wasNightLastFrame)
        {
            ...
            {
                if (clearPreviousSpawnedOnNewNight) { ClearSpawnedEnemies(); }
        ...
        else if (!isNight && _wasNightLastFrame)
        {
            ...
            {
                if (clearPreviousSpawnedOnNewDay) { ClearSpawnedEnemies(); }
    ...
    private void SpawnWave(bool isNightWave)
    {
        ...
        {
            Debug.LogWarning("SpawnNpcsAtNight: no terrains assigned/found."); return;
        }
        if (zombie == null && skeleton == null)
        {
            Debug.LogWarning("SpawnNpcsAtNight: assign zombie and/or skeleton prefab."); return;
        }
        EnsureSpawnedParent();
        int monstersPerTerrainForWave = GetMonstersPerTerrainForWave(isNightWave);
        if (monstersPerTerrainForWave <= 0) { return; }
        int spawnBudget = maxTotalAliveFromThisSpawner <= 0 ? int.MaxValue
        ...
        for (int terrainIndex = 0; terrainIndex < terrains.Length; terrainIndex++)
        {
            ...
            {
                if (totalSpawned >= spawnBudget) { break; }
                if (!TryGetSpawnPositionOnTerrain(terrain, out Vector3 spawnPos)) { continue; }
    ...
    {
        int baseCount = Mathf.Max(0, monstersPerTerrain); if (isNightWave) { return baseCount; }
    ...
    public void ClearSpawnedEnemies()
    {
        ...
        {
            GameObject enemy = _spawnedEnemies[i]; if (enemy == null) { continue; }
    ...
    {
        for (int i = _spawnedEnemies.Count - 1; i >= 0; i--) { if (_spawnedEnemies[i] == null) { _spawnedEnemies.RemoveAt(i); } }
    }
    private void ResolveReferences()
    {
        if (lightingManager == null) { lightingManager = UnitySceneSearch.FindFirst<LightingManager>(false); }
        if (lookingController == null) { lookingController = UnitySceneSearch.FindFirst<LookingController>(false); }
        if (enemiesHandler == null) { enemiesHandler = UnitySceneSearch.FindFirst<EnemiesHandler>(false); }
        ResolvePracticeCapsuleHitTemplate(); _playerNormal = null; _playerBuilding = null;
        if (lookingController != null)
        {
            if (lookingController.normalcapsule != null) { _playerNormal = lookingController.normalcapsule.transform; }
            if (lookingController.buildingcapsule != null) { _playerBuilding = lookingController.buildingcapsule.transform; }
        }
    }
    private void ResolvePracticeCapsuleHitTemplate()
    {
        if (practiceCapsuleHitTemplate != null) { return; }
        practiceCapsuleHitTemplate = TestHitting.FindPracticeTemplate();
    }
    private void ResolveTerrains() { if (terrains == null || terrains.Length == 0) { terrains = Terrain.activeTerrains; } }
    private void EnsureSpawnedParent()
    {
        if (spawnedParent != null) { return; }
        GameObject root = new GameObject("NightSpawnedEnemies"); spawnedParent = root.transform;
    }
    private bool IsNightNow()
    {
        if (lightingManager == null) { return treatAsNightWhenNoLightingManager; }
        float hour = Mathf.Repeat(lightingManager.TimeOfDay, 24f); bool noNightConfigured =
        lightingManager.morningInterval.x <= 0f && lightingManager.afterNoonInterval.y >= 1f;
        if (useFallbackNightHours && noNightConfigured) { return IsHourInsideNightWindow(hour, fallbackNightStartsAtHour, fallbackNightEndsAtHour); }
    ...
    {
        float safeHour = Mathf.Repeat(hour, 24f); float safeStart = Mathf.Repeat(startHour, 24f);
        float safeEnd = Mathf.Repeat(endHour, 24f); if (Mathf.Approximately(safeStart, safeEnd))
        {
            return true;
        }
        if (safeStart > safeEnd) { return safeHour >= safeStart || safeHour < safeEnd; }
        return safeHour >= safeStart && safeHour < safeEnd;
    }
    private GameObject GetRandomEnemyPrefab()
    {
        if (zombie != null && skeleton != null) { return Random.value < 0.5f ? zombie : skeleton; }
        return zombie != null ? zombie : skeleton;
    }
    private bool TryGetSpawnPositionOnTerrain(Terrain terrain, out Vector3 spawnPosition)
    {
        spawnPosition = Vector3.zero; TerrainData data = terrain.terrainData;
        ...
        for (int i = 0; i < attempts; i++)
        {
            ...
            if (activePlayer != null && minDistanceFromPlayer > 0f) { if (Vector3.Distance(candidate, activePlayer.position) < minDistanceFromPlayer) { continue; } }
    ...
    private bool TryProjectSpawnPosition(Terrain terrain, Vector3 candidate, out Vector3 spawnPosition)
    {
        ...
        RaycastHit[] hits = Physics.RaycastAll(rayStart, Vector3.down, probeRadius * 2f + 2f,
        spawnBlockMask, QueryTriggerInteraction.Ignore); float bestY = spawnPosition.y;
        bool foundGround = false; for (int i = 0; i < hits.Length; i++)
        {
            RaycastHit hit = hits[i]; if (hit.collider == null || hit.normal.y < 0.45f) { continue; }
            if (!foundGround || hit.point.y > bestY) { bestY = hit.point.y; foundGround = true; }
        }
        spawnPosition.y = foundGround ? bestY : spawnPosition.y; return true;
    }
    private bool IsSpawnPositionClear(Vector3 spawnPosition)
    {
        ...
        Collider[] overlaps = Physics.OverlapCapsule(bottom, top, radius, spawnBlockMask,
        QueryTriggerInteraction.Ignore); for (int i = 0; i < overlaps.Length; i++)
        {
            Collider overlap = overlaps[i];
            if (overlap == null || overlap is TerrainCollider) { continue; }
            return false;
        }
        return true;
    }
    private Transform GetActivePlayerTransform()
    {
        if (lookingController != null)
        {
            if (lookingController.switched && _playerBuilding != null) { return _playerBuilding; }
            if (!lookingController.switched && _playerNormal != null) { return _playerNormal; }
        }
        if (_playerNormal != null && _playerNormal.gameObject.activeInHierarchy) { return _playerNormal; }
        if (_playerBuilding != null && _playerBuilding.gameObject.activeInHierarchy) { return _playerBuilding; }
        if (_playerNormal != null) { return _playerNormal; }
        if (_playerBuilding != null) { return _playerBuilding; }
    ...
    private void ConfigureSpawnedEnemy(GameObject enemy)
    {
        ...
        {
            if (zombieScript.lookingController == null) { zombieScript.lookingController = lookingController; }
            if (zombieScript.PlayerNormal == null && _playerNormal != null) { zombieScript.PlayerNormal = _playerNormal.gameObject; }
            if (zombieScript.PlayerBuilding == null && _playerBuilding != null) { zombieScript.PlayerBuilding = _playerBuilding.gameObject; }
            if (zombieScript.enemiesHandler == null) { zombieScript.enemiesHandler = enemiesHandler; }
        }
        RandomSkeletonScript skeletonScript = enemy.GetComponent<RandomSkeletonScript>();
        if (skeletonScript != null)
        {
            if (skeletonScript.lookingController == null) { skeletonScript.lookingController = lookingController; }
            if (skeletonScript.PlayerNormal == null && _playerNormal != null) { skeletonScript.PlayerNormal = _playerNormal.gameObject; }
            if (skeletonScript.PlayerBuilding == null && _playerBuilding != null) { skeletonScript.PlayerBuilding = _playerBuilding.gameObject; }
            if (skeletonScript.enemiesHandler == null) { skeletonScript.enemiesHandler = enemiesHandler; }
        }
        ApplyMonsterHitFeedback(enemy);
    }
    private void ApplyMonsterHitFeedback(GameObject enemy)
    {
        if (!applyPracticeCapsuleHitFeedbackToMonsters || enemy == null) { return; }
    ...
    {
        if (practiceCapsuleHitTemplate != null && practiceCapsuleHitTemplate.hitmat != null) { return practiceCapsuleHitTemplate.hitmat; }
        if (fallbackMonsterHitMaterial != null) { return fallbackMonsterHitMaterial; }
