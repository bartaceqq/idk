using System.Collections.Generic; using Sydewa; using UnityEngine;

// Spawns enemies on terrain when night starts.
public class SpawnNpcsAtNight : MonoBehaviour {
    [Header("Prefabs")] public GameObject zombie;
    public GameObject skeleton;

    [Header("Spawn Area")] public Terrain[] terrains;
    ...
    private void Start() {
        ...
        if (isNight && spawnOnStartIfAlreadyNight) {
            ...
            _spawnedAtLeastOnce = true; } else if (!isNight && spawnDuringDay && spawnOnStartIfAlreadyDay) {
            SpawnDayWave();
    ...
    private void Update() {
        ...
        if (isNight && !_wasNightLastFrame) {
            ...
            if (shouldSpawnThisNight && !_spawnedThisNight) { if (clearPreviousSpawnedOnNewNight) { ClearSpawnedEnemies(); }

                SpawnNightWave();
                _spawnedThisNight = true;
                _spawnedAtLeastOnce = true; } } else if (!isNight && _wasNightLastFrame) {
            _spawnedThisNight = false;

            bool shouldSpawnThisDay = spawnDuringDay && (spawnEveryDay || !_spawnedAtLeastOnce);
            if (shouldSpawnThisDay && !_spawnedThisDay) { if (clearPreviousSpawnedOnNewDay) { ClearSpawnedEnemies(); }
    ...
    private void SpawnWave(bool isNightWave) {
        ...
        int totalSpawned = 0;
        for (int terrainIndex = 0; terrainIndex < terrains.Length; terrainIndex++) { if (totalSpawned >= spawnBudget) { break; }

            Terrain terrain = terrains[terrainIndex];
            if (terrain == null || terrain.terrainData == null) { continue; }

            int spawnedOnThisTerrain = 0;
            for (int i = 0; i < monstersPerTerrainForWave; i++) { if (totalSpawned >= spawnBudget) { break; }
    ...
    private int GetMonstersPerTerrainForWave(bool isNightWave) {
        ...
        if (isNightWave) { return baseCount; }
    ...
    private void ResolveReferences() { if (lightingManager == null) { lightingManager = UnitySceneSearch.FindFirst<LightingManager>(false); }
        ...
        if (lookingController != null) { if (lookingController.normalcapsule != null) { _playerNormal = lookingController.normalcapsule.transform; }
            if (lookingController.buildingcapsule != null) { _playerBuilding = lookingController.buildingcapsule.transform; } } }
    private void ResolvePracticeCapsuleHitTemplate() { if (practiceCapsuleHitTemplate != null) { return; }
    ...
    private void EnsureSpawnedParent() { if (spawnedParent != null) { return; }

        GameObject root = new GameObject("NightSpawnedEnemies");
        spawnedParent = root.transform; }
    private bool IsNightNow() { if (lightingManager == null) { return treatAsNightWhenNoLightingManager; }
    ...
    private GameObject GetRandomEnemyPrefab() { if (zombie != null && skeleton != null) { return Random.value < 0.5f ? zombie : skeleton; }
    ...
    private bool TryProjectSpawnPosition(Terrain terrain, Vector3 candidate, out Vector3 spawnPosition) {
        ...
        for (int i = 0; i < hits.Length; i++) {
            ...
            if (hit.collider == null || hit.normal.y < 0.45f) { continue; }
    ...
    private Transform GetActivePlayerTransform() {
        if (lookingController != null) { if (lookingController.switched && _playerBuilding != null) { return _playerBuilding; }
    ...
    private void ConfigureSpawnedEnemy(GameObject enemy) { if (enemy == null) { return; }

        RandomZombieScript zombieScript = enemy.GetComponent<RandomZombieScript>();
        if (zombieScript != null) { if (zombieScript.lookingController == null) { zombieScript.lookingController = lookingController; }
        ...
        if (skeletonScript != null) { if (skeletonScript.lookingController == null) { skeletonScript.lookingController = lookingController; }
    ...
    private void ApplyMonsterHitFeedback(GameObject enemy) { if (!applyPracticeCapsuleHitFeedbackToMonsters || enemy == null) { return; }

        ResolvePracticeCapsuleHitTemplate();
        TestHitting.EnsureEnemyHitFeedback(enemy, practiceCapsuleHitTemplate, ResolveMonsterHitMaterial(enemy)); }
    private Material ResolveMonsterHitMaterial(GameObject enemy) { if (practiceCapsuleHitTemplate != null && practiceCapsuleHitTemplate.hitmat != null) { return practiceCapsuleHitTemplate.hitmat; }
