...
public class TerrainTreePrefabStreamer : MonoBehaviour
{
    ...
    [Header("Streaming Range")][SerializeField, Min(1f)] private float activateRadius = 60f;
    ...
    private bool _initialized; private bool _terrainDirty; private float _nextUpdateTime;
    private float _nextTerrainRefreshTime; private int _scanIndex;
    private float _activateRadiusSqr; private float _deactivateRadiusSqr;
    private sealed class StreamRecord
    {
        public GameObject gameObject;
        public Transform transform; public TreeInstance treeInstance; public bool prefabActive;
        public bool hiddenByStreamer; public bool removed;
    }
    private IEnumerator Start()
    {
        if (!Application.isPlaying) { yield break; }
        ApplyViewDistanceMultiplier(GameSettings.ViewDistance); yield return null; Initialize();
    }
    public void ApplyViewDistanceMultiplier(float multiplier)
    {
        ...
        RefreshDistanceSquares(); if (_initialized)
        {
            _terrainDirty = true; _nextUpdateTime = 0f;
    ...
    private void Update()
    {
        ...
        {
            _nextUpdateTime = Time.time + Mathf.Max(0.05f, updateInterval); ProcessStreamingBatch();
        }
    ...
    private void OnDisable()
    {
        ...
        {
            _runtimeTerrainData.treeInstances = _baseTreeInstances; targetTerrain.Flush();
        }
    }
    ...
    public void Initialize()
    {
        ...
        RefreshDistanceSquares(); BuildRecords(); _initialized = true; _terrainDirty = true;
        _nextTerrainRefreshTime = 0f; if (logSummary)
        {
    ...
    private TerrainData PrepareRuntimeTerrainData(Terrain terrain)
    {
        ...
        if (Application.isPlaying && cloneTerrainDataInPlayMode)
        {
            ...
            data.name = $"{terrain.terrainData.name}_StreamingRuntime"; terrain.terrainData = data;
    ...
    private void BuildRecords()
    {
        ...
        int childCount = convertedRoot.childCount; for (int i = 0; i < childCount; i++)
        {
            ...
            SetPrefabActive(record, shouldBePrefab); _records.Add(record);
        }
    }
    private void ProcessStreamingBatch()
    {
        ...
        for (int i = 0; i < budget; i++)
        {
            ...
            StreamRecord record = _records[_scanIndex++]; ProcessRecord(record);
        }
    }
    ...
    private void SetPrefabActive(StreamRecord record, bool active)
    {
        ...
        record.prefabActive = active; record.hiddenByStreamer = !active;
    ...
    private void RebuildTerrainTreeInstances()
    {
        ...
        result.AddRange(_baseTreeInstances); for (int i = 0; i < _records.Count; i++)
        {
            StreamRecord record = _records[i];
    ...
    private static Dictionary<string, int> BuildPrototypeNameMap(TreePrototype[] prototypes)
    {
        ...
        {
            GameObject prefab = prototypes[i]?.prefab; if (prefab == null) { continue; }
            string key = NormalizeName(prefab.name); if (!map.ContainsKey(key)) { map.Add(key, i); }
        }
        return map;
    }
    ...
    private static bool TryCreateTreeInstance(Transform source, Terrain terrain,
    TreePrototype[] prototypes, int prototypeIndex, out TreeInstance treeInstance)
    {
        ...
        ? prototypes[prototypeIndex].prefab.transform.localScale : Vector3.one;
