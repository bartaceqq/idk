...
public class TerrainTreeProxySpawner : MonoBehaviour
{
    ...
    public void SpawnMappedRuntimePrefabs()
    {
        ...
        if (mappingLookup.Count == 0)
        {
            ...
            {
                Debug.Log("TerrainTreeProxySpawner: No valid proxy mappings configured.", this);
            }
        ...
        for (int terrainIndex = 0; terrainIndex < terrains.Length; terrainIndex++)
        {
            ...
            if (!TryResolveMappedRuntimePrefabs(terrain.terrainData, mappingLookup, out GameObject[] runtimePrefabsByPrototypeIndex))
            {
                continue;
            }
            ...
            if (removePaintedProxyInstances)
            {
                ...
                if (!TryResolveMappedRuntimePrefabs(terrainData, mappingLookup, out runtimePrefabsByPrototypeIndex))
                {
                    continue;
                }
            }
    ...
    private Dictionary<GameObject, GameObject> BuildMappingLookup()
    {
        ...
        for (int i = 0; i < mappings.Count; i++)
        {
            ...
            if (mapping == null || mapping.terrainProxyPrefab == null || mapping.runtimePrefab == null)
            {
                continue;
            }
        ...
        out GameObject[] runtimePrefabsByPrototypeIndex)
    {
        ...
        for (int i = 0; i < prototypes.Length; i++)
        {
            ...
            if (!mappingLookup.TryGetValue(proxyPrefab, out GameObject runtimePrefab) || runtimePrefab == null)
            {
                continue;
            }
    ...
    private TerrainData EnsureRuntimeTerrainData(Terrain terrain)
    {
        ...
        if (_runtimeTerrainData.TryGetValue(terrain, out TerrainData cachedRuntimeData) && cachedRuntimeData != null)
        {
    ...
    private void SpawnRuntimePrefabForInstance(
        ...
        Transform parent)
    {
        if (terrain == null || terrainData == null || runtimePrefab == null || parent == null)
        {
            return;
        }
    ...
    private void RefreshRuntimeOptimizer(Transform resolvedSpawnParent)
    {
        ...
        ResHandler optimizer = FindFirstObjectByType<ResHandler>(FindObjectsInactive.Include);
#else
        ResHandler optimizer = FindObjectOfType<ResHandler>(true);
#endif
    ...
    private void ApplyTerrainPlacement(Transform target, Terrain terrain)
    {
        ...
        if (alignSpawnedObjectsToTerrainNormal)
        {
            AlignObjectToTerrainNormal(target, terrain);
        }

        if (snapSpawnedObjectsToTerrain)
        {
    ...
    private static bool TryEncapsulateRenderers(Transform target, bool includeInactive, ref Bounds bounds, ref bool hasBounds)
    {
        ...
        for (int i = 0; i < renderers.Length; i++)
        {
            ...
            if (!includeInactive && (!renderer.enabled || !renderer.gameObject.activeInHierarchy))
            {
                continue;
            }
    ...
    private static bool TryEncapsulateColliders(Transform target, bool includeInactive, ref Bounds bounds, ref bool hasBounds)
    {
        ...
        for (int i = 0; i < colliders.Length; i++)
        {
            ...
            if (!includeInactive && (!collider.enabled || !collider.gameObject.activeInHierarchy || collider.isTrigger))
            {
                continue;
            }
