...
public class TerrainTreeProxySpawner : MonoBehaviour
{
    ...
    [SerializeField] private bool logConversionSummary = true; [Header("Spawn Root")]
    [SerializeField] private Transform spawnParent;
    [SerializeField] private string spawnParentName = "ConvertedTerrainTrees";
    [SerializeField] private bool createSpawnParentIfMissing = true; [Header("Placement")]
    ...
    [SerializeField, Min(0f)] private float mineStoneEmbedDepth = 0.2f; [Header("Mappings")]
    ...
    private void Start()
    {
        if (!Application.isPlaying || !spawnOnStart) { return; }
        SpawnMappedRuntimePrefabs();
    }
    [ContextMenu("Spawn Mapped Runtime Prefabs")]
    public void SpawnMappedRuntimePrefabs()
    {
        Dictionary<GameObject, GameObject> mappingLookup = BuildMappingLookup();
        if (mappingLookup.Count == 0)
        {
            if (logConversionSummary) { Debug.Log("TerrainTreeProxySpawner: No valid proxy mappings configured.", this); }
            return;
        }
        ...
        int terrainsConverted = 0; int instancesConverted = 0;
        ...
        for (int terrainIndex = 0; terrainIndex < terrains.Length; terrainIndex++)
        {
            ...
            ? new List<TreeInstance>(treeInstances.Length) : null; int terrainConvertedCount = 0;
            for (int instanceIndex = 0; instanceIndex < treeInstances.Length; instanceIndex++)
            {
                ...
                if (instance.prototypeIndex < 0 || instance.prototypeIndex >= runtimePrefabsByPrototypeIndex.Length)
                {
                    if (removePaintedProxyInstances) { remainingInstances.Add(instance); }
                    continue;
                }
            ...
            terrainsConverted++; instancesConverted += terrainConvertedCount;
            if (removePaintedProxyInstances) { terrainData.treeInstances = remainingInstances.ToArray(); }
        }
        RefreshRuntimeOptimizer(resolvedSpawnParent); if (logConversionSummary)
        {
            Debug.Log(
    ...
    private bool TryResolveMappedRuntimePrefabs(TerrainData terrainData,
    ...
    out GameObject[] runtimePrefabsByPrototypeIndex)
    {
        ...
        bool hasMappedPrototype = false; for (int i = 0; i < prototypes.Length; i++)
        {
            ...
            runtimePrefabsByPrototypeIndex[i] = runtimePrefab; hasMappedPrototype = true;
        }
    ...
    private TerrainData EnsureRuntimeTerrainData(Terrain terrain)
    {
        ...
        TerrainData sourceData = terrain.terrainData; TerrainData runtimeData = sourceData;
#if UNITY_EDITOR
        if (cloneTerrainDataDuringPlayInEditor)
        {
            runtimeData = Instantiate(sourceData);
            runtimeData.name = $"{sourceData.name} Runtime"; terrain.terrainData = runtimeData;
            TerrainCollider terrainCollider = terrain.GetComponent<TerrainCollider>();
            if (terrainCollider != null) { terrainCollider.terrainData = runtimeData; }
        }
#endif
        _runtimeTerrainData[terrain] = runtimeData; return runtimeData;
    }
    private void SpawnRuntimePrefabForInstance(Terrain terrain, TerrainData terrainData,
    TreeInstance instance, GameObject runtimePrefab,
    Transform parent)
    {
        if (terrain == null || terrainData == null || runtimePrefab == null || parent == null) { return; }
        ...
        spawned.transform.localScale = new Vector3(baseScale.x * instance.widthScale,
    ...
    private Transform ResolveSpawnParent()
    {
        ...
        {
            GameObject existingRoot = GameObject.Find(spawnParentName); if (existingRoot != null)
            {
                spawnParent = existingRoot.transform; return spawnParent;
            }
        }
        ...
        GameObject createdRoot = new GameObject(rootName); spawnParent = createdRoot.transform;
        return spawnParent;
    }
    private void RefreshRuntimeOptimizer(Transform resolvedSpawnParent)
    {
        if (resolvedSpawnParent == null) { return; }
