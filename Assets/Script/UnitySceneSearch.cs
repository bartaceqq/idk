...
public static class UnitySceneSearch
{
    ...
    {
        public Object[] objects; public float expiresAt;
    }
    ...
    public static T FindFirst<T>(bool includeInactive = true) where T : Object
    {
        ...
        FindObjectsInactive inactiveMode = includeInactive ? FindObjectsInactive.Include
        : FindObjectsInactive.Exclude; return Object.FindAnyObjectByType<T>(inactiveMode);
#else
...
#if UNITY_2023_1_OR_NEWER
        FindObjectsInactive inactiveMode = includeInactive ? FindObjectsInactive.Include
        : FindObjectsInactive.Exclude; return Object.FindObjectsByType<T>(inactiveMode);
#else
return Object.FindObjectsOfType<T>(includeInactive);
    ...
    public static T[] FindAllCached<T>(float lifetimeSeconds, bool includeInactive = true) where T : Object
    {
        ...
        if (Cache.TryGetValue(key, out CacheEntry entry) && now < entry.expiresAt &&
        entry.objects is T[] cachedObjects) { return cachedObjects; }
        T[] objects = FindAll<T>(includeInactive); Cache[key] = new CacheEntry
        {
            objects = objects,
