...
public class TerrainTreeToPrefabConverter : MonoBehaviour {
    ...
    private void Start() { if (!Application.isPlaying || !convertPaintedTreesOnStart) { return; }

        ConvertPaintedTreesToPrefabs(); }

    [ContextMenu("Convert Painted Trees To Prefabs")] // Handle Convert Painted Trees To Prefabs.
    public void ConvertPaintedTreesToPrefabs() { if (targetTerrain == null) { targetTerrain = GetComponent<Terrain>(); }
    ...
    private TerrainData GetEditableTerrainData(Terrain terrain) { if (terrain == null || terrain.terrainData == null) { return null; }
    ...
    public void ConvertPrefabsBackToTerrainTrees() { if (targetTerrain == null) { targetTerrain = GetComponent<Terrain>(); }
    ...
    public void SnapExistingConvertedObjectsToTerrain() { if (targetTerrain == null) { targetTerrain = GetComponent<Terrain>(); }
    ...
    private void ConvertDetailMeshesToPrefabs(Terrain terrain, Transform parent) {
        ...
        if (parent == null) {
            ...
            detailRoot = root; } else { detailRoot = parent.gameObject; }
        ...
        for (int protoIndex = 0; protoIndex < detailPrototypes.Length; protoIndex++) {
            ...
            if (protoPrefab == null) { if (debugDetailConversion) { Debug.Log($"TerrainTreeToPrefabConverter: Detail prototype {protoIndex} has no prefab (skip).", this); }
                continue; }
            ...
            for (int z = 0; z < detailHeight; z++) {
                for (int x = 0; x < detailWidth; x++) {
                    ...
                    for (int i = 0; i < cappedCount; i++) { if (maxDetailInstancesTotal > 0 && converted >= maxDetailInstancesTotal) { goto Done; }
                        ...
                        if (deterministicDetailPlacement) {
                            ...
                            heightScale = Mathf.Lerp(proto.minHeight, proto.maxHeight, h5); } else {
...
Done:
    ...
    private static int SumDetailLayer(int[,] layer) { if (layer == null) { return 0; }
    ...
    private bool ShouldConvertDetailPrefab(GameObject protoPrefab) { if (protoPrefab == null) { return false; }
    ...
    private static string NormalizePrefabName(string rawName) { if (string.IsNullOrWhiteSpace(rawName)) { return string.Empty; }
    ...
    private static GameObject GetDetailPrototypePrefab(DetailPrototype proto) { if (proto == null) { return null; }
    ...
    private static int ResolvePrototypeIndexForRestoredObject(GameObject candidate, TreePrototype[] prototypes, Dictionary<string, int> prototypeByName) { if (candidate == null) { return -1; }
    ...
    private void AssignConvertedObjectReferences(GameObject convertedObject) { if (convertedObject == null) { return; }
    ...
    private bool ShouldRandomizeBranch01Yaw(GameObject prototypePrefab) { if (!randomizeBranch01Yaw || prototypePrefab == null || string.IsNullOrWhiteSpace(branch01PrototypeName)) { return false; }

        return MatchesPrototypeName(prototypePrefab, branch01PrototypeName); }

    private static bool MatchesPrototypeName(GameObject prototypePrefab, string prototypeName) { if (prototypePrefab == null || string.IsNullOrWhiteSpace(prototypeName)) { return false; }
    ...
    private static void AlignObjectToTerrainNormal(Transform target, Terrain terrain) { if (target == null || terrain == null || terrain.terrainData == null) { return; }
