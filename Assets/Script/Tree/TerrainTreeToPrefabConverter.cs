...
{
    [Header("Converted Resource References")] public InventoryAddHandler inventoryAddHandler;
    [SerializeField] private Terrain targetTerrain;
    [SerializeField] private Transform parentForSpawnedTrees; [Header("Placement")]
    ...
    [SerializeField] private int branch01RotationSeed = 9143; [Header("Auto Conversion")]
    ...
    private void Start()
    {
        if (!Application.isPlaying || !convertPaintedTreesOnStart) { return; }
        ConvertPaintedTreesToPrefabs();
    }
    [ContextMenu("Convert Painted Trees To Prefabs")] // Handle Convert Painted Trees To Prefabs.
    public void ConvertPaintedTreesToPrefabs()
    {
        ...
        if (!hasTreeInstances) { Debug.Log("TerrainTreeToPrefabConverter: No painted terrain trees to convert.", this); }
        List<TreeInstance> remainingInstances = hasTreeInstances
        ? new List<TreeInstance>(treeInstances.Length) : new List<TreeInstance>();
        List<ConvertedTreeRecord> existingConvertedTrees = skipSpawnForExistingConvertedObjects
        ? BuildExistingConvertedTreeRecords(parentForSpawnedTrees) : null; int convertedCount = 0;
        int spawnedCount = 0; int matchedExistingCount = 0;
        for (int i = 0; i < (hasTreeInstances ? treeInstances.Length : 0); i++)
        {
            ...
            {
                remainingInstances.Add(instance); continue;
            }
            GameObject prototypePrefab = prototypes[instance.prototypeIndex].prefab;
            if (prototypePrefab == null) { remainingInstances.Add(instance); continue; }
            ...
            if (TryGetExistingConvertedObject(existingConvertedTrees, prototypePrefab, worldPosition, out GameObject existingConvertedObject))
            {
                ...
                matchedExistingCount++; convertedCount++;
            }
            else
            {
                ...
                spawnedTree.transform.localScale = new Vector3(baseScale.x * instance.widthScale,
                baseScale.y * instance.heightScale, baseScale.z * instance.widthScale);
                ...
                NormalizePrefabName(prototypePrefab.name), spawnedTree.transform.position,
                spawnedTree));
                }
                spawnedCount++; convertedCount++;
            }
            if (!clearConvertedTerrainTrees) { remainingInstances.Add(instance); }
        }
        if (clearConvertedTerrainTrees && hasTreeInstances) { terrainData.treeInstances = remainingInstances.ToArray(); }
        if (hasTreeInstances)
        {
            Debug.Log(
        $"TerrainTreeToPrefabConverter: Converted {convertedCount} tree instances. " +
        $"Spawned={spawnedCount}, Existing={matchedExistingCount}.", this);
        }
        if (convertDetailMeshes) { ConvertDetailMeshesToPrefabs(targetTerrain, parentForSpawnedTrees); }
    }
    private TerrainData GetEditableTerrainData(Terrain terrain)
    {
        if (terrain == null || terrain.terrainData == null) { return null; }
        if (!Application.isPlaying || !cloneTerrainDataForRuntimeConversion) { return terrain.terrainData; }
        if (runtimeTerrainDataByTerrain.TryGetValue(terrain, out TerrainData existingRuntimeData) && existingRuntimeData != null) { return existingRuntimeData; }
        TerrainData runtimeData = Instantiate(terrain.terrainData);
        runtimeData.name = terrain.terrainData.name + "_Runtime";
        terrain.terrainData = runtimeData; runtimeTerrainDataByTerrain[terrain] = runtimeData;
        return runtimeData;
    }
    [ContextMenu("Convert Prefabs Back To Terrain Trees")] // Handle Convert Prefabs Back To Terrain Trees.
    public void ConvertPrefabsBackToTerrainTrees()
    {
        ...
        ? new List<TreeInstance>(terrainData.treeInstances) : new List<TreeInstance>();
        ...
        int restoredCount = 0; int skippedCount = 0; int outsideTerrainCount = 0;
        for (int i = 0; i < candidates.Count; i++)
        {
            GameObject candidate = candidates[i];
            if (candidate == null) { skippedCount++; continue; }
            int prototypeIndex = ResolvePrototypeIndexForRestoredObject(candidate, prototypes, prototypeByName);
            if (prototypeIndex < 0) { skippedCount++; continue; }
            ...
            {
                outsideTerrainCount++; continue;
            }
            result.Add(treeInstance); restoredCount++;
        ...
        if (removeRestoredGameObjects && clearParentAfterRestore && parentForSpawnedTrees != null)
        {
            ...
            {
                Transform child = parentForSpawnedTrees.GetChild(i); if (child == null) { continue; }
    ...
    [ContextMenu("Snap Existing Converted Objects To Terrain")] // Handle Snap Existing Converted Objects To Terrain.
    public void SnapExistingConvertedObjectsToTerrain()
    {
        ...
        int snappedCount = 0; for (int i = 0; i < candidates.Count; i++)
        {
            GameObject candidate = candidates[i]; if (candidate == null) { continue; }
            ApplyTerrainPlacement(candidate.transform, targetTerrain); snappedCount++;
        }
    ...
    private void ConvertDetailMeshesToPrefabs(Terrain terrain, Transform parent)
    {
        ...
        {
            Debug.Log("TerrainTreeToPrefabConverter: No detail prototypes found.", this); return;
        }
        int detailWidth = data.detailWidth; int detailHeight = data.detailHeight;
        if (detailWidth <= 0 || detailHeight <= 0)
        {
            Debug.LogWarning("TerrainTreeToPrefabConverter: Detail resolution is invalid.", this);
            return;
        }
        GameObject detailRoot = null; if (parent == null)
        {
            GameObject root = new GameObject("ConvertedTerrainDetails");
            root.transform.SetParent(terrain.transform, false);
            detailRoot = root;
        }
        else { detailRoot = parent.gameObject; }
        int converted = 0;
        float stepX = data.size.x / detailWidth; float stepZ = data.size.z / detailHeight;
        for (int protoIndex = 0; protoIndex < detailPrototypes.Length; protoIndex++)
        {
            ...
            if (protoPrefab == null)
            {
                if (debugDetailConversion) { Debug.Log($"TerrainTreeToPrefabConverter: Detail prototype {protoIndex} has no prefab (skip).", this); }
                continue;
            }
            bool shouldConvert = ShouldConvertDetailPrefab(protoPrefab);
            if (debugDetailConversion) { Debug.Log($"TerrainTreeToPrefabConverter: Detail prototype {protoIndex} prefab '{protoPrefab.name}' convert={shouldConvert}.", this); }
            ...
            if (debugDetailConversion) { Debug.Log($"TerrainTreeToPrefabConverter: Detail prototype {protoIndex} count={layerCount}.", this); }
            bool anyConverted = false; for (int z = 0; z < detailHeight; z++)
            {
                for (int x = 0; x < detailWidth; x++)
                {
                    ...
                    {
                        if (maxDetailInstancesTotal > 0 && converted >= maxDetailInstancesTotal) { goto Done; }
                        float jitterX; float jitterZ; float rotY; float widthScale; float heightScale;
                        if (deterministicDetailPlacement)
                        {
                            ...
                            rotY = h3 * 360f; widthScale = Mathf.Lerp(proto.minWidth, proto.maxWidth, h4);
                            heightScale = Mathf.Lerp(proto.minHeight, proto.maxHeight, h5);
                        }
                        else
                        {
                            jitterX = Random.value; jitterZ = Random.value; rotY = Random.Range(0f, 360f);
                            widthScale = Random.Range(proto.minWidth, proto.maxWidth);
                            heightScale = Random.Range(proto.minHeight, proto.maxHeight);
                        }
                        float offsetX = (x + jitterX) * stepX; float offsetZ = (z + jitterZ) * stepZ;
                        ...
                        ApplyTerrainPlacement(spawned.transform, terrain); converted++;
                    }
                    if (clearConvertedDetailMeshes) { layer[z, x] = 0; }
                }
            }
            if (clearConvertedDetailMeshes && anyConverted) { data.SetDetailLayer(0, 0, protoIndex, layer); }
        }
    Done:
    ...
    private static int SumDetailLayer(int[,] layer)
    {
        ...
        int total = 0; int width = layer.GetLength(1); int height = layer.GetLength(0);
        for (int z = 0; z < height; z++) { for (int x = 0; x < width; x++) { total += layer[z, x]; } }
        return total;
    }
    private readonly struct ConvertedTreeRecord
    {
        public readonly string normalizedPrefabName; public readonly Vector3 position;
        public readonly GameObject gameObject;
        public ConvertedTreeRecord(string normalizedPrefabName, Vector3 position, GameObject gameObject)
        {
            this.normalizedPrefabName = normalizedPrefabName; this.position = position;
    ...
    private bool TryGetExistingConvertedObject(
        List<ConvertedTreeRecord> existingConvertedTrees, GameObject prototypePrefab,
        Vector3 worldPosition, out GameObject existingConvertedObject)
    {
        existingConvertedObject = null;
        if (existingConvertedTrees == null || prototypePrefab == null) { return false; }
        string prototypeName = NormalizePrefabName(prototypePrefab.name);
        if (string.IsNullOrEmpty(prototypeName)) { return false; }
        ...
        for (int i = 0; i < existingConvertedTrees.Count; i++)
        {
            ...
            if (record.gameObject == null || record.normalizedPrefabName != prototypeName) { continue; }
            float dx = record.position.x - worldPosition.x;
            float dz = record.position.z - worldPosition.z;
            if ((dx * dx) + (dz * dz) > matchRadiusSqr) { continue; }
            existingConvertedObject = record.gameObject; return true;
        }
        return false;
    }
    private bool ShouldConvertDetailPrefab(GameObject protoPrefab)
    {
        if (protoPrefab == null) { return false; }
        if (includePickableItems && protoPrefab.GetComponentInChildren<InventoryItem>(true) != null) { return true; }
        if (detailPrefabsToConvert == null || detailPrefabsToConvert.Count == 0) { return false; }
        if (detailPrefabsToConvert.Contains(protoPrefab)) { return true; }
        string protoName = NormalizePrefabName(protoPrefab.name);
        for (int i = 0; i < detailPrefabsToConvert.Count; i++)
        {
            GameObject candidate = detailPrefabsToConvert[i]; if (candidate == null) { continue; }
            if (NormalizePrefabName(candidate.name) == protoName) { return true; }
        }
        return false;
    }
    private static string NormalizePrefabName(string rawName)
    {
        ...
        name = name.Replace(" variant", string.Empty).Trim(); return name;
    }
    private static GameObject GetDetailPrototypePrefab(DetailPrototype proto)
    {
        if (proto == null) { return null; }
#if UNITY_2021_2_OR_NEWER
        if (proto.usePrototypeMesh && proto.prototype != null) { return proto.prototype; }
#else
if (proto.prototype != null) { return proto.prototype; }
#endif
        return null;
    }
    private static Dictionary<string, int> BuildPrototypeNameMap(TreePrototype[] prototypes)
    {
        ...
        for (int i = 0; i < prototypes.Length; i++)
        {
            ...
            if (proto == null || proto.prefab == null) { continue; }
    ...
    {
        List<GameObject> result = new List<GameObject>(); if (root == null) { return result; }
    ...
    private static int ResolvePrototypeIndexForRestoredObject(GameObject candidate, TreePrototype[] prototypes, Dictionary<string, int> prototypeByName)
    {
        ...
        if (source != null)
        {
            ...
            {
                TreePrototype proto = prototypes[i];
                if (proto != null && proto.prefab == source) { return i; }
            }
        }
#endif
    ...
    private static bool TryCreateTreeInstanceFromObject(Transform source, Terrain terrain,
    TreePrototype[] prototypes, int prototypeIndex, out TreeInstance treeInstance)
    {
        treeInstance = default;
        if (source == null || terrain == null || terrain.terrainData == null) { return false; }
        TerrainData data = terrain.terrainData;
        Vector3 local = source.position - terrain.transform.position;
        Vector3 normalized = new Vector3(data.size.x > 0f ? local.x / data.size.x : 0f,
        ...
        normalized.x = Mathf.Clamp01(normalized.x); normalized.y = Mathf.Clamp01(normalized.y);
        normalized.z = Mathf.Clamp01(normalized.z); Vector3 sourceScale = source.localScale;
        Vector3 prefabScale = Vector3.one; TreePrototype prototype = prototypes[prototypeIndex];
        if (prototype != null && prototype.prefab != null) { prefabScale = prototype.prefab.transform.localScale; }
        ...
        ? Mathf.Max(0.01f, sourceScale.y / prefabScale.y) : 1f; treeInstance = new TreeInstance
        {
            position = normalized,
    ...
    private void AssignConvertedObjectReferences(GameObject convertedObject)
    {
        ...
        if (treeHandlers == null || treeHandlers.Length == 0) { return; }
        ...
        for (int i = 0; i < treeHandlers.Length; i++)
        {
            ...
            if (treeHandler == null) { continue; }
            if (resolvedAddHandler != null) { treeHandler.inventoryAddHandler = resolvedAddHandler; }
            InventoryItem resolvedItem = ResolveInventoryItem(treeHandler, convertedObject);
            if (resolvedItem != null) { treeHandler.inventoryItem = resolvedItem; }
        }
    }
    ...
    {
        if (!randomizeBranch01Yaw || prototypePrefab == null || string.IsNullOrWhiteSpace(branch01PrototypeName)) { return false; }
    ...
    {
        if (prototypePrefab == null || string.IsNullOrWhiteSpace(prototypeName)) { return false; }
    ...
    private InventoryAddHandler ResolveInventoryAddHandler()
    {
        ...
        {
            cachedInventoryAddHandler = UnitySceneSearch.FindFirst<InventoryAddHandler>();
        }
    ...
    {
        if (treeHandler.inventoryItem != null) { return treeHandler.inventoryItem; }
        InventoryItem resolvedItem = treeHandler.GetComponent<InventoryItem>();
        if (resolvedItem == null) { resolvedItem = treeHandler.GetComponentInChildren<InventoryItem>(true); }
        if (resolvedItem == null && convertedObject != null) { resolvedItem = convertedObject.GetComponentInChildren<InventoryItem>(true); }
        if (resolvedItem != null) { resolvedItem.ResolveReferences(); }
        return resolvedItem;
    }
    private void ApplyTerrainPlacement(Transform target, Terrain terrain)
    {
        if (target == null || terrain == null || terrain.terrainData == null) { return; }
        if (alignConvertedObjectsToTerrainNormal) { AlignObjectToTerrainNormal(target, terrain); }
        if (snapConvertedObjectsToTerrain) { SnapObjectBaseToTerrain(target, terrain); }
    }
    ...
    {
        if (mineStoneEmbedDepth <= 0f || target == null) { return 0f; }
        return target.GetComponentInChildren<MineStone>(true) != null ? mineStoneEmbedDepth
        : 0f;
    }
    private static bool TryGetGroundingBounds(Transform target, out Bounds bounds)
    {
        if (TryGetMineStoneBounds(target, out bounds)) { return true; }
    ...
    private static bool TryGetMineStoneBounds(Transform target, out Bounds bounds)
    {
        ...
        if (mineStone.mainstoneparts != null && mineStone.mainstoneparts.Length > 0)
        {
            ...
            {
                GameObject part = mineStone.mainstoneparts[i]; if (part == null) { continue; }
    ...
    {
        if (target == null) { return hasBounds; }
        Renderer[] renderers = target.GetComponentsInChildren<Renderer>(includeInactive);
        for (int i = 0; i < renderers.Length; i++)
        {
            Renderer currentRenderer = renderers[i];
            if (currentRenderer == null) { continue; }
            if (!includeInactive && (!currentRenderer.enabled || !currentRenderer.gameObject.activeInHierarchy)) { continue; }
            if (!hasBounds)
            {
    ...
    private static bool TryEncapsulateColliders(Transform target, bool includeInactive, ref Bounds bounds, ref bool hasBounds)
    {
        ...
        for (int i = 0; i < colliders.Length; i++)
        {
            ...
            if (!includeInactive && (!currentCollider.enabled || !currentCollider.gameObject.activeInHierarchy || currentCollider.isTrigger)) { continue; }
            if (!hasBounds)
            {
    ...
    private static float HashTo01(int seed, int a, int b, int c, int d, int e)
    {
        unchecked
        {
            int h = seed; h = h * 31 + a; h = h * 31 + b; h = h * 31 + c; h = h * 31 + d;
            h = h * 31 + e; uint uh = (uint)h; return (uh % 1000003) / 1000003f;
        }
    }
}
