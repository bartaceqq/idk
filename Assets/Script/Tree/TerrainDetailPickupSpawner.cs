...
{
    [SerializeField] private Terrain targetTerrain; [SerializeField] private Transform player;
    [SerializeField] private List<DetailPickupConfig> pickupConfigs = new List<DetailPickupConfig>();
    [Header("Spawn Settings")][SerializeField] private float spawnRadius = 8f;
    ...
    private int cachedDetailIndex = -1; private float _nextSpawnRefreshTime;
    private float _nextResolveReferencesTime; private void Awake() { ResolveReferences(); }
    private void Update()
    {
        ...
        if (targetTerrain == null || player == null || pickupConfigs == null || pickupConfigs.Count == 0) { return; }
        if (Time.time < _nextSpawnRefreshTime) { return; }
        ...
        for (int i = 0; i < pickupConfigs.Count; i++)
        {
            ...
            if (config == null || config.pickupPrefab == null) { continue; }
            int detailIndex = ResolveDetailPrototypeIndex(config); if (detailIndex < 0) { continue; }
    ...
    {
        if (config.cachedDetailIndex >= 0) { return config.cachedDetailIndex; }
        if (config.detailPrototypeIndex >= 0)
        {
            config.cachedDetailIndex = config.detailPrototypeIndex; return config.cachedDetailIndex;
        }
        if (targetTerrain == null || targetTerrain.terrainData == null || config.detailPrototypePrefab == null) { return -1; }
    ...
    {
        if (active.Count >= maxSpawnedActive) { return; }
        TerrainData data = targetTerrain.terrainData; int detailWidth = data.detailWidth;
        ...
        float cellSizeX = data.size.x / detailWidth; float cellSizeZ = data.size.z / detailHeight;
        ...
        int width = maxX - minX + 1; int height = maxZ - minZ + 1;
        ...
        for (int z = 0; z < height; z++)
        {
            ...
            {
                if (active.Count >= maxSpawnedActive) { return; }
                int count = layer[z, x]; if (count <= 0) { continue; }
                int cellX = minX + x;
                int cellZ = minZ + z; long key = PackKey(cellX, cellZ);
                ...
                for (int i = 0; i < toSpawn; i++)
                {
                    ...
                    if (item != null) { item.ResolveReferences(); }
                    DetailPickupMarker marker = spawned.GetComponent<DetailPickupMarker>();
                    if (marker == null) { marker = spawned.AddComponent<DetailPickupMarker>(); }
    ...
    private void DespawnFar()
    {
        ...
        float maxDistanceSqr = despawnRadius * despawnRadius; Vector3 playerPos = player.position;
        List<long> toRemove = null;
        foreach (KeyValuePair<long, DetailPickupMarker> pair in active)
        {
            DetailPickupMarker marker = pair.Value; if (marker == null) { continue; }
        ...
        if (toRemove != null) { for (int i = 0; i < toRemove.Count; i++) { active.Remove(toRemove[i]); } }
    }
    ...
    {
        float jitterX; float jitterZ; if (deterministicPlacement)
        {
        ...
        {
            jitterX = Random.value; jitterZ = Random.value;
        }
    ...
    {
        if (!deterministicPlacement) { return Random.Range(0f, 360f); }
        float h = HashTo01(placementSeed, 0, cellX, cellZ, index, 3); return h * 360f;
    }
    private static long PackKey(int x, int z) { return ((long)x << 32) ^ (uint)z; }
...
if (proto.prototype != null) { return proto.prototype; }
    ...
    {
        public GameObject detailPrototypePrefab; public GameObject pickupPrefab;
