...
public class TerrainDetailPickupSpawner : MonoBehaviour {
    ...
    private int ResolveDetailPrototypeIndex(DetailPickupConfig config) { if (config.cachedDetailIndex >= 0) { return config.cachedDetailIndex; }
    ...
    private void SpawnNearby(int detailIndex, DetailPickupConfig config) { if (active.Count >= maxSpawnedActive) { return; }
        ...
        for (int z = 0; z < height; z++) {
            for (int x = 0; x < width; x++) { if (active.Count >= maxSpawnedActive) { return; }
                ...
                for (int i = 0; i < toSpawn; i++) { if (active.Count >= maxSpawnedActive) { return; }
    ...
    private void DespawnFar() { if (active.Count == 0) { return; }
    ...
    private Vector3 DetailCellToWorld(TerrainData data, int cellX, int cellZ, int index, int detailIndex, float cellSizeX, float cellSizeZ) {
        ...
        if (deterministicPlacement) {
            ...
            jitterZ = Mathf.Lerp(0.5f - cellJitter * 0.5f, 0.5f + cellJitter * 0.5f, h2); } else {
    ...
    private float SampleRotation(int cellX, int cellZ, int index) { if (!deterministicPlacement) { return Random.Range(0f, 360f); }
    ...
    [System.Serializable] public class DetailPickupConfig {
