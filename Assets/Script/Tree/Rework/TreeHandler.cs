...
public class TreeHandler : MonoBehaviour
{
    ...
    public InventoryAddHandler inventoryAddHandler; public InventoryItem inventoryItem;
    public int placeinlist = 0; public int idk;
    private static PhysicsMaterial fallbackNoRollMaterial;
    private static InventoryAddHandler cachedInventoryAddHandler;
    public List<Material> materials = new List<Material>(); public int test;
    [Header("Parts")] public float timetowaittodestroy = 8f; public GameObject toppart;
    public GameObject bottompart; public int counttochop = 3; [Header("Fall Physics")]
    ...
    [Header("Chop")][SerializeField] private int chopsToFall = 4;
    ...
    private int chopCount; private int defaultChopsToFall; private float nextChopAllowedTime;
    private bool hasFallen; public void Start()
    {
        ResolveReferences();
    ...
    {
        if (toppart == null || materials == null || materials.Count == 0) { return; }
        MeshRenderer renderer = toppart.GetComponent<MeshRenderer>();
        if (renderer == null) { return; }
        Material[] rendmatlist = (Material[])renderer.materials.Clone();
        if (placeinlist < 0 || placeinlist >= rendmatlist.Length) { return; }
        rendmatlist[placeinlist] = materials[Random.Range(0, materials.Count)];
        renderer.materials = rendmatlist;
    }
    public void Chop() { Chop(null); }
    public void Chop(Transform attacker)
    {
        ResolveReferences(); if (hasFallen) { return; }
        if (Time.time < nextChopAllowedTime) { return; }
        nextChopAllowedTime = Time.time + Mathf.Max(0f, hitCooldownSeconds);
        int requiredChops = ResolveRequiredChops(attacker); chopCount++;
        SpawnChopImpact(attacker); if (chopCount < requiredChops)
        {
            counttochop = Mathf.Max(0, requiredChops - chopCount); return;
        }
        hasFallen = true;
    ...
    {
        yield return new WaitForSeconds(timetowaittodestroy); Destroy(toppart);
    }
    public void TreeFall() { TreeFall(null); }
    public void TreeFall(Transform attacker)
    {
        ...
        PrepareTopPartCollidersForFall(topTransform); IgnoreOwningTreeColliders(topTransform);
        Rigidbody topRigidbody = toppart.GetComponent<Rigidbody>();
        if (topRigidbody == null) { topRigidbody = toppart.AddComponent<Rigidbody>(); }
        ConfigureTopPartRigidbody(topRigidbody); AddFallImpulse(topRigidbody, fallDirection);
    ...
    private Vector3 ResolveFallDirection(Transform attacker)
    {
        ...
        if (attacker != null) { fallDirection = transform.position - attacker.position; } else if (Camera.main != null) { fallDirection = transform.position - Camera.main.transform.position; }
        fallDirection.y = 0f;
    ...
    private void TiltTopPartTowardDirection(Transform topTransform, Vector3 fallDirection)
    {
        ...
        if (tiltAxis.sqrMagnitude <= 0.0001f) { tiltAxis = topTransform.right; }
    ...
    {
        if (topRigidbody == null) { return; }
        float topHeight = 1f;
        if (TryGetBounds(toppart.transform, out Bounds bounds)) { topHeight = Mathf.Max(1f, bounds.size.y); }
        ...
        if (torqueAxis.sqrMagnitude > 0.0001f) { topRigidbody.AddTorque(torqueAxis.normalized * Mathf.Max(0.5f, topRigidbody.mass), ForceMode.Impulse); }
    }
    private void SpawnChopImpact(Transform attacker)
    {
        if (chopImpactParticlePrefab == null) { return; }
        ...
        if (flattenImpactFacingToHorizontal) { lookDirection.y = 0f; }
        ...
        if (impactInstance != null && destroyDelay > 0f) { Destroy(impactInstance, destroyDelay); }
    }
    private Vector3 ResolveTreeImpactPoint(Vector3 attackerPosition)
    {
        if (bottompart != null && TryResolveClosestImpactPoint(bottompart.transform, attackerPosition, out Vector3 closestImpactPoint)) { return closestImpactPoint; }
    ...
    private Vector3 ResolveTreeFallbackPoint() { return transform.position + Vector3.up * 0.5f; }
    ...
    {
        int tierChops = ResolveAxeTierChops(attacker); if (tierChops > 0) { return tierChops; }
    ...
    private void PrepareTopPartCollidersForFall(Transform topTransform)
    {
        ...
        for (int i = 0; i < capsuleColliders.Length; i++) { ReplaceCapsuleColliderForFall(capsuleColliders[i], noRollMaterial); }
        Collider[] colliders = topTransform.GetComponentsInChildren<Collider>(true);
        bool hasSolidCollider = false; for (int i = 0; i < colliders.Length; i++)
        {
            Collider currentCollider = colliders[i];
            if (currentCollider == null || !currentCollider.enabled || currentCollider.isTrigger) { continue; }
        ...
        if (!hasSolidCollider) { TryEnsureFallbackBoxCollider(topTransform, noRollMaterial); }
    }
    private static void ReplaceCapsuleColliderForFall(CapsuleCollider capsuleCollider, PhysicsMaterial noRollMaterial)
    {
        if (capsuleCollider == null || !capsuleCollider.enabled || capsuleCollider.isTrigger) { return; }
        bool replacementCreated = TryEnsureConvexMeshCollider(capsuleCollider.gameObject, noRollMaterial);
        if (!replacementCreated) { replacementCreated = TryEnsureFallbackBoxCollider(capsuleCollider.transform, noRollMaterial); }
        if (replacementCreated) { capsuleCollider.enabled = false; return; }
    ...
    private static bool TryEnsureConvexMeshCollider(GameObject target, PhysicsMaterial noRollMaterial)
    {
        ...
        meshCollider.providesContacts = true; meshCollider.material = noRollMaterial;
        meshCollider.enabled = true; return true;
    }
    private static bool TryEnsureFallbackBoxCollider(Transform target, PhysicsMaterial noRollMaterial)
    {
        if (target == null || !TryGetLocalBounds(target, out Bounds localBounds)) { return false; }
        ...
        if (boxCollider == null) { boxCollider = target.gameObject.AddComponent<BoxCollider>(); }
        boxCollider.center = localBounds.center; boxCollider.size = size;
        boxCollider.material = noRollMaterial; boxCollider.enabled = true; return true;
    }
    private void IgnoreOwningTreeColliders(Transform fallingTop)
    {
        if (fallingTop == null) { return; }
        ...
        for (int i = 0; i < topColliders.Length; i++)
        {
            ...
            if (topCollider == null || !topCollider.enabled) { continue; }
            for (int j = 0; j < rootColliders.Length; j++)
            {
                Collider rootCollider = rootColliders[j];
                if (rootCollider == null || !rootCollider.enabled || rootCollider == topCollider) { continue; }
    ...
    {
        if (topRigidbody == null) { return; }
        topRigidbody.linearVelocity = Vector3.zero; topRigidbody.angularVelocity = Vector3.zero;
        topRigidbody.useGravity = true; topRigidbody.isKinematic = false;
        ...
        topRigidbody.ResetCenterOfMass(); topRigidbody.ResetInertiaTensor();
    }
    private void ConfigureGroundDespawnOnTopPart()
    {
        if (toppart == null) { return; }
        TreeTopGroundDespawn groundDespawn = toppart.GetComponent<TreeTopGroundDespawn>();
        if (groundDespawn == null) { groundDespawn = toppart.AddComponent<TreeTopGroundDespawn>(); }
    ...
    {
        if (fallbackNoRollMaterial != null) { return fallbackNoRollMaterial; }
...
sealed class TreeTopGroundDespawn : MonoBehaviour
{
    ...
    private float despawnDelaySeconds = 1f; private float minimumGroundNormalY = 0.35f;
    private bool despawnScheduled;
    public void Configure(Transform treeRoot, float delaySeconds, float groundNormalY)
    {
        owningTreeRoot = treeRoot; despawnDelaySeconds = Mathf.Max(0f, delaySeconds);
        minimumGroundNormalY = Mathf.Clamp01(groundNormalY); despawnScheduled = false;
    }
    private void OnCollisionEnter(Collision collision) { TryScheduleDespawn(collision); }
    private void OnCollisionStay(Collision collision) { TryScheduleDespawn(collision); }
    private void TryScheduleDespawn(Collision collision)
    {
        if (despawnScheduled || !IsGroundHit(collision)) { return; }
        despawnScheduled = true; Destroy(gameObject, despawnDelaySeconds);
    }
    private bool IsGroundHit(Collision collision)
    {
        ...
        if (owningTreeRoot != null && collision.collider.transform.IsChildOf(owningTreeRoot)) { return false; }
        if (collision.collider is TerrainCollider) { return true; }
        for (int i = 0; i < collision.contactCount; i++) { if (collision.GetContact(i).normal.y >= minimumGroundNormalY) { return true; } }
        return false;
    }
}
