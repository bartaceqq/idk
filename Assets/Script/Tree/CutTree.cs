...
public class CutTree : MonoBehaviour
{
    ...
    private static GameObject cachedDestructionVfxPrefab; public string texttoshow;
    public Sprite sprite; public InfoHandler infoHandler;
    public List<GameObject> treeparts = new List<GameObject>();
    public GameObject topofthetree; [SerializeField] private float destroyDelaySeconds = 1f;
    ...
    public InventoryItem inventoryItem; public bool broken = false;
    ...
    private Rigidbody topRigidbody; private MeshCollider topMeshCollider;
    private bool topInitialUseGravity; private bool topInitialIsKinematic;
    private bool topInitialConvex; private bool topInitialProvidesContacts;
    private bool isRebuilding; private bool topHadInitialRigidbody;
    ...
    {
        if (destructionVfxPrefab != null) { return destructionVfxPrefab; }
    ...
    {
        bounds = default; if (source == null) { return false; }
        ...
        for (int i = 0; i < renderers.Length; i++)
        {
            ...
            {
                bounds = currentRenderer.bounds; continue;
            }
    ...
    private void ResolveReferences()
    {
        ...
        else { cachedSlotManager = inventoryItem.slotManager; }
    }
    ...
    {
        initialTreeParts.Clear(); initialTransforms.Clear();
        foreach (GameObject treePart in treeparts)
        {
            if (treePart == null) { continue; }
            initialTreeParts.Add(treePart); CacheTransform(treePart.transform);
        }
        if (topofthetree != null)
        {
            ...
            topHadInitialRigidbody = topRigidbody != null; if (topRigidbody != null)
            {
    ...
    {
        if (targetTransform == null || initialTransforms.ContainsKey(targetTransform)) { return; }
    ...
    {
        if (targetTransform == null || !initialTransforms.TryGetValue(targetTransform, out TransformSnapshot snapshot)) { return; }
    ...
    {
        yield return new WaitForSeconds(delaySeconds); RebuildTree();
    }
    private void RebuildTree()
    {
        broken = false; isRebuilding = false; treeparts.Clear();
        treeparts.AddRange(initialTreeParts);
        foreach (GameObject treePart in initialTreeParts)
        {
            if (treePart == null) { continue; }
            RestoreTransform(treePart.transform); treePart.SetActive(true);
            SetTreePartVisible(treePart, true);
        }
        if (topofthetree != null)
        {
            RestoreTransform(topofthetree.transform); topofthetree.SetActive(true);
        }
        if (topRigidbody != null)
        {
            topRigidbody.linearVelocity = Vector3.zero;
            topRigidbody.angularVelocity = Vector3.zero; if (topHadInitialRigidbody)
            {
    ...
    public void CutPart()
    {
        ResolveReferences(); if (broken || isRebuilding) { return; }
        if (treeparts.Count == 0)
        {
            SpawnDestructionVfx(); if (topofthetree != null)
            {
            ...
            broken = true; isRebuilding = true;
            StartCoroutine(RebuildTreeAfterSeconds(destroyDelaySeconds + rebuildDelaySeconds));
        }
        else
        {
            GameObject treepart = treeparts[treeparts.Count - 1]; if (treepart == null)
            {
                treeparts.RemoveAt(treeparts.Count - 1); return;
            }
