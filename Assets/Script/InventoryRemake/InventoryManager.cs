...
public class InventoryManager : MonoBehaviour
{
    ...
    public Image backgroundslider; public Image fill; public TMP_Text text1;
    public TMP_Text text2; public Image backgroundofwholeinventory;
    public List<SlotInsideUI> slotlist = new List<SlotInsideUI>();
    public bool UIShown = false; public KeyCode key;
    ...
    {
        EnsureSlotList(); if (item == null || count <= 0) { return false; }
        ...
        int number = 30; SlotInsideUI finalslot = null; foreach (SlotInsideUI slot in slotlist)
        {
            if (slot.id < number && !slot.occupied) { number = slot.id; finalslot = slot; }
        }
        if (finalslot == null) { return false; }
        finalslot.occupied = true;
        if (finalslot.image != null) { finalslot.image.sprite = item.inventorysprite; }
        finalslot.nameofslot = !string.IsNullOrWhiteSpace(item.nameofitem) ? item.nameofitem
        : item.name; finalslot.count = count; finalslot.Item = item;
        if (finalslot.text != null) { finalslot.text.text = count.ToString(); }
        return true;
    }
    public void EnableInventory(bool status)
    {
        UIShown = status; IsInventoryOpen = status;
        ApplyCursorState(); EnsureSlotList(); backgroundslider.enabled = status;
        fill.enabled = status; text1.enabled = status; text2.enabled = status;
    ...
    {
        if (slotlist == null) { slotlist = new List<SlotInsideUI>(); }
    ...
    private static bool IsSameItem(SlotInsideUI slot, InventoryItem item)
    {
        ...
        string slotName = !string.IsNullOrWhiteSpace(slot.nameofslot) ? slot.nameofslot
