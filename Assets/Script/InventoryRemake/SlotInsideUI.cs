...
public class SlotInsideUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IDropHandler, IPointerClickHandler
{
    ...
    public InventoryManager inventoryManager; public int count; public string nameofslot;
    public Image image; public Image background; public TMP_Text text; public int id;
    public bool occupied; public InventoryItem Item; private Canvas rootCanvas;
    private GameObject dragGhost; private RectTransform dragGhostRect;
    private Image dragGhostImage; void Awake()
    {
    ...
    {
        if (!InventoryManager.IsInventoryOpen || !HasItem()) { return; }
        draggedSlot = this; CreateDragGhost(); UpdateDragGhostPosition(eventData);
    }
    public void OnDrag(PointerEventData eventData)
    {
        if (draggedSlot != this || dragGhostRect == null) { return; }
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        if (!HasItem())
        {
            MoveFrom(source); return;
        }
        if (IsSameItem(source))
        {
            count += source.count;
            RefreshView(); source.ClearSlot(); return;
        }
        SwapWith(source);
    }
    ...
    {
        Item = source.Item;
        nameofslot = source.nameofslot; count = source.count; occupied = source.occupied;
        RefreshView(); source.ClearSlot();
    }
    private void SwapWith(SlotInsideUI source)
    {
        InventoryItem oldItem = Item; string oldName = nameofslot; int oldCount = count;
        bool oldOccupied = occupied; Item = source.Item; nameofslot = source.nameofslot;
        count = source.count; occupied = source.occupied; RefreshView(); source.Item = oldItem;
        source.nameofslot = oldName; source.count = oldCount; source.occupied = oldOccupied;
        source.RefreshView();
    }
    private void ClearSlot()
    {
        Item = null; nameofslot = string.Empty;
        count = 0; occupied = false; RefreshView();
    }
    ...
    {
        if (!InventoryManager.IsInventoryOpen || !HasItem()) { return; }
        Item.ResolveReferences(); if (Item.itemType != InventoryItemType.Building) { return; }
    ...
    {
        if (rootCanvas == null || Item == null || Item.inventorysprite == null) { return; }
        ...
        CanvasGroup cg = dragGhost.GetComponent<CanvasGroup>(); cg.blocksRaycasts = false;
        cg.alpha = 0.9f; dragGhostImage = dragGhost.GetComponent<Image>();
        dragGhostImage.sprite = Item.inventorysprite; dragGhostImage.raycastTarget = false;
    ...
    private void DestroyDragGhost()
    {
        ...
        dragGhost = null; dragGhostRect = null; dragGhostImage = null;
    }
}
