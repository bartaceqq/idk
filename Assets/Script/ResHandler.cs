...
public class ResHandler : MonoBehaviour
{
    ...
    private const float BaseLightShadowDistance = 8f; [Header("Resolution")]
    ...
    [SerializeField, Min(256)] private int initialCullBatchSize = 400; [Header("Safety Caps")]
    ...
    [Header("Global Quality")][SerializeField] private bool applyGlobalQualityClamps = true;
    ...
    [Header("Adaptive Runtime")][SerializeField] private bool adaptiveDistanceByFps = true;
    ...
    private float _nextCullingUpdateTime; private float _nextLightsUpdateTime;
    private float _nextAdaptiveCheckTime; private int _objectRoundRobinIndex;
    private int _rendererRoundRobinIndex; private int _fpsFrameCount;
    private float _fpsAccumTime; private bool settingsViewDistanceOverrideActive;
    private struct ManagedObject
    {
        public GameObject gameObject; public bool initialActive;
        public Vector3 cachedPosition;
    }
    private struct ManagedRenderer
    {
        public Renderer renderer; public bool initialEnabled;
        public ShadowCastingMode originalShadows;
    }
    private struct ManagedLight
    {
        public Light light; public bool initialEnabled; public LightShadows originalShadows;
        public LightType type;
    }
    private void OnValidate() { ClampRuntimeTuning(); }
    private void Awake()
    {
        ClampRuntimeTuning(); if (forceResolutionOnStart)
        {
        ...
        ResolveCamera(); ApplyViewDistanceMultiplier(GameSettings.ViewDistance);
        ...
        CollectManagedData(); CollectLights();
    }
    private void Start()
    {
    ...
    private void Update()
    {
        if (targetCamera == null) { ResolveCamera(); if (targetCamera == null) { return; } }
        _fpsFrameCount++; _fpsAccumTime += Mathf.Max(0.0001f, Time.unscaledDeltaTime);
    ...
    public void ApplyViewDistanceMultiplier(float multiplier)
    {
        ...
        float scaledRoot = Mathf.Sqrt(viewDistance); settingsViewDistanceOverrideActive = true;
        ...
        ClampRuntimeTuning(); QualitySettings.lodBias = viewDistance;
        ApplyTerrainDistanceSettings(); ResolveCamera();
        ...
        _nextCullingUpdateTime = 0f; _nextLightsUpdateTime = 0f;
    }
    public void RegisterManagedRoot(Transform root)
    {
        ...
        expandedRoots[managedRoots.Length] = root; managedRoots = expandedRoots;
    }
    private void ClampRuntimeTuning()
    {
        if (!enforceDistanceCaps) { return; }
    ...
    {
        List<Transform> found = new List<Transform>(6); if (managedRoots != null)
        {
    ...
    private void CollectManagedData()
    {
        ...
        _managedRenderers.Clear(); _uniqueManagedObjectIds.Clear();
        _uniqueManagedRendererIds.Clear(); if (managedRoots != null)
        {
        ...
        _objectRoundRobinIndex = 0; _rendererRoundRobinIndex = 0;
    }
    private static Transform GetTopObjectUnderRoot(Transform candidate, Transform root)
    {
        if (candidate == null || root == null || candidate == root) { return null; }
    ...
    private void AddManagedRenderer(Renderer renderer)
    {
        ...
        int id = renderer.GetInstanceID(); if (_uniqueManagedRendererIds.Contains(id)) { return; }
        _uniqueManagedRendererIds.Add(id); _managedRenderers.Add(new ManagedRenderer
        {
            renderer = renderer,
    ...
    {
        if (top == null) { return; }
        int id = top.GetInstanceID(); if (_uniqueManagedObjectIds.Contains(id)) { return; }
        _uniqueManagedObjectIds.Add(id); _managedObjects.Add(new ManagedObject
        {
    ...
    !(renderer is ParticleSystemRenderer) && !(renderer is LineRenderer) &&
    ...
    private void RestoreManagedState()
    {
        ...
        {
            ManagedRenderer item = _managedRenderers[i]; if (item.renderer == null) { continue; }
    ...
    {
        if (_managedObjects.Count == 0 || targetCamera == null) { yield break; }
    ...
    {
        if (_managedRenderers.Count == 0 || targetCamera == null) { return; }
        Vector3 camPos = targetCamera.transform.position;
        float renderDistSqr = treeRenderDistance * treeRenderDistance;
        ...
        for (int i = 0; i < _managedRenderers.Count; i++)
        {
            ...
            bool shouldRender = distSqr <= renderDistSqr; item.renderer.enabled = shouldRender;
            if (shouldRender)
            {
                item.renderer.shadowCastingMode = distSqr <= shadowDistSqr
            ? item.originalShadows : ShadowCastingMode.Off;
            }
        }
    }
    private void ProcessObjectCullingBatch()
    {
        if (_managedObjects.Count == 0 || targetCamera == null) { return; }
        Vector3 camPos = targetCamera.transform.position;
        float renderDistSqr = treeRenderDistance * treeRenderDistance;
        int budget = Mathf.Max(1, maxObjectsProcessedPerTick);
        for (int i = 0; i < budget; i++)
        {
            if (_objectRoundRobinIndex >= _managedObjects.Count) { _objectRoundRobinIndex = 0; }
            ManagedObject item = _managedObjects[_objectRoundRobinIndex]; _objectRoundRobinIndex++;
            if (item.gameObject == null || !item.initialActive) { continue; }
            bool shouldBeActive = (item.cachedPosition - camPos).sqrMagnitude <= renderDistSqr;
            if (item.gameObject.activeSelf != shouldBeActive) { item.gameObject.SetActive(shouldBeActive); }
        }
    ...
    private void ProcessRendererCullingBatch()
    {
        ...
        for (int i = 0; i < budget; i++)
        {
            if (_rendererRoundRobinIndex >= _managedRenderers.Count) { _rendererRoundRobinIndex = 0; }
    ...
    private void ProcessLights()
    {
        ...
        {
            ManagedLight item = _managedLights[i];
            Light light = item.light; if (light == null) { continue; }
    ...
    private void AdaptDistancesFromFps()
    {
        ...
        {
            _fpsFrameCount = 0;
            _fpsAccumTime = 0f; return;
        }
        float fps = _fpsFrameCount / _fpsAccumTime;
        _fpsFrameCount = 0; _fpsAccumTime = 0f; float renderBefore = treeRenderDistance;
        float shadowBefore = treeShadowDistance; if (fps < lowFpsThreshold)
        {
            treeRenderDistance -= adaptiveStep; treeShadowDistance -= adaptiveStep * 0.5f;
            terrainTreeDistance -= adaptiveStep; terrainDetailDensityScale -= 0.04f;
        ...
        ClampRuntimeTuning(); ApplyQualityClamps();
    ...
    private void ApplyQualityClamps()
    {
        ...
        QualitySettings.realtimeReflectionProbes = false; QualitySettings.softParticles = false;
        ApplyTerrainDistanceSettings(); Terrain[] terrains = Terrain.activeTerrains;
