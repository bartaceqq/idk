using System.Collections; using TMPro; using UnityEngine; using UnityEngine.InputSystem;
public class RayScript : MonoBehaviour {
    ...
    [Header("Timing")] public float cutDelaySeconds = 0.65f;
    ...
    private void UpdateSwordBlockState(bool swordEquipped, bool rightMouseHeld) { if (actionScript == null) { return; }
    ...
    private float HandleCurrentItemAction(bool leftClick, bool rightClick, int swordSpecialIndex) {
        if (IsAxeEquipped()) { if (!leftClick) { return 0f; }

            actionScript?.ResetUnarmedPunchCombo();
            return HandleAxeAction(); }

        if (IsPickaxeEquipped()) { if (!leftClick) { return 0f; }
    ...
    private float HandleAxeAction() {
        ...
        if (TryGetClosestTreeHandlerTarget(out TreeHandler treeHandlerTarget)) {
            if (!useDelayedAxeHit || axeHitDelaySeconds <= 0f) {
                treeHandlerTarget.Chop(interactionOrigin); } else {
                StartCoroutine(TriggerAfterDelayTreeHandler(treeHandlerTarget, interactionOrigin, axeHitDelaySeconds)); } } else if (TryGetClosestTreeTarget(out ColliderScript treeTarget)) {
    ...
    private bool TryGetClosestTreeHandlerTarget(out TreeHandler closestTreeHandler) { if (TryGetClosestTreeHandlerTargetInternal(triggerInteraction, out closestTreeHandler)) { return true; }
    ...
    private float HandlePickaxeAction() {
        ...
        if (actionScript != null) { if (!actionScript.TryMine()) { return actionScript.GetRemainingUpperBodyActionLockSeconds(); }
        ...
        if (TryGetClosestStoneTarget(out MineStone stoneTarget)) {
            ...
            if (!useDelayedPickaxeHit || mineDelay <= 0f) {
                ...
                stoneTarget.Mine(); } else { StartCoroutine(TriggerAfterDelayPickaxe(stoneTarget, interactionOrigin, mineDelay)); } }
    ...
    private float HandleUnarmedAction(bool leftClick, bool rightClick) { if (!leftClick) { return 0f; }
    ...
    private float ResolveToolSwingCooldown(float dedicatedCooldown) { if (dedicatedCooldown > 0f) { return dedicatedCooldown; }
    ...
    private static int GetSwordSpecialHotkeyIndex() { if (GameSettings.GetKeyDown(GameSettings.Key.SwordSpecial1, KeyCode.Alpha3)) { return 0; }
    ...
    private bool IsEquippedWeaponType(string expectedWeaponName, int legacyItemId) { if (itemSwitchScript == null) { return false; }
    ...
    private string ResolveEquippedItemName() { if (itemSwitchScript == null) { return string.Empty; }
        ...
        return string.Empty; }
    private bool HasEquippedItem() { if (itemSwitchScript == null) { return false; }
    ...
    private void OnPickableInRange(GameObject objectik) { if (objectik == null) { return; }
    ...
    private void TryPickupInventoryItem(GameObject pickableObject, int amount) { if (pickableObject == null || amount <= 0) { return; }
    ...
    private void ShowPickupInfo(InventoryItem inventoryItem, int amount) { if (inventoryItem == null) { return; }

        ResolveInfoHandler();
        if (infoHandler == null) { return; }
    ...
    private void SetPickupTextVisible(bool visible, GameObject pickableObject) { if (pickuptext == null) { return; }
    ...
    private static GameObject ResolvePickupDestroyTarget(GameObject pickableObject, InventoryItem inventoryItem) { if (inventoryItem == null) { return pickableObject; }
    ...
    private static GameObject ResolvePickableRoot(GameObject candidate) { if (candidate == null) { return null; }
    ...
    private void ResolveInventoryAddHandler() {
        if (inventoryAddHandler == null) { if (cachedInventoryAddHandler == null) { cachedInventoryAddHandler = FindInventoryAddHandlerInScene(); }

            inventoryAddHandler = cachedInventoryAddHandler; } else { cachedInventoryAddHandler = inventoryAddHandler; } }
    ...
    private bool TryGetClosestTreeTarget(out ColliderScript closestTree) { if (TryGetClosestTreeTargetInternal(triggerInteraction, out closestTree)) { return true; }
    ...
    private bool TryGetClosestStoneTarget(out MineStone closestStone) { if (TryGetClosestStoneTargetInternal(triggerInteraction, out closestStone)) { return true; }
    ...
    private bool TryGetClosestTargetFromColliders<T>(
        float searchRadius,
        QueryTriggerInteraction queryMode,
        TargetFromCollider<T> resolveTarget,
    ...
    private bool TryGetClosestSceneTarget<T>(float searchRadius, out T closestTarget) where T : Component {
        ...
        float radiusSqr = Mathf.Max(0.01f, searchRadius);
        radiusSqr *= radiusSqr;
        Vector3 origin = interactionOrigin.position;
        Transform playerRoot = interactionOrigin.root;
        float bestDistanceSqr = float.MaxValue;
    ...
    private static MineStone ResolveMineStoneFromCollider(Collider hit) { if (hit == null) { return null; }
    ...
    private IEnumerator TriggerAfterDelayPickaxe(MineStone mineStone, Transform attacker, float delaySeconds) {
        ...
        if (mineStone != null) {
            ...
            mineStone.Mine(); } }
    private void SpawnStoneImpact(MineStone mineStone, Transform attacker) { if (stoneparticle == null || mineStone == null) { return; }
    ...
    private Vector3 ResolveStoneImpactPoint(Transform stoneTransform, Vector3 attackerPosition) { if (TryResolveClosestImpactPoint(stoneTransform, attackerPosition, out Vector3 closestImpactPoint)) { return closestImpactPoint; }
    ...
    private static Vector3 ResolveStoneFallbackPoint(Transform stoneTransform) { if (stoneTransform != null) { return stoneTransform.position + Vector3.up * 0.35f; }

        return Vector3.zero; }
    private static Vector3 ResolveAttackerPosition(Transform attacker, Vector3 fallbackTargetPoint) { if (attacker != null) { return attacker.position; }
    ...
    private void TryPlayWeaponSound(AudioSource source, float delaySeconds, ref float nextAllowedTime, float minBlockSeconds) { if (source == null) { return; }
    ...
    private static string ToDisplayName(string rawName) { if (string.IsNullOrWhiteSpace(rawName)) { return "Item"; }

        string normalized = rawName.Trim().Replace('_', ' ');
        string[] parts = normalized.Split(' ');
        for (int i = 0; i < parts.Length; i++) { if (string.IsNullOrWhiteSpace(parts[i])) { continue; }
