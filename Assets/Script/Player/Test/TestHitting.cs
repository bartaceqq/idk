...
{
    [Header("Time Tracking")] public float currentInsideSeconds;
    ...
    [Header("Visuals")] public Renderer meshRenderer; public Material hitmat;
    [Header("Detection")][SerializeField] private bool usePollingFallback = true;
    ...
    [SerializeField] private bool useNameFallback = true; [Header("Attack Animation Gate")]
    ...
    [SerializeField] private float actionScriptScanInterval = 0.5f; [Header("Slow Motion")]
    ...
    [Header("Camera Focus")][SerializeField] private bool focusCameraOnHit = false;
    ...
    [Header("Hit Trail Tint")][SerializeField] private bool tintHitTrails = true;
    [SerializeField] private Color hitTrailColor = new Color(0.35f, 0f, 0f, 0.9f);
    [Header("Hit Stroke")][SerializeField] private bool drawHitStroke = true;
    ...
    private Material _originalMaterial; private Collider _selfCollider;
    ...
    private float _nextSwordScanTime; private float _nextActionScriptScanTime;
    private float _nextOverlapPollTime; private bool _isUsingHitMaterial;
    private bool _wasSwordInsideLastFrame;
    private readonly List<ActionScript> _knownActionScripts = new List<ActionScript>();
    private int _anyAttackStateFrame = -1; private bool _anySwordAttackActiveThisFrame;
    private float _defaultFixedDeltaTime; private float _timeScaleBeforeSlow = 1f;
    private bool _timeSlowActive; private bool _swordInsideThisFrame;
    private bool _hasFocusPointThisFrame; private Vector3 _focusPointThisFrame;
    private bool _cameraWasFocusing; private float _cameraBaseFov;
    ...
    private GameObject _hitStrokeObject; private LineRenderer _hitStrokeRenderer;
    private Material _hitStrokeMaterial; private bool _hitStrokeActive;
    ...
    private bool _hasSmoothedFocusPoint; private Vector3 _smoothedFocusPoint;
    ...
    private static bool _sharedAnySwordAttackActiveThisFrame; private void Awake()
    {
        _selfCollider = GetComponent<Collider>(); _defaultFixedDeltaTime = Time.fixedDeltaTime;
        ResolveMeshRendererIfNeeded(); CaptureOriginalMaterial();
    }
    ...
    public void ApplySettingsFrom(TestHitting template)
    {
        ...
        focusCameraOnHit = template.focusCameraOnHit; focusCamera = template.focusCamera;
        ...
        focusPointOffset = template.focusPointOffset; tintHitTrails = template.tintHitTrails;
        hitTrailColor = template.hitTrailColor; drawHitStroke = template.drawHitStroke;
        hitStrokeColor = template.hitStrokeColor; hitStrokeWidth = template.hitStrokeWidth;
    ...
    {
        TestHitting[] hitTargets = UnitySceneSearch.FindAll<TestHitting>();
        for (int i = 0; i < hitTargets.Length; i++)
        {
            TestHitting candidate = hitTargets[i];
            if (candidate == null) { continue; }
            if (candidate.GetComponentInParent<NPCDemageScript>() != null) { continue; }
            return candidate;
        }
        return null;
    }
    public static TestHitting EnsureEnemyHitFeedback(GameObject enemy, TestHitting template, Material fallbackHitMaterial)
    {
        ...
        if (targetCollider == null) { targetCollider = enemy.GetComponentInChildren<Collider>(true); }
        if (targetCollider == null) { return null; }
        TestHitting hitFeedback = targetCollider.GetComponent<TestHitting>();
        if (hitFeedback == null) { hitFeedback = enemy.GetComponentInChildren<TestHitting>(true); }
        if (hitFeedback == null) { hitFeedback = targetCollider.gameObject.AddComponent<TestHitting>(); }
        if (template != null) { hitFeedback.ApplySettingsFrom(template); }
    ...
    private static Material ResolveEnemyHitMaterial(GameObject enemy, TestHitting template, Material fallbackHitMaterial)
    {
        ...
        if (fallbackHitMaterial != null) { return fallbackHitMaterial; }
        NPCDemageScript damageScript = enemy.GetComponent<NPCDemageScript>();
        if (damageScript == null) { damageScript = enemy.GetComponentInChildren<NPCDemageScript>(true); }
    ...
    private static Renderer ResolveEnemyRenderer(GameObject enemy)
    {
        ...
        NPCDemageScript damageScript = enemy.GetComponent<NPCDemageScript>();
        if (damageScript == null) { damageScript = enemy.GetComponentInChildren<NPCDemageScript>(true); }
        if (damageScript != null && damageScript.meshRenderer != null) { return damageScript.meshRenderer; }
        return enemy.GetComponentInChildren<Renderer>(true);
    }
    private void ResolveMeshRendererIfNeeded()
    {
        if (meshRenderer == null) { meshRenderer = GetComponent<Renderer>(); }
        if (meshRenderer == null) { meshRenderer = GetComponentInChildren<Renderer>(true); }
    }
    private void CaptureOriginalMaterial() { if (meshRenderer != null) { _originalMaterial = meshRenderer.material; } }
    private void Start()
    {
        ...
        RefreshSwordColliders(); RefreshActionScripts();
    ...
    private void Update()
    {
        ...
        UpdateSmoothedFocusPointState(); ApplyTimeScaleState(swordInside);
        float timeDeltaForCounters = Time.unscaledDeltaTime; if (swordInside)
        {
        ...
        {
            CommitCurrentSessionToHistoryIfNeeded(); currentInsideSeconds = 0f;
        }
        _wasSwordInsideLastFrame = swordInside; ApplyMaterialState(swordInside);
        UpdateHitTrailColors();
        if (swordInside && _hasFocusPointThisFrame) { AppendHitStrokePoint(_focusPointThisFrame); }
        UpdateHitStrokeVisual();
    }
    private void LateUpdate() { ApplyCameraFocusState(); }
    private void OnDisable()
    {
        CommitCurrentSessionToHistoryIfNeeded();
        currentInsideSeconds = 0f; _wasSwordInsideLastFrame = false;
        RestoreTimeScaleIfNeeded(true); RestoreCameraStateImmediate(); RestoreAllTrailColors();
        ResetHitStroke(clearPoints: true); _hasSmoothedFocusPoint = false;
        _touchingSwordColliders.Clear(); _overlappingSwordRenderers.Clear();
    ...
    public void ClearInsideHistory() { insideHistorySeconds.Clear(); }
    private bool IsSwordInsideAndAllowedThisFrame()
    {
        ...
        foreach (Collider collider in _touchingSwordColliders) { if (IsSwordAttackAnimationActive(collider)) { return true; } }
        foreach (Renderer renderer in _overlappingSwordRenderers) { if (IsSwordAttackAnimationActive(renderer)) { return true; } }
        return false;
    }
    private void OnTriggerEnter(Collider other) { if (IsSwordCollider(other)) { _touchingSwordColliders.Add(other); } }
    private void OnTriggerExit(Collider other) { if (other != null) { _touchingSwordColliders.Remove(other); } }
    private void OnCollisionEnter(Collision collision)
    {
        Collider other = GetOtherCollider(collision);
        if (IsSwordCollider(other)) { _touchingSwordColliders.Add(other); }
    }
    private void OnCollisionExit(Collision collision)
    {
        Collider other = GetOtherCollider(collision);
        if (other != null) { _touchingSwordColliders.Remove(other); }
    }
    private void UpdateSwordOverlapStateFromPolling()
    {
        for (int i = _swordColliders.Count - 1; i >= 0; i--)
        {
            Collider swordCollider = _swordColliders[i]; if (swordCollider == null)
            {
                _swordColliders.RemoveAt(i); continue;
            }
            if (IsTouchingSwordCollider(swordCollider)) { _touchingSwordColliders.Add(swordCollider); } else { _touchingSwordColliders.Remove(swordCollider); }
        }
        for (int i = _swordRenderers.Count - 1; i >= 0; i--)
        {
            Renderer swordRenderer = _swordRenderers[i]; if (swordRenderer == null)
            {
                _swordRenderers.RemoveAt(i); continue;
            }
            if (IsSwordRendererOverlapping(swordRenderer)) { _overlappingSwordRenderers.Add(swordRenderer); } else { _overlappingSwordRenderers.Remove(swordRenderer); }
        }
    }
    private void ApplyMaterialState(bool swordInside)
    {
        if (meshRenderer == null || hitmat == null) { return; }
        if (swordInside)
        {
        ...
        {
            meshRenderer.material = _originalMaterial; _isUsingHitMaterial = false;
        }
    }
    private void UpdateHitTrailColors()
    {
        ...
        {
            if (swordCollider == null || !IsSwordAttackAnimationActive(swordCollider)) { continue; }
            TryAddDesiredTrail(swordCollider);
        }
        foreach (Renderer swordRenderer in _overlappingSwordRenderers)
        {
            if (swordRenderer == null || !IsSwordAttackAnimationActive(swordRenderer)) { continue; }
        ...
        foreach (KeyValuePair<SwordTrailEffect, Color> entry in _originalTrailColors)
        {
            ...
            if (trail == null || !_desiredTintTrails.Contains(trail)) { _trailRestoreBuffer.Add(trail); }
        }
        for (int i = 0; i < _trailRestoreBuffer.Count; i++)
        {
            SwordTrailEffect trail = _trailRestoreBuffer[i];
            if (trail != null && _originalTrailColors.TryGetValue(trail, out Color originalColor)) { trail.trailColor = originalColor; }
            _originalTrailColors.Remove(trail);
        }
        foreach (SwordTrailEffect trail in _desiredTintTrails)
        {
            if (trail == null) { continue; }
            if (!_originalTrailColors.ContainsKey(trail)) { _originalTrailColors.Add(trail, trail.trailColor); }
    ...
    private void TryAddDesiredTrail(Component source)
    {
        ...
        if (trail != null) { _desiredTintTrails.Add(trail); }
    }
    private void RestoreAllTrailColors()
    {
        foreach (KeyValuePair<SwordTrailEffect, Color> entry in _originalTrailColors) { if (entry.Key != null) { entry.Key.trailColor = entry.Value; } }
        _originalTrailColors.Clear(); _desiredTintTrails.Clear(); _trailRestoreBuffer.Clear();
    }
    private void CommitCurrentSessionToHistoryIfNeeded()
    {
        if (!_wasSwordInsideLastFrame || currentInsideSeconds <= 0f) { return; }
    ...
    private void ApplyTimeScaleState(bool swordInside)
    {
        ...
        {
            RestoreTimeScaleIfNeeded(); return;
        }
        float targetScale = Mathf.Clamp(insideTimeScale, 0.01f, 1f); if (swordInside)
        {
            if (!_timeSlowActive) { _timeScaleBeforeSlow = Time.timeScale; _timeSlowActive = true; }
    ...
    private void RestoreTimeScaleIfNeeded(bool immediate = false)
    {
        ...
        if (immediate) { SetGlobalTimeScale(restoreScale); _timeSlowActive = false; return; }
    ...
    private void ApplyTargetTimeScale(float targetScale)
    {
        ...
        if (!smoothTimeScaleTransitions) { SetGlobalTimeScale(clampedTarget); return; }
        ...
        if (Mathf.Abs(nextScale - clampedTarget) <= 0.001f) { nextScale = clampedTarget; }
        SetGlobalTimeScale(nextScale);
    }
    private void SetGlobalTimeScale(float scale)
    {
        float clampedScale = Mathf.Clamp(scale, 0.01f, 100f); Time.timeScale = clampedScale;
    ...
    private void ApplyCameraFocusState()
    {
        if (!focusCameraOnHit)
        {
            RestoreCameraStateIfNeeded(); return;
        }
        ...
        if (_swordInsideThisFrame && _hasFocusPointThisFrame)
        {
            ...
            if (!_cameraWasFocusing)
            {
                ...
                _cameraBaseRotation = cameraTransform.rotation; _cameraWasFocusing = true;
            }
        ...
        else { RestoreCameraStateIfNeeded(); }
    }
    private void RestoreCameraStateIfNeeded()
    {
        if (!_cameraWasFocusing) { return; }
        Camera activeCamera = ResolveFocusCamera(); if (activeCamera == null)
        {
            _cameraWasFocusing = false; return;
        }
        if (changeFovWhileFocusing)
        {
            ...
            if (Mathf.Abs(activeCamera.fieldOfView - _cameraBaseFov) <= 0.05f) { activeCamera.fieldOfView = _cameraBaseFov; }
        }
        Transform cameraTransform = activeCamera.transform;
        ...
        if (fovRestored && rotationRestored) { _cameraWasFocusing = false; }
    }
    private void RestoreCameraStateImmediate()
    {
        ...
        {
            if (changeFovWhileFocusing) { activeCamera.fieldOfView = _cameraBaseFov; }
    ...
    {
        if (focusCamera != null) { return focusCamera; }
        return Camera.main;
    }
    private void UpdateSmoothedFocusPointState()
    {
        ...
        if (!_hasSmoothedFocusPoint)
        {
            ...
            _hasSmoothedFocusPoint = true; return;
        }
    ...
    {
        if (!drawHitStroke) { return; }
        float now = Time.unscaledTime; float restartGap = Mathf.Max(0f, hitStrokeRestartGap);
        if (_hitStrokePoints.Count > 0 && now - _lastHitStrokeContactTime > restartGap) { ResetHitStroke(clearPoints: true); }
        _lastHitStrokeContactTime = now; _hitStrokeActive = true;
        Vector3 strokePoint = ComputeHitStrokePoint(worldHitPoint);
        float minSpacing = Mathf.Max(0.001f, hitStrokePointSpacing);
        float minSpacingSqr = minSpacing * minSpacing; if (_hitStrokePoints.Count > 0)
        {
            ...
            {
                _hitStrokePointTimes[lastIndex] = now; return;
            }
        }
        _hitStrokePoints.Add(strokePoint);
        _hitStrokePointTimes.Add(now); EnsureHitStrokeRenderer();
    }
    private Vector3 ComputeHitStrokePoint(Vector3 worldHitPoint)
    {
        if (_selfCollider == null) { return worldHitPoint; }
        Vector3 surfacePoint = _selfCollider.ClosestPoint(worldHitPoint);
        Vector3 normal = worldHitPoint - surfacePoint; if (normal.sqrMagnitude < 0.000001f)
        {
    ...
    private void UpdateHitStrokeVisual()
    {
        if (!drawHitStroke)
        {
            ResetHitStroke(clearPoints: true); return;
        }
        if (_hitStrokePoints.Count == 0)
        {
            ...
            _hitStrokeActive = false; return;
        }
        float life = Mathf.Max(0.05f, hitStrokeLifetime); float now = Time.unscaledTime;
        while (_hitStrokePointTimes.Count > 0 && now - _hitStrokePointTimes[0] > life)
        {
            _hitStrokePointTimes.RemoveAt(0); _hitStrokePoints.RemoveAt(0);
        }
        if (_hitStrokePoints.Count == 0) { ResetHitStroke(clearPoints: true); return; }
        EnsureHitStrokeRenderer(); _hitStrokeRenderer.enabled = true;
        _hitStrokeRenderer.positionCount = _hitStrokePoints.Count;
        for (int i = 0; i < _hitStrokePoints.Count; i++) { _hitStrokeRenderer.SetPosition(i, _hitStrokePoints[i]); }
    ...
    {
        if (_hitStrokeRenderer != null) { return; }
        ...
        _hitStrokeRenderer.numCornerVertices = 6; _hitStrokeRenderer.numCapVertices = 6;
        _hitStrokeRenderer.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
        _hitStrokeRenderer.receiveShadows = false; _hitStrokeRenderer.positionCount = 0;
    ...
    private void ApplyHitStrokeStyle(float intensity)
    {
        ...
        _hitStrokeRenderer.startWidth = width; _hitStrokeRenderer.endWidth = width * 0.82f;
        Color tint = hitStrokeColor; tint.a *= Mathf.Lerp(0.3f, 1f, clampedIntensity);
        _hitStrokeRenderer.startColor = tint; _hitStrokeRenderer.endColor = tint;
        if (_hitStrokeMaterial != null)
        {
            if (_hitStrokeMaterial.HasProperty("_BaseColor")) { _hitStrokeMaterial.SetColor("_BaseColor", tint); }
            if (_hitStrokeMaterial.HasProperty("_Color")) { _hitStrokeMaterial.SetColor("_Color", tint); }
        }
    }
    private void ResetHitStroke(bool clearPoints)
    {
        _hitStrokeActive = false;
        if (clearPoints) { _hitStrokePoints.Clear(); _hitStrokePointTimes.Clear(); }
        if (_hitStrokeRenderer != null)
        {
            _hitStrokeRenderer.positionCount = _hitStrokePoints.Count;
    ...
    private void DestroyHitStrokeResources()
    {
        ...
        {
            if (Application.isPlaying) { Destroy(_hitStrokeObject); } else { DestroyImmediate(_hitStrokeObject); }
            _hitStrokeObject = null; _hitStrokeRenderer = null;
        }
        if (_hitStrokeMaterial != null)
        {
            if (Application.isPlaying) { Destroy(_hitStrokeMaterial); } else { DestroyImmediate(_hitStrokeMaterial); }
    ...
    private bool IsSwordAttackAnimationActive(Component swordSource)
    {
        ...
        IsActionScriptInSwordAttack(actionScript)) { return true; }
    ...
    {
        actionScript = attackingActionScript; if (actionScript != null) { return true; }
        if (swordSource == null) { return false; }
        actionScript = swordSource.GetComponentInParent<ActionScript>();
        if (actionScript != null) { return true; }
        Sword sword = swordSource.GetComponentInParent<Sword>(); if (sword != null)
        {
    ...
    {
        return actionScript != null && actionScript.isActiveAndEnabled &&
        actionScript.TryGetActiveSwordAttackStateInfo(out _, out _);
    }
    private bool IsAnyKnownActionScriptInSwordAttack()
    {
        if (_anyAttackStateFrame == Time.frameCount) { return _anySwordAttackActiveThisFrame; }
        ...
        {
            ActionScript candidate = _knownActionScripts[i]; if (candidate == null)
            {
                _knownActionScripts.RemoveAt(i); continue;
            }
            if (IsActionScriptInSwordAttack(candidate))
            {
                _anySwordAttackActiveThisFrame = true; break;
            }
        }
    ...
    private bool TryGetFocusPoint(out Vector3 focusPoint)
    {
        ...
        foreach (Collider swordCollider in _touchingSwordColliders)
        {
            ...
            if (!found || distanceSqr < bestDistanceSqr)
            {
                ...
                bestDistanceSqr = distanceSqr; found = true;
            }
        }
        foreach (Renderer swordRenderer in _overlappingSwordRenderers)
        {
    ...
    {
        if (_selfCollider == null || swordCollider == null) { return selfCenter + focusPointOffset; }
    ...
    {
        if (_selfCollider == null || swordRenderer == null) { return selfCenter + focusPointOffset; }
    ...
    private Collider GetOtherCollider(Collision collision)
    {
        ...
        if (collision.collider != null && collision.collider.gameObject != gameObject) { return collision.collider; }
        return null;
    }
    private bool IsSwordCollider(Collider other)
    {
        if (other == null) { return false; }
        if (LooksLikeSwordPart(other.transform)) { return true; }
        Sword sword = other.GetComponent<Sword>() ?? other.GetComponentInParent<Sword>();
        if (sword != null && LooksLikeSwordPart(sword.transform)) { return true; }
        return false;
    }
    private bool IsTouchingSwordCollider(Collider swordCollider)
    {
        if (_selfCollider == null || swordCollider == null || !_selfCollider.enabled || !swordCollider.enabled) { return false; }
        if (!_selfCollider.gameObject.activeInHierarchy || !swordCollider.gameObject.activeInHierarchy) { return false; }
        if (!_selfCollider.bounds.Intersects(swordCollider.bounds)) { return false; }
        return Physics.ComputePenetration(_selfCollider, _selfCollider.transform.position,
        _selfCollider.transform.rotation, swordCollider, swordCollider.transform.position,
        swordCollider.transform.rotation, out _, out _);
    }
    private bool IsSwordRendererOverlapping(Renderer swordRenderer)
    {
        if (_selfCollider == null || swordRenderer == null || !swordRenderer.enabled) { return false; }
        if (!_selfCollider.gameObject.activeInHierarchy || !swordRenderer.gameObject.activeInHierarchy) { return false; }
    ...
    private void RefreshSwordColliders()
    {
        ...
        _swordColliders.Clear(); _swordRenderers.Clear();
        _touchingSwordColliders.RemoveWhere(c => c == null);
        _overlappingSwordRenderers.RemoveWhere(r => r == null);
        ...
        if (useNameFallback && _swordColliders.Count == 0 && _swordRenderers.Count == 0)
        {
            ...
            for (int i = 0; i < allColliders.Length; i++)
            {
                ...
                if (seenColliders.Add(candidate)) { _swordColliders.Add(candidate); }
            }
    ...
    _sharedUseNameFallback != useNameFallback) { return false; }
        CopySharedSwordCacheToInstance(); return true;
    }
    private void SaveSharedSwordCache()
    {
        SharedSwordColliders.Clear(); SharedSwordRenderers.Clear();
    ...
    private bool TryCopySharedActionCache()
    {
        ...
        CopySharedActionCacheToInstance(); return true;
    }
    ...
    {
        if (root == null || root == gameObject || root.transform.IsChildOf(transform)) { return; }
        Collider[] colliders = root.GetComponentsInChildren<Collider>(includeInactiveSwordObjects);
        for (int i = 0; i < colliders.Length; i++)
        {
            Collider candidate = colliders[i];
            if (candidate == null || candidate == _selfCollider || candidate.transform.IsChildOf(transform)) { continue; }
            if (!CandidateBelongsToSwordHierarchy(candidate.transform, root.transform)) { continue; }
            if (seen.Add(candidate)) { _swordColliders.Add(candidate); }
        }
    }
    ...
    private static bool HasSwordTagInHierarchy(Transform candidateTransform)
    {
        ...
        while (current != null)
        {
            if (HasSwordTag(current.gameObject)) { return true; }
    ...
    private static bool HasSwordTag(GameObject obj)
    {
        ...
        try { return obj.CompareTag("Sword"); } catch (UnityException) { return false; }
    }
    private static bool NameLooksLikeSword(Transform candidateTransform)
    {
        while (candidateTransform != null)
        {
            ...
            if (!string.IsNullOrEmpty(name))
            {
                ...
                name.IndexOf("katana", StringComparison.OrdinalIgnoreCase) >= 0) { return true; }
            }
