...
[RequireComponent(typeof(Collider))] public class TestHitting : MonoBehaviour {
    ...
    [Header("Visuals")] public Renderer meshRenderer;
    ...
    public void ConfigureHitTarget(Renderer targetRenderer, Material hitMaterial) { if (targetRenderer != null) { meshRenderer = targetRenderer; }
    ...
    public void ApplySettingsFrom(TestHitting template) { if (template == null || template == this) { return; }
    ...
    public static TestHitting EnsureEnemyHitFeedback(GameObject enemy, TestHitting template, Material fallbackHitMaterial) { if (enemy == null || !enemy.scene.IsValid()) { return null; }
    ...
    private static Material ResolveEnemyHitMaterial(GameObject enemy, TestHitting template, Material fallbackHitMaterial) { if (template != null && template.hitmat != null) { return template.hitmat; }
    ...
    private static Renderer ResolveEnemyRenderer(GameObject enemy) { if (enemy == null) { return null; }
    ...
    private void ResolveMeshRendererIfNeeded() { if (meshRenderer == null) { meshRenderer = GetComponent<Renderer>(); }
    ...
    private void Update() {
        ...
        if (swordInside) {
            ...
            totalInsideSeconds += timeDeltaForCounters; } else {
    ...
    private void ApplyMaterialState(bool swordInside) { if (meshRenderer == null || hitmat == null) { return; }

        if (swordInside) {
            if (!_isUsingHitMaterial) {
                meshRenderer.material = hitmat;
                _isUsingHitMaterial = true; } } else if (_isUsingHitMaterial) {
    ...
    private void UpdateHitTrailColors() {
        ...
        foreach (Collider swordCollider in _touchingSwordColliders) { if (swordCollider == null || !IsSwordAttackAnimationActive(swordCollider)) { continue; }

            TryAddDesiredTrail(swordCollider); }

        foreach (Renderer swordRenderer in _overlappingSwordRenderers) { if (swordRenderer == null || !IsSwordAttackAnimationActive(swordRenderer)) { continue; }
        ...
        foreach (SwordTrailEffect trail in _desiredTintTrails) { if (trail == null) { continue; }

            if (!_originalTrailColors.ContainsKey(trail)) { _originalTrailColors.Add(trail, trail.trailColor); }

            trail.trailColor = hitTrailColor; } }

    private void TryAddDesiredTrail(Component source) { if (source == null) { return; }
    ...
    private void CommitCurrentSessionToHistoryIfNeeded() { if (!_wasSwordInsideLastFrame || currentInsideSeconds <= 0f) { return; }
    ...
    private void ApplyTimeScaleState(bool swordInside) {
        ...
        if (swordInside) {
            ...
            ApplyTargetTimeScale(targetScale); } else { RestoreTimeScaleIfNeeded(); } }

    private void RestoreTimeScaleIfNeeded(bool immediate = false) { if (!_timeSlowActive) { return; }
    ...
    private void ApplyCameraFocusState() {
        ...
        if (_swordInsideThisFrame && _hasFocusPointThisFrame) {
            ...
            if (changeFovWhileFocusing) {
                ...
                activeCamera.fieldOfView = Mathf.Lerp(activeCamera.fieldOfView, targetFov, fovLerp); } } else { RestoreCameraStateIfNeeded(); } }

    private void RestoreCameraStateIfNeeded() { if (!_cameraWasFocusing) { return; }
    ...
    private void RestoreCameraStateImmediate() { if (!_cameraWasFocusing) { return; }

        Camera activeCamera = ResolveFocusCamera();
        if (activeCamera != null) { if (changeFovWhileFocusing) { activeCamera.fieldOfView = _cameraBaseFov; }
    ...
    private Camera ResolveFocusCamera() { if (focusCamera != null) { return focusCamera; }
    ...
    private void AppendHitStrokePoint(Vector3 worldHitPoint) { if (!drawHitStroke) { return; }
    ...
    private Vector3 ComputeHitStrokePoint(Vector3 worldHitPoint) { if (_selfCollider == null) { return worldHitPoint; }
    ...
    private void EnsureHitStrokeRenderer() { if (_hitStrokeRenderer != null) { return; }
    ...
    private void ApplyHitStrokeStyle(float intensity) { if (_hitStrokeRenderer == null) { return; }
        ...
        if (_hitStrokeMaterial != null) { if (_hitStrokeMaterial.HasProperty("_BaseColor")) { _hitStrokeMaterial.SetColor("_BaseColor", tint); }
    ...
    private bool IsSwordAttackAnimationActive(Component swordSource) { if (!requireActiveSwordAttackAnimation) { return true; }
    ...
    private Vector3 ComputeFocusPointFromCollider(Collider swordCollider, Vector3 selfCenter) { if (_selfCollider == null || swordCollider == null) { return selfCenter + focusPointOffset; }
    ...
    private Vector3 ComputeFocusPointFromRenderer(Renderer swordRenderer, Vector3 selfCenter) { if (_selfCollider == null || swordRenderer == null) { return selfCenter + focusPointOffset; }
    ...
    private Collider GetOtherCollider(Collision collision) { if (collision == null) { return null; }

        if (collision.collider != null && collision.collider.gameObject != gameObject) { return collision.collider; }

        return null; }

    private bool IsSwordCollider(Collider other) { if (other == null) { return false; }
    ...
    private bool IsTouchingSwordCollider(Collider swordCollider) { if (_selfCollider == null || swordCollider == null || !_selfCollider.enabled || !swordCollider.enabled) { return false; }
    ...
    private bool IsSwordRendererOverlapping(Renderer swordRenderer) { if (_selfCollider == null || swordRenderer == null || !swordRenderer.enabled) { return false; }
    ...
    private void RefreshSwordColliders() {
        ...
        try {
            ...
            for (int i = 0; i < taggedSwordObjects.Length; i++) {
                ...
                AddCandidateRenderers(taggedSwordObjects[i], seenRenderers); } } catch (UnityException) {
    ...
    private void AddCandidateColliders(GameObject root, HashSet<Collider> seen) { if (root == null || root == gameObject || root.transform.IsChildOf(transform)) { return; }
    ...
    private void AddCandidateRenderers(GameObject root, HashSet<Renderer> seen) { if (root == null || root == gameObject || root.transform.IsChildOf(transform)) { return; }
    ...
    private bool CandidateBelongsToSwordHierarchy(Transform candidate, Transform swordRoot) { if (candidate == null) { return false; }
    ...
    private bool LooksLikeSwordPart(Transform candidateTransform) { if (candidateTransform == null) { return false; }
    ...
    private static bool HasSwordTagInHierarchy(Transform candidateTransform) {
        ...
        while (current != null) { if (HasSwordTag(current.gameObject)) { return true; }

            current = current.parent; }

        return false; }

    private static bool HasSwordTag(GameObject obj) { if (obj == null) { return false; }
