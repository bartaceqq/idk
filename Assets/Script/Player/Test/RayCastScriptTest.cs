...
public class RayCastScriptTest : MonoBehaviour
{
    ...
    private const float TinyValue = 0.000001f; private const float MinSize = 0.01f;
    ...
    [Header("Placement")] public Camera camera; public LayerMask hitMask = ~0;
    public float range = 100f; public GameObject wall; public GameObject floor;
    public GameObject stair;
    [Header("Raycast Filtering")] public bool ignorePlayerColliderInBuildRaycast = true;
    public string playerTag = "Player"; public Transform playerRoot;
    ...
    [Header("Visuals")] public Material buildingMaterial; public Material doneBuildMaterial;
    ...
    public string tableTag = "Table"; public string tableNameContains = "table";
    ...
    public float snapEngageDistance = 0.8f; public float snapReleaseDistance = 1.2f;
    public float snapYOffset = 0f; public bool alignRotationFromPairs = true;
    public float pairLengthTolerance = 0.15f; public bool compareSnapPairLengthInXZ = true;
    public float pairLengthToleranceRatio = 0.08f;
    public bool constrainSnappedRotationToYaw = true;
    public float snapVerticalDistanceWeight = 0.35f;
    ...
    public Transform floorHeightReference; public float floorNearestPointRadius = 8f;
    public bool floorNearestPointUseXZ = true;
    [Header("Extrude")] public KeyCode extrudeKey = KeyCode.G;
    public bool extrudeModeStaysOn = true; public bool wallExtrudeUseFloorSpacing = true;
    public float wallExtrudeSpacingScale = 1f; public bool extrudePreventDuplicates = true;
    ...
    public bool logDetectionState = false; public float logDistanceDelta = 0.005f;
    public float logInterval = 0.15f; public Color debugLineColorA = Color.red;
    public Color debugLineColorB = Color.yellow; private GameObject _previewObject;
    private float _previewYRotation; private float _bottomOffset;
    private float _lastLoggedDistanceA = -1f; private float _lastLoggedDistanceB = -1f;
    private float _lastLogTime = -10f; private BuildType _buildType = BuildType.Wall;
    ...
    private float _nextLookingControllerResolveTime; private bool _isSnapLocked;
    private SnapPoint _lockedPreviewA; private SnapPoint _lockedPreviewB;
    private SnapPoint _lockedTargetA; private SnapPoint _lockedTargetB;
    private Vector3 _lockedPreviewOffsetA; private Vector3 _lockedPreviewOffsetB;
    private bool _isExtrudeMode; private bool _isExtruding;
    private BuildType _extrudeBuildType; private GameObject _extrudePrefab;
    private Vector3 _extrudeStartPosition; private Quaternion _extrudeStartRotation;
    private Vector3 _extrudeRight; private Vector3 _extrudeForward;
    private Vector2 _extrudeCellSize = Vector2.one; private Vector2Int _extrudeCellOffset;
    private readonly Dictionary<Vector2Int, GameObject> _extrudeGhostByCell = new Dictionary<Vector2Int, GameObject>(64);
    private readonly HashSet<Vector2Int> _neededExtrudeCells = new HashSet<Vector2Int>();
    private readonly List<Vector2Int> _extrudeCellsToRemove = new List<Vector2Int>(64);
    private Vector2Int _lastExtrudeGhostOffset; private bool _hasExtrudeGhostLayout;
    ...
    private GameObject _destroyTarget; private bool _isDestroyMode;
    ...
    private float _nextSnapTargetRefreshTime; private float _nextFloorHeightSnapRefreshTime;
    private bool _hasSnapTargetCache; private bool _hasFloorHeightSnapPointCache;
    private struct PairCandidate
    {
        public SnapPoint preview; public SnapPoint target;
        public Vector3 previewWorld; public Vector3 targetWorld; public float distance;
    }
    private void Start() { EnsureCamera(); InitializeBuildType(); CreatePreviewObject(); }
    private void Update()
    {
        if (GameplayUiState.IsGameplayInputBlocked)
        {
            HidePreviewAndCancelBuildInteraction(); return;
        }
        ...
        HandleDestroyModeInput(); if (_isDestroyMode) { HandleDestroyMode(); return; }
        if (!TryPrepareBuildPreview(out GameObject activePrefab)) { return; }
        HandleBuildTypeCycleInput(ref activePrefab); if (activePrefab == null) { return; }
        HandleExtrudeAndRotationInput(); if (_isExtruding) { HandleExtrudeDrag(); return; }
        MovePreviewObject(); if (_isExtrudeMode) { HandleExtrudeIdleInput(); return; }
    ...
    {
        if (_isDestroyMode) { _isDestroyMode = false; ClearDestroyTargetHighlight(); }
        _isExtrudeMode = false; if (_isExtruding)
        {
            CancelExtrudeState();
        }
        else { ClearExtrudeGhosts(); }
    ...
    {
        return TrySelectBuildPrefabDirect(prefab, sourceItemName, Vector3.zero, Vector3.one,
    ...
    public bool TrySelectInventoryBuildingItem(InventoryItem inventoryItem)
    {
        ...
        return TrySelectBuildPrefabDirect(inventoryItem.itemPrefab, inventoryItem.name,
        inventoryItem.buildRotationEuler, inventoryItem.buildScale,
        autoSwitchToBuildingCapsule);
    }
    private bool TrySelectBuildPrefabDirect(
        GameObject prefab, string sourceItemName, Vector3 buildRotationEuler, Vector3 buildScale,
        bool switchToBuildingCapsule)
    {
        ...
        _isExtrudeMode = false; CancelExtrudeState();
        CreatePreviewObject(); if (_previewObject == null) { return false; }
        _previewObject.SetActive(true);
        LogDetectionState("Build mode: " + _inventoryBuildItemName); return true;
    }
    private bool TryPrepareBuildPreview(out GameObject activePrefab)
    {
        activePrefab = GetActivePrefab(); if (activePrefab == null) { return false; }
        if (_previewObject != null)
        {
            if (!_previewObject.activeSelf) { _previewObject.SetActive(true); }
            return true;
        }
        CreatePreviewObject(); return _previewObject != null;
    }
    ...
    private void TryPlaceSingleObject(GameObject activePrefab)
    {
        ...
        ApplyPlacedObjectVisuals(created); InvalidateSnapTargetCache();
    }
    ...
    {
        RefreshAvailableBuildTypes(); if (_availableBuildTypes.Count < 2)
        {
        ...
        CreatePreviewObject(); LogDetectionState("Build mode: " + _buildType);
    }
    private void ClearInventoryBuildSelection()
    {
        _inventoryBuildPrefab = null;
        _inventoryBuildItemName = string.Empty; _inventoryBuildRotationEuler = Vector3.zero;
    ...
    private bool TryHandleTableRightClickShortcut(ref GameObject activePrefab)
    {
        ...
        bool changed = false;
        if (autoSwitchToBuildingCapsule) { EnsureBuildingCapsuleActive(); changed = true; }
        if (tableRightClickBuildPrefab != null)
        {
            changed |= TrySelectBuildPrefabDirect(
        tableRightClickBuildPrefab, tableRightClickBuildName, tableBuildRotationEuler,
        tableBuildScale, false); activePrefab = GetActivePrefab();
        }
        return changed;
    }
    private bool TryGetClosestTableHit(Ray ray, out RaycastHit closestTableHit)
    {
        ...
        bool found = false; for (int i = 0; i < hitCount; i++)
        {
            RaycastHit hit = _placementHits[i]; Collider collider = hit.collider;
            ...
            closestTableHit = hit; found = true;
        }
        return found;
    }
    private bool IsTableTransform(Transform startTransform)
    {
        ...
        bool hasNameFilter = !string.IsNullOrEmpty(contains); Transform current = startTransform;
        while (current != null)
        {
    ...
    {
        _availableBuildTypes.Clear(); if (wall != null) _availableBuildTypes.Add(BuildType.Wall);
    ...
    private void CreatePreviewObject()
    {
        ...
        if (activePrefab == null) return; if (_previewObject != null) Destroy(_previewObject);
        ...
        SetPreviewMode(_previewObject, true); _bottomOffset = GetBottomOffset(_previewObject);
        ClearSnapLock(); InvalidateSnapTargetCache(); CancelExtrudeState();
    }
    private void MovePreviewObject()
    {
        ...
        if (!enableTwoPointSnap) { if (_isSnapLocked) ClearSnapLock(); return; }
    ...
    private Vector3 GetRawPlacementPosition(RaycastHit hit)
    {
        ...
        rawPosition.y += _bottomOffset; return rawPosition;
    }
    ...
    private bool TryGetPlacementHit(Ray ray, out RaycastHit closestHit)
    {
        ...
        bool found = false; for (int i = 0; i < hitCount; i++)
        {
            RaycastHit candidate = _placementHits[i]; Collider collider = candidate.collider;
            if (ShouldSkipPlacementCollider(collider)) { continue; }
            if (candidate.distance >= closestDistance) { continue; }
            closestDistance = candidate.distance; closestHit = candidate; found = true;
        }
        return found;
    }
    ...
    private static Vector3 SanitizeScale(Vector3 scale)
    {
        ...
        scale.z = Mathf.Abs(scale.z) < TinyValue ? 1f : scale.z; return scale;
    }
    ...
    {
        ClearDestroyTargetHighlight(); ClearExtrudeGhosts();
    ...
    private void ToggleDestroyMode()
    {
        ...
        if (_isDestroyMode)
        {
            _isExtrudeMode = false; if (_isExtruding) CancelExtrudeState();
            if (_previewObject != null) _previewObject.SetActive(false); ClearSnapLock();
            ClearDestroyTargetHighlight(); LogDetectionState("Destroy mode: ON"); return;
        }
        ClearDestroyTargetHighlight(); if (_previewObject == null)
        {
            CreatePreviewObject();
        }
        else { _previewObject.SetActive(true); }
        LogDetectionState("Destroy mode: OFF");
    }
    private void HandleDestroyMode()
    {
        Ray ray = GetCenterRay(); if (TryGetLookedAtBuildTarget(ray, out GameObject target))
        {
            SetDestroyTargetHighlight(target);
        }
        else { ClearDestroyTargetHighlight(); }
        if (!Input.GetMouseButtonDown(0) || _destroyTarget == null) { return; }
        GameObject destroyNow = _destroyTarget; ClearDestroyTargetHighlight();
        Destroy(destroyNow); InvalidateSnapTargetCache();
    }
    private bool TryGetLookedAtBuildTarget(Ray ray, out GameObject target)
    {
        ...
        for (int i = 0; i < hitCount; i++)
        {
            ...
            bestDistance = _placementHits[i].distance; target = owner;
        }
        return target != null;
    }
    private void SetDestroyTargetHighlight(GameObject target)
    {
        ...
        for (int i = 0; i < renderers.Length; i++)
        {
            ...
            _destroyOriginalMaterials.Add((Material[])original.Clone()); if (original.Length == 0)
            {
                renderer.sharedMaterial = destroyBuildMaterial; continue;
            }
    ...
    private void ClearDestroyTargetHighlight()
    {
        ...
        _destroyHighlightedRenderers.Clear(); _destroyOriginalMaterials.Clear();
        _destroyTarget = null;
    }
    private void ToggleExtrudeMode()
    {
        ...
        _isExtrudeMode = !_isExtrudeMode; if (!_isExtrudeMode) CancelExtrudeState();
    ...
    private void HandleExtrudeIdleInput()
    {
        ...
        {
            _isExtrudeMode = false; CancelExtrudeState(); return;
        }
        if (Input.GetMouseButtonDown(0)) BeginExtrude();
    }
    private void HandleExtrudeDrag()
    {
        if (Input.GetKeyDown(KeyCode.Escape)) { CancelExtrudeState(); return; }
        UpdateExtrudePreview(); if (Input.GetMouseButtonUp(0))
        {
            CommitExtrude();
            _isExtruding = false; ClearExtrudeGhosts();
    ...
    private void BeginExtrude()
    {
        ...
        if (_previewObject == null || _extrudePrefab == null) return; _isExtruding = true;
        _extrudeBuildType = _buildType; _extrudeStartPosition = _previewObject.transform.position;
    ...
    private void UpdateExtrudePreview()
    {
        ...
        _extrudeCellOffset = new Vector2Int(xCells, zCells); Vector3 snappedPosition =
        _extrudeStartPosition + (_extrudeRight * (xCells * _extrudeCellSize.x))
    ...
    {
        point = Vector3.zero; if (camera == null) return false; Ray ray = GetCenterRay();
        if (TryGetPlacementHit(ray, out RaycastHit hit)) { point = hit.point; return true; }
        if (TryGetCursorPointOnHorizontalPlane(_extrudeStartPosition.y, out point)) { return true; }
        point = ray.GetPoint(Mathf.Max(1f, range * 0.5f)); return true;
    }
    private void CommitExtrude()
    {
        if (_extrudePrefab == null) return; ClearExtrudeGhosts();
        ...
        int placedCount = 0; int skippedAsDuplicateCount = 0; for (int x = minX; x <= maxX; x++)
        {
            for (int z = minZ; z <= maxZ; z++)
            {
                Vector3 placementPosition = _extrudeStartPosition
            + (_extrudeRight * (x * _extrudeCellSize.x))
            + (_extrudeForward * (z * _extrudeCellSize.y)); if (extrudePreventDuplicates &&
            HasObjectAtPosition(existing, placementPosition, _extrudeStartRotation, occupyToleranceXZ, occupyToleranceY))
                {
                    skippedAsDuplicateCount++; continue;
                }
                ...
                ApplyPlacedObjectVisuals(created); existing.Add(created.transform); placedCount++;
            }
        }
    ...
    {
        _isExtruding = false;
        _extrudePrefab = null; _extrudeCellOffset = Vector2Int.zero; ClearExtrudeGhosts();
    }
    private void UpdateExtrudeGhosts()
    {
        if (!_isExtruding || _extrudePrefab == null)
        {
            ClearExtrudeGhosts(); return;
        }
        if (_hasExtrudeGhostLayout && _extrudeCellOffset == _lastExtrudeGhostOffset) { return; }
        _hasExtrudeGhostLayout = true; _lastExtrudeGhostOffset = _extrudeCellOffset;
        int minX = Mathf.Min(0, _extrudeCellOffset.x);
        int maxX = Mathf.Max(0, _extrudeCellOffset.x);
        int minZ = Mathf.Min(0, _extrudeCellOffset.y);
        int maxZ = Mathf.Max(0, _extrudeCellOffset.y); _neededExtrudeCells.Clear();
        for (int x = minX; x <= maxX; x++)
        {
            for (int z = minZ; z <= maxZ; z++)
            {
                Vector2Int cell = new Vector2Int(x, z); if (cell == _extrudeCellOffset) { continue; }
                _neededExtrudeCells.Add(cell); Vector3 cellPosition = GetExtrudeCellWorldPosition(x, z);
                ...
                else { _extrudeGhostByCell[cell] = CreateExtrudeGhost(cellPosition); }
            }
        }
        ...
        foreach (KeyValuePair<Vector2Int, GameObject> pair in _extrudeGhostByCell)
        {
            if (!_neededExtrudeCells.Contains(pair.Key))
            {
                if (pair.Value != null)
                {
                    pair.Value.SetActive(false); Destroy(pair.Value);
                }
    ...
    private GameObject CreateExtrudeGhost(Vector3 position)
    {
        ...
        ApplyScaleMultiplier(ghost, GetActiveBuildScale()); SetPreviewMode(ghost, true);
        ApplyMaterialToMeshRenderers(ghost, buildingMaterial); return ghost;
    }
    private Vector3 GetExtrudeCellWorldPosition(int xCell, int zCell)
    {
        return _extrudeStartPosition + (_extrudeRight * (xCell * _extrudeCellSize.x))
        + (_extrudeForward * (zCell * _extrudeCellSize.y));
    }
    private void ClearExtrudeGhosts()
    {
        _neededExtrudeCells.Clear(); _extrudeCellsToRemove.Clear();
        _hasExtrudeGhostLayout = false; if (_extrudeGhostByCell.Count == 0) { return; }
        foreach (KeyValuePair<Vector2Int, GameObject> pair in _extrudeGhostByCell)
        {
    ...
    {
        List<Transform> transforms = new List<Transform>(128); switch (type)
        {
            case BuildType.Wall: AddActiveTransforms(WallSnapPoints.Instances, transforms); break;
            case BuildType.Floor: AddActiveTransforms(FloorScript.Instances, transforms); break;
            case BuildType.Stair: AddActiveTransforms(StairScript.Instances, transforms); break;
        }
        return transforms;
    }
    ...
    private static bool HasObjectAtPosition(
            List<Transform> existing, Vector3 position, Quaternion rotation, float toleranceXZ,
            float toleranceY)
    {
        ...
        for (int i = 0; i < existing.Count; i++)
        {
            ...
            Vector3 existingPosition = candidate.position; float dx = existingPosition.x - position.x;
    ...
    {
        point = Vector3.zero; if (camera == null) return false;
        Plane plane = new Plane(Vector3.up, new Vector3(0f, planeY, 0f));
        Ray ray = GetCenterRay(); if (!plane.Raycast(ray, out float enter)) return false;
        point = ray.GetPoint(enter); return true;
    }
    ...
    private Vector2 GetExtrudeCellSize(GameObject target, Vector3 axisRight, Vector3 axisForward)
    {
        ...
        cellSize.y = Mathf.Max(MinSize, cellSize.y * spacingScale); return cellSize;
    }
    private bool TryGetWallExtrudeReferenceFloor(out GameObject floorReference)
    {
        floorReference = null; IReadOnlyList<FloorScript> floors = FloorScript.Instances;
        if (floors.Count == 0) { floorReference = floor; return floorReference != null; }
        float bestDistanceSqr = float.MaxValue; Vector3 from = _extrudeStartPosition;
        for (int i = 0; i < floors.Count; i++)
        {
            ...
            Vector3 to = floorScript.transform.position; float dx = to.x - from.x;
            float dz = to.z - from.z; float distanceSqr = (dx * dx) + (dz * dz);
            if (distanceSqr >= bestDistanceSqr) { continue; }
            bestDistanceSqr = distanceSqr;
            floorReference = floorScript.gameObject;
        }
        if (floorReference != null) { return true; }
        floorReference = floor; return floorReference != null;
    }
    private float GetExtrudeOccupyToleranceXZ(BuildType type)
    {
        switch (type)
        {
            case BuildType.Wall: return Mathf.Max(MinTolerance, wallExtrudeOccupyToleranceXZ);
            case BuildType.Floor: return Mathf.Max(MinTolerance, floorExtrudeOccupyToleranceXZ);
            case BuildType.Stair: return Mathf.Max(MinTolerance, stairExtrudeOccupyToleranceXZ);
            default: return 0.02f;
        }
    }
    private static bool TryGetCellSizeFromSnapPoints(
            SnapPoint[] snapPoints, Vector3 axisRight, Vector3 axisForward, out Vector2 size)
    {
        size = Vector2.zero; if (snapPoints == null || snapPoints.Length == 0) return false;
        if (axisRight.sqrMagnitude < TinyValue || axisForward.sqrMagnitude < TinyValue) return false;
        axisRight.Normalize(); axisForward.Normalize(); bool hasPoint = false;
        float minRight = float.MaxValue; float maxRight = float.MinValue;
        float minForward = float.MaxValue; float maxForward = float.MinValue;
        for (int i = 0; i < snapPoints.Length; i++)
        {
            SnapPoint snapPoint = snapPoints[i];
            if (snapPoint == null) continue; Vector3 point = snapPoint.transform.position;
            float rightDot = Vector3.Dot(point, axisRight);
            float forwardDot = Vector3.Dot(point, axisForward);
            minRight = Mathf.Min(minRight, rightDot); maxRight = Mathf.Max(maxRight, rightDot);
            minForward = Mathf.Min(minForward, forwardDot);
            maxForward = Mathf.Max(maxForward, forwardDot); hasPoint = true;
        }
        if (!hasPoint) return false; float sizeRight = maxRight - minRight;
        float sizeForward = maxForward - minForward;
        if (sizeRight < MinSize || sizeForward < MinSize) return false;
        size = new Vector2(sizeRight, sizeForward); return true;
    }
    private static bool TryGetProjectedBoundsSize(GameObject target, Vector3 axisRight,
    Vector3 axisForward, out Vector2 size)
    {
        size = Vector2.zero;
        if (!TryGetRenderableBoundsWithoutSnapMarkers(target, out Bounds bounds)) return false;
        if (axisRight.sqrMagnitude < TinyValue || axisForward.sqrMagnitude < TinyValue) return false;
        axisRight.Normalize(); axisForward.Normalize(); Vector3 min = bounds.min;
        Vector3 max = bounds.max; Vector3[] corners = { new Vector3(min.x, min.y, min.z),
new Vector3(min.x, min.y, max.z), new Vector3(min.x, max.y, min.z),
new Vector3(min.x, max.y, max.z), new Vector3(max.x, min.y, min.z),
new Vector3(max.x, min.y, max.z), new Vector3(max.x, max.y, min.z),
new Vector3(max.x, max.y, max.z), }; float minRight = float.MaxValue;
        float maxRight = float.MinValue; float minForward = float.MaxValue;
        float maxForward = float.MinValue; for (int i = 0; i < corners.Length; i++)
        {
    ...
    private void HandleStickyTwoPointSnap(Vector3 rawPosition, Quaternion rawRotation)
    {
        if (_isSnapLocked)
        {
            if (TrySolveFromLockedPairs(rawPosition, rawRotation, out Vector3 lockedPosition, out Quaternion lockedRotation, out float lockedDistanceA, out float lockedDistanceB))
            {
                ...
                DrawAndLogDistances(_lockedPreviewA.transform.position,
                _lockedTargetA.transform.position, _lockedPreviewB.transform.position,
                _lockedTargetB.transform.position, lockedDistanceA, lockedDistanceB,
        ...
        if (!TryGetClosestTwoPairsOnSameObject(out PairCandidate pairA, out PairCandidate pairB,
        out PairCandidate fallbackPair, out bool foundFallbackPair, out int scannedTargets,
        out int validTargets, out int totalCandidates))
        {
            ...
            TryApplySinglePointSnapFallback(fallbackPair, foundFallbackPair, rawRotation); return;
        }
        ...
        _isSnapLocked = true; _lockedPreviewA = pairA.preview; _lockedPreviewB = pairB.preview;
        _lockedTargetA = pairA.target; _lockedTargetB = pairB.target;
        ...
        if (TrySolveFromLockedPairs(rawPosition, rawRotation, out Vector3 snappedPosition, out Quaternion snappedRotation, out float snappedDistanceA, out float snappedDistanceB))
        {
            ...
            DrawAndLogDistances(_lockedPreviewA.transform.position,
            _lockedTargetA.transform.position, _lockedPreviewB.transform.position,
            _lockedTargetB.transform.position, snappedDistanceA, snappedDistanceB,
    ...
    private bool TryApplySinglePointSnapFallback(PairCandidate pair, bool foundPair, Quaternion rawRotation)
    {
        ...
        ? stairSinglePointSnapEngageDistance : snapEngageDistance;
        ...
        snappedPosition.y += snapYOffset;
        _previewObject.transform.SetPositionAndRotation(snappedPosition, rawRotation);
        DrawAndLogDistances(pair.previewWorld, pair.targetWorld, pair.previewWorld,
        pair.targetWorld, pair.distance, pair.distance, "Closest pair(single-snap)");
        return true;
    }
    private void GetRawLockedPairDistances(Vector3 rawPosition,
        Quaternion rawRotation, out float rawDistanceA, out float rawDistanceB)
    {
        rawDistanceA = float.MaxValue; rawDistanceB = float.MaxValue;
    ...
    private bool TryGetClosestTwoPairsOnSameObject(out PairCandidate bestA,
    out PairCandidate bestB, out PairCandidate bestSingle, out bool foundSingle,
    out int scannedTargets, out int validTargets, out int totalCandidates)
    {
        bestA = default;
        bestB = default; bestSingle = default; foundSingle = false; scannedTargets = 0;
        validTargets = 0; totalCandidates = 0;
        if (!TryGetSnapPoints(_previewObject, out SnapPoint[] previewSnapPoints))
        {
            LogDetectionState("Preview object has no snap points configured."); return false;
        }
        bool found = false; float bestScore = float.MaxValue;
        ...
        {
            WallSnapPoints wallTarget = _cachedWallSnapTargets[i]; EvaluateTargetPairs(
            wallTarget != null ? wallTarget.gameObject : null,
            wallTarget != null ? wallTarget.snapPoints : null, previewSnapPoints, ref bestA,
            ref bestB, ref bestSingle, ref found, ref foundSingle, ref bestScore, ref bestSingleScore,
            ref scannedTargets, ref validTargets, ref totalCandidates);
        }
        for (int i = 0; i < _cachedFloorSnapTargets.Count; i++)
        {
            FloorScript floorTarget = _cachedFloorSnapTargets[i]; EvaluateTargetPairs(
            floorTarget != null ? floorTarget.gameObject : null,
            floorTarget != null ? floorTarget.snapPoints : null, previewSnapPoints, ref bestA,
            ref bestB, ref bestSingle, ref found, ref foundSingle, ref bestScore, ref bestSingleScore,
            ref scannedTargets, ref validTargets, ref totalCandidates);
        }
        for (int i = 0; i < _cachedStairSnapTargets.Count; i++)
        {
            StairScript stairTarget = _cachedStairSnapTargets[i]; EvaluateTargetPairs(
            stairTarget != null ? stairTarget.gameObject : null,
            stairTarget != null ? stairTarget.snapPoints : null, previewSnapPoints, ref bestA,
            ref bestB, ref bestSingle, ref found, ref foundSingle, ref bestScore, ref bestSingleScore,
            ref scannedTargets, ref validTargets, ref totalCandidates);
        }
        return found;
    ...
    private void RefreshSnapTargetCacheIfNeeded(Vector3 searchCenter)
    {
        ...
        _cachedWallSnapTargets.Clear(); _cachedFloorSnapTargets.Clear();
        _cachedStairSnapTargets.Clear();
        CollectSnapTargets(WallSnapPoints.Instances, _cachedWallSnapTargets, searchCenter, radius);
    ...
    private void RefreshFloorHeightSnapPointCacheIfNeeded(Vector3 searchCenter)
    {
        ...
        float radiusSqr = radius * radius; for (int i = 0; i < allSnapPoints.Count; i++)
        {
    ...
    {
        float radiusSqr = radius * radius; for (int i = 0; i < components.Count; i++)
        {
            T component = components[i];
            if (component == null || component.gameObject == _previewObject || !component.gameObject.activeInHierarchy) { continue; }
    ...
    private static bool IsTransformNearSearchCenter(Transform target, Vector3 searchCenter, float radiusSqr)
    {
        ...
        float dx = position.x - searchCenter.x; float dz = position.z - searchCenter.z;
        return (dx * dx) + (dz * dz) <= radiusSqr;
    }
    private void InvalidateSnapTargetCache()
    {
        _hasSnapTargetCache = false; _hasFloorHeightSnapPointCache = false;
        _nextSnapTargetRefreshTime = 0f; _nextFloorHeightSnapRefreshTime = 0f;
        _snapMarkerColliderCache.Clear(); _snapOwnerByCollider.Clear();
    }
    ...
    private static BuildPieceKind ToBuildPieceKind(BuildType type)
    {
        switch (type)
        {
            case BuildType.Floor: return BuildPieceKind.Floor;
            case BuildType.Stair:
                return BuildPieceKind.Stair;
            default: return BuildPieceKind.Wall;
        }
    }
    ...
    {
        float dx = b.x - a.x;
        float dz = b.z - a.z; return Mathf.Sqrt((dx * dx) + (dz * dz));
    }
    private void EvaluateTargetPairs(GameObject targetObject, SnapPoint[] targetSnapPoints,
    SnapPoint[] previewSnapPoints, ref PairCandidate bestA, ref PairCandidate bestB,
    ref PairCandidate bestSingle, ref bool found, ref bool foundSingle, ref float bestScore,
    ref float bestSingleScore, ref int scannedTargets, ref int validTargets,
    ref int totalCandidates)
    {
        ...
        validTargets++;
        _candidates.Clear(); for (int i = 0; i < previewSnapPoints.Length; i++)
        {
            SnapPoint previewPoint = previewSnapPoints[i]; if (previewPoint == null) { continue; }
            Vector3 previewWorld = previewPoint.transform.position;
            for (int j = 0; j < targetSnapPoints.Length; j++)
            {
                SnapPoint targetPoint = targetSnapPoints[j]; if (targetPoint == null) { continue; }
                ...
                if (candidate.distance < bestSingleScore)
                {
                    ...
                    bestSingle = candidate; foundSingle = true;
                }
                _candidates.Add(candidate);
            }
        }
        totalCandidates += _candidates.Count; if (_candidates.Count < 2) { return; }
        for (int i = 0; i < _candidates.Count - 1; i++)
        {
            ...
            for (int j = i + 1; j < _candidates.Count; j++)
            {
                ...
                bestScore = score; bestA = candidateA;
                bestB = candidateB; found = true;
            }
        }
    }
    private bool TrySolveFromLockedPairs(
                Vector3 rawPosition, Quaternion rawRotation, out Vector3 position,
                out Quaternion rotation, out float distanceA, out float distanceB)
    {
        position = Vector3.zero; rotation = rawRotation; distanceA = float.MaxValue;
        ...
        Vector3 targetWorldB = _lockedTargetB.transform.position; if (alignRotationFromPairs)
        {
            Vector3 previewVector = _lockedPreviewOffsetB - _lockedPreviewOffsetA;
            Vector3 previewVectorWorld = rawRotation * previewVector;
            Vector3 targetVector = targetWorldB - targetWorldA; if (constrainSnappedRotationToYaw)
            {
        ...
        position = targetWorldA - rotatedOffsetA; position.y += snapYOffset;
        Vector3 solvedA = position + rotatedOffsetA; Vector3 solvedB = position + rotatedOffsetB;
        distanceA = GetSnapPointDistance(solvedA, targetWorldA);
        distanceB = GetSnapPointDistance(solvedB, targetWorldB); return true;
    }
    private bool TryGetSnapPoints(GameObject target, out SnapPoint[] snapPoints)
    {
        snapPoints = null; if (target == null) { return false; }
        ...
        {
            snapPoints = wallSnapPoints.snapPoints; return true;
        }
        ...
        {
            snapPoints = floorSnapPoints.snapPoints; return true;
        }
        ...
        {
            snapPoints = stairSnapPoints.snapPoints; return true;
        }
        return false;
    }
    ...
    private void DrawAndLogDistances(Vector3 fromA,
            Vector3 toA, Vector3 fromB, Vector3 toB, float distanceA, float distanceB,
            string label = "Closest pairs(raw)")
    {
        Debug.DrawLine(fromA, toA, debugLineColorA);
        Debug.DrawLine(fromB, toB, debugLineColorB); if (!logClosestPair) { return; }
        ...
        _lastLoggedDistanceA = distanceA; _lastLoggedDistanceB = distanceB;
    ...
    {
        owner = null; if (hitCollider == null) { return false; }
        if (_snapOwnerByCollider.TryGetValue(hitCollider, out GameObject cachedOwner))
        {
            owner = cachedOwner; return owner != null;
        }
        bool found = TryGetSnapOwnerFromCollider(hitCollider, out owner);
        _snapOwnerByCollider[hitCollider] = found ? owner : null; return found;
    }
    private static bool TryGetSnapOwnerFromCollider(Collider hitCollider, out GameObject owner)
    {
        ...
        if (wallOwner != null) { owner = wallOwner.gameObject; return true; }
        FloorScript floorOwner = hitCollider.GetComponentInParent<FloorScript>();
        if (floorOwner != null) { owner = floorOwner.gameObject; return true; }
        StairScript stairOwner = hitCollider.GetComponentInParent<StairScript>();
        if (stairOwner != null) { owner = stairOwner.gameObject; return true; }
        return false;
    }
    private bool IsSnapMarkerCollider(Collider collider)
    {
        ...
        _snapMarkerColliderCache[collider] = isSnapMarker; return isSnapMarker;
    }
    ...
    {
        y = 0f; if (target == null) { return false; }
    ...
    {
        y = searchTop ? float.MinValue : float.MaxValue; bool found = false;
        for (int i = 0; i < colliders.Length; i++)
        {
            Collider collider = colliders[i];
            if (collider == null || IsSnapMarkerTransform(collider.transform)) { continue; }
            float candidate = searchTop ? collider.bounds.max.y : collider.bounds.min.y; if (!found)
            {
                y = candidate; found = true; continue;
            }
    ...
    private static bool TryGetExtremeYFromRenderers(Renderer[] renderers, bool searchTop, out float y)
    {
        ...
        for (int i = 0; i < renderers.Length; i++)
        {
            ...
            if (renderer == null || IsSnapMarkerTransform(renderer.transform)) { continue; }
            float candidate = searchTop ? renderer.bounds.max.y : renderer.bounds.min.y; if (!found)
            {
                y = candidate; found = true; continue;
            }
            y = searchTop ? Mathf.Max(y, candidate) : Mathf.Min(y, candidate);
    ...
    {
        _isSnapLocked = false; _lockedPreviewA = null;
        _lockedPreviewB = null; _lockedTargetA = null; _lockedTargetB = null;
        _lockedPreviewOffsetA = Vector3.zero; _lockedPreviewOffsetB = Vector3.zero;
    }
    private static void SetPreviewMode(GameObject target, bool isPreview)
    {
        ...
        else if (!isPreview && target.TryGetComponent(out BuildPreviewMarker previewMarker)) { UnityEngine.Object.Destroy(previewMarker); }
    ...
    {
        bounds = default; if (target == null) { return false; }
        Renderer[] renderers = target.GetComponentsInChildren<Renderer>(true);
        bool hasBounds = false; for (int i = 0; i < renderers.Length; i++)
        {
            Renderer renderer = renderers[i];
            if (renderer == null || IsSnapMarkerTransform(renderer.transform)) { continue; }
            if (!hasBounds)
    ...
    private bool TryGetNearestSnapPointYNearReference(out float nearestY)
    {
        ...
        float radius = Mathf.Max(MinSize, floorNearestPointRadius);
        float maxDistanceSqr = radius * radius; Vector3 referencePosition = reference.position;
        bool found = false; float bestDistanceSqr = float.MaxValue;
        ...
        for (int i = 0; i < _cachedFloorHeightSnapPoints.Count; i++)
        {
            ...
            if (snapPoint == null || !snapPoint.gameObject.activeInHierarchy) { continue; }
            if (_previewObject != null && snapPoint.transform.IsChildOf(_previewObject.transform)) { continue; }
            Vector3 pointPosition = snapPoint.transform.position; float distanceSqr;
            ...
            else { distanceSqr = (pointPosition - referencePosition).sqrMagnitude; }
            if (distanceSqr > maxDistanceSqr || distanceSqr >= bestDistanceSqr) { continue; }
            bestDistanceSqr = distanceSqr; nearestY = pointPosition.y; found = true;
        }
        return found;
    }
}
