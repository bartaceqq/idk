...
public class RadiusForAttackScript : MonoBehaviour
{
    ...
    public GameObject player; public Transform attackOrigin;
    public EnemiesHandler enemiesHandler; public ActionScript actionScript;
    public ItemSwitchScript itemSwitchScript; public float attackRadius = 5f;
    ...
    private float _nextReferenceRefreshTime; private float _nextFallbackTargetRefreshTime;
    private int _trackedSwordStateHash; private float _lastTrackedSwordProgress;
    private void Awake() { ResolveReferences(true); EnsureSwordHitProfiles(); }
    private void OnValidate() { EnsureSwordHitProfiles(); }
    private void Update() { UpdateSwordAttackHits(); }
    private void ResolveReferences(bool force = false)
    {
        if (!force &&
    Time.time < _nextReferenceRefreshTime && player != null && enemiesHandler != null &&
    actionScript != null && itemSwitchScript != null) { return; }
    ...
    {
        if (swordHitProfiles != null && swordHitProfiles.Count > 0) { return; }
        swordHitProfiles = new List<SwordHitProfile> {
CreateProfile("SwordAttack", 1f, new SwordHitWindow { startNormalized = 0.14f,
endNormalized = 0.40f, damageMultiplier = 1f }), CreateProfile( "SwordAttack2", 1f,
new SwordHitWindow { startNormalized = 0.10f, endNormalized = 0.30f,
damageMultiplier = 0.55f }, new SwordHitWindow { startNormalized = 0.50f,
endNormalized = 0.70f, damageMultiplier = 0.65f }), CreateProfile( "SpecialAttack1",
1.15f, new SwordHitWindow { startNormalized = 0.12f, endNormalized = 0.32f,
damageMultiplier = 0.65f }, new SwordHitWindow { startNormalized = 0.48f,
endNormalized = 0.74f, damageMultiplier = 0.7f }), CreateProfile( "SpecialAttack2", 1.2f,
new SwordHitWindow { startNormalized = 0.05f, endNormalized = 0.18f,
damageMultiplier = 0.35f }, new SwordHitWindow { startNormalized = 0.28f,
endNormalized = 0.44f, damageMultiplier = 0.35f }, new SwordHitWindow {
startNormalized = 0.52f, endNormalized = 0.68f, damageMultiplier = 0.35f },
new SwordHitWindow { startNormalized = 0.80f, endNormalized = 0.94f,
damageMultiplier = 0.5f }), CreateProfile( "SpecialAttack3", 1.15f, new SwordHitWindow {
startNormalized = 0.28f, endNormalized = 0.50f, damageMultiplier = 0.55f },
new SwordHitWindow { startNormalized = 0.58f, endNormalized = 0.80f,
damageMultiplier = 0.75f }) };
    ...
    private static SwordHitProfile CreateProfile(string stateName, float radiusMultiplier, params SwordHitWindow[] windows)
    {
        ...
        {
            for (int i = 0; i < windows.Length; i++) { if (windows[i] != null) { profile.windows.Add(windows[i]); } }
        }
        return profile;
    }
    private void UpdateSwordAttackHits()
    {
        ...
        if (stateRestarted) { BeginTrackedSwordAttack(stateInfo.fullPathHash, profile.windows.Count); }
        ...
        if (damage <= 0f) { return; }
        PerformAttackSweep(ResolveAttackOrigin(), radius, damage,
        _windowHitTargetIds[activeWindowIndex]);
    }
    private void BeginTrackedSwordAttack(int stateHash, int windowCount)
    {
        _trackedSwordStateHash = stateHash; _lastTrackedSwordProgress = 0f;
        EnsureWindowRegistryCapacity(windowCount);
        for (int i = 0; i < _windowHitTargetIds.Count; i++) { _windowHitTargetIds[i].Clear(); }
    }
    ...
    private void EnsureWindowRegistryCapacity(int windowCount)
    {
        ...
        while (_windowHitTargetIds.Count < requiredCount) { _windowHitTargetIds.Add(new HashSet<int>()); }
    }
    private bool TryResolveSwordHitProfile(AnimatorStateInfo stateInfo, out SwordHitProfile resolvedProfile)
    {
        resolvedProfile = null; if (swordHitProfiles == null) { return false; }
        for (int i = 0; i < swordHitProfiles.Count; i++)
        {
            ...
            profile.windows == null || profile.windows.Count == 0) { continue; }
            if (!MatchesStateName(stateInfo, profile.stateName)) { continue; }
            resolvedProfile = profile; return true;
        }
        return false;
    }
    private static int FindActiveWindowIndex(SwordHitProfile profile, float normalizedProgress)
    {
        if (profile == null || profile.windows == null) { return -1; }
        for (int i = 0; i < profile.windows.Count; i++)
        {
            SwordHitWindow window = profile.windows[i]; if (window == null) { continue; }
            float start = Mathf.Clamp01(window.startNormalized);
            float end = Mathf.Clamp(window.endNormalized, start, 1f);
            if (normalizedProgress >= start && normalizedProgress <= end) { return i; }
        }
        return -1;
    }
    public void Attack()
    {
        ResolveReferences(true); PerformAttackSweep(ResolveAttackOrigin(),
    Mathf.Max(0.01f, attackRadius), ResolveSwordBaseDamage(), null);
    }
    private float ResolveSwordBaseDamage()
    {
        if (itemSwitchScript != null && itemSwitchScript.TryGetEquippedSword(out Sword equippedSword)) { return equippedSword.GetResolvedDamage(); }
        return Mathf.Max(0f, attackDamage);
    }
    private Vector3 ResolveAttackOrigin()
    {
        if (attackOrigin != null) { return attackOrigin.position; }
        if (player != null) { return player.transform.TransformPoint(attackOriginLocalOffset); }
    ...
    {
        if (damage <= 0f) { return; }
        float radiusSqr = radius * radius; CollectTargets(origin, radius, radiusSqr);
        foreach (NPCDemageScript damageTarget in _uniqueTargets)
        {
            ...
            if (alreadyHitTargetIds != null && alreadyHitTargetIds.Contains(targetId)) { continue; }
            damageTarget.TakeDemage(damage); alreadyHitTargetIds?.Add(targetId);
        }
        foreach (Animalec animalTarget in _uniqueAnimals)
        {
            ...
            if (alreadyHitTargetIds != null && alreadyHitTargetIds.Contains(targetId)) { continue; }
            animalTarget.TakeDamage(damage); alreadyHitTargetIds?.Add(targetId);
        }
    }
    private void CollectTargets(Vector3 origin, float radius, float radiusSqr)
    {
        _uniqueTargets.Clear(); _uniqueAnimals.Clear();
        int hitCount = Physics.OverlapSphereNonAlloc(origin, Mathf.Max(0.01f, radius),
        _overlapHits, enemyMask, triggerInteraction); for (int i = 0; i < hitCount; i++)
        {
            Collider hit = _overlapHits[i];
            if (hit == null || (player != null && hit.transform.IsChildOf(player.transform))) { continue; }
            NPCDemageScript damageTarget = hit.GetComponent<NPCDemageScript>();
            if (damageTarget == null) { damageTarget = hit.GetComponentInParent<NPCDemageScript>(); }
            if (damageTarget == null) { damageTarget = hit.GetComponentInChildren<NPCDemageScript>(); }
            if (damageTarget != null) { _uniqueTargets.Add(damageTarget); }
            Animalec animalTarget = hit.GetComponent<Animalec>();
            if (animalTarget == null) { animalTarget = hit.GetComponentInParent<Animalec>(); }
            if (animalTarget == null) { animalTarget = hit.GetComponentInChildren<Animalec>(); }
            if (animalTarget != null) { _uniqueAnimals.Add(animalTarget); }
        }
        if (_uniqueTargets.Count == 0) { CollectTargetsFromEnemyLists(origin, radiusSqr); }
        if (_uniqueAnimals.Count == 0) { CollectAnimalTargetsFromScene(origin, radiusSqr); }
    }
    private void CollectTargetsFromEnemyLists(Vector3 origin, float radiusSqr)
    {
        if (enemiesHandler != null && enemiesHandler.enemies != null)
        {
            foreach (GameObject enemy in enemiesHandler.enemies)
            {
                ...
                if (fromList == null) { fromList = enemy.GetComponentInChildren<NPCDemageScript>(); }
                if (fromList == null) { fromList = enemy.GetComponentInParent<NPCDemageScript>(); }
    ...
    {
        RefreshFallbackTargetCacheIfNeeded(); Animalec[] allAnimals = _cachedAnimals;
