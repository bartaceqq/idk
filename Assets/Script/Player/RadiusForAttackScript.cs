...
public class RadiusForAttackScript : MonoBehaviour {
    ...
    private void EnsureSwordHitProfiles() { if (swordHitProfiles != null && swordHitProfiles.Count > 0) { return; }
    ...
    private bool TryResolveSwordHitProfile(AnimatorStateInfo stateInfo, out SwordHitProfile resolvedProfile) {
        ...
        if (swordHitProfiles == null) { return false; }
        ...
        return false; }
    private static int FindActiveWindowIndex(SwordHitProfile profile, float normalizedProgress) { if (profile == null || profile.windows == null) { return -1; }
    ...
    public void Attack() {
        ...
        PerformAttackSweep(
            ...
            null); }
    private float ResolveSwordBaseDamage() { if (itemSwitchScript != null && itemSwitchScript.TryGetEquippedSword(out Sword equippedSword)) { return equippedSword.GetResolvedDamage(); }

        return Mathf.Max(0f, attackDamage); }
    private Vector3 ResolveAttackOrigin() { if (attackOrigin != null) { return attackOrigin.position; }
    ...
    private void PerformAttackSweep(Vector3 origin, float radius, float damage, HashSet<int> alreadyHitTargetIds) { if (damage <= 0f) { return; }

        float radiusSqr = radius * radius;
        CollectTargets(origin, radius, radiusSqr);

        foreach (NPCDemageScript damageTarget in _uniqueTargets) { if (damageTarget == null) { continue; }
        ...
        foreach (Animalec animalTarget in _uniqueAnimals) { if (animalTarget == null) { continue; }
    ...
    private void CollectTargetsFromEnemyLists(Vector3 origin, float radiusSqr) {
        if (enemiesHandler != null && enemiesHandler.enemies != null) {
            foreach (GameObject enemy in enemiesHandler.enemies) { if (enemy == null) { continue; }
