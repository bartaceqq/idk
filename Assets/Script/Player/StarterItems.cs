...
public class StarterItems : MonoBehaviour {
    ...
    private void Start() {
        ...
        if (stoneItem != null) {
            addHandler.AddItemToInventoryAmount(stoneItem, startAmount); } else { Debug.LogWarning("StarterItems: No GameObject with tag 'LittleStone' and InventoryItem found.", this); }

        InventoryItem stickItem = FindInventoryItemByTag("Stick");
        if (stickItem != null) {
            addHandler.AddItemToInventoryAmount(stickItem, startAmount); } else { Debug.LogWarning("StarterItems: No GameObject with tag 'Stick' and InventoryItem found.", this); } }
