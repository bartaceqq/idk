...
public class StaminaScript : MonoBehaviour
{
    ...
    public void AddStamina()
    {
        if (image == null)
        {
            enoughstamina = true; return;
        }
        if (Time.time < _staminaRechargeBlockedUntil)
        {
            UpdateEnoughStaminaState(); return;
        }
    ...
    {
        if (image == null) { enoughstamina = true; return; }
    ...
    {
        if (image == null) { enoughstamina = true; return true; }
        float clampedAmount = Mathf.Max(0f, amount); if (clampedAmount <= 0f)
        {
            BlockStaminaRegeneration(rechargeDelaySeconds); UpdateEnoughStaminaState(); return true;
        }
        if ((image.fillAmount + minimumStaminaThreshold) < clampedAmount)
        {
            UpdateEnoughStaminaState(); return false;
        }
