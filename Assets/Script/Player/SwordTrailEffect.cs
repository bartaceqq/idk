...
public class SwordTrailEffect : MonoBehaviour
{
    ...
    private void RebuildTrailMesh(float now)
    {
        ...
        for (int i = 0; i < _samples.Count; i++)
        {
            ...
            Color sampleColor = trailColor;
            sampleColor.a *= alpha;

            _vertices.Add(sample.basePosition);
            _vertices.Add(sample.tipPosition);
            _colors.Add(sampleColor);
            _colors.Add(sampleColor);

            float v = segmentCount <= 0 ? 0f : (float)i / segmentCount;
