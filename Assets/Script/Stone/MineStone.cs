...
public class MineStone : MonoBehaviour
{
    ...
    public string type; public Material greymat; public string texttoshow;
    public Sprite sprite; public InfoHandler infoHandler;
    public List<GameObject> parts = new List<GameObject>(); public GameObject fullstone;
    public List<int> chosen = new List<int>(); public Material blackmaterial;
    public int counter = 8; public GameObject[] mainstoneparts;
    ...
    public GetRandomOreType getRandomOreType; public string currentore;
    private InventoryManager inventoryManager;
    private string selectedRewardName = DefaultStoneItemName;
    private Sprite selectedRewardSprite; [Header("Auto Detect")]
    ...
    private struct TransformSnapshot
    {
        ...
        public Quaternion localRotation; public Vector3 localScale;
    }
    private struct RigidbodySnapshot
    {
        public bool useGravity; public bool isKinematic;
        public RigidbodyInterpolation interpolation;
        public CollisionDetectionMode collisionDetectionMode; public float linearDamping;
        public float angularDamping; public float maxAngularVelocity;
    }
    private void Awake()
    {
    ...
    private void OnValidate()
    {
        ...
        {
            ResolveReferences();
            AutoPopulateStoneStructure(); CacheFragmentDefaults();
        }
    }
    private void Reset()
    {
        fullstone = gameObject; AutoPopulateStoneStructure();
    }
    private void Start()
    {
        ResolveReferences(); AutoPopulateStoneStructure(); CacheFragmentDefaults();
    ...
    {
        if (fullstone == null) { fullstone = gameObject; }
        if (infoHandler == null)
        {
        ...
        {
            if (cachedRandomOreType == null) { cachedRandomOreType = FindOreTypeProviderInScene(); }
            getRandomOreType = cachedRandomOreType;
        }
        else { cachedRandomOreType = getRandomOreType; }
        if (inventoryManager == null)
        {
            if (cachedInventoryManager == null) { cachedInventoryManager = FindInventoryManagerInScene(); }
            inventoryManager = cachedInventoryManager;
        }
        else { cachedInventoryManager = inventoryManager; }
    ...
    private void AutoPopulateStoneStructure()
    {
        ...
        for (int i = 0; i < descendants.Length; i++)
        {
            ...
            {
                detectedStoneParts.Add(current.gameObject); continue;
            }
            if (NameContains(current.name, oreChildNameContains))
            {
                detectedOreParts.Add(current.gameObject); detectedOreTransforms.Add(current);
            }
        }
        if (detectedStoneParts.Count == 0) { CollectFallbackStoneParts(root, descendants, detectedOreTransforms, detectedStoneParts); }
        SortByName(detectedStoneParts); SortByName(detectedOreParts); parts.Clear();
        parts.AddRange(detectedStoneParts); orelist.Clear(); orelist.AddRange(detectedOreParts);
        mainstoneparts = detectedStoneParts.ToArray(); chosen.Clear();
    }
    ...
    {
        ResolveReferences(); string inventoryName = DefaultStoneItemName;
        Sprite inventorySprite = sprite; if (getRandomOreType == null)
        {
            Debug.LogWarning($"{name}: Missing GetRandomOreType reference.", this);
            selectedOre = null; currentore = "noore";
        }
        else
        {
        ...
        selectedRewardName = inventoryName; selectedRewardSprite = inventorySprite;
        ...
        for (int i = 0; i < orelist.Count; i++)
        {
            ...
            selectedOre.oreName != "noore" && selectedOre.material != null;
    ...
    public void Mine()
    {
        ...
        if (parts != null && parts.Count > 0)
        {
            ...
            {
                int selectedIndex = GetRandomAvailablePartIndex(); if (selectedIndex >= 0)
                {
    ...
    {
        isRebuilding = true; BreakStoneApart();
        yield return new WaitForSeconds(Mathf.Max(0f, destroyDelaySeconds));
        SetStoneVisible(false); RestoreFragmentsFromBreak();
        yield return new WaitForSeconds(Mathf.Max(0f, rebuildDelaySeconds)); RebuildStone();
        isRebuilding = false;
    }
    ...
    private void GiveStoneReward()
    {
        ...
        if (infoHandler != null)
        {
            ...
            ? rewardItem.inventorysprite : sprite;
            infoHandler.ShowInfoNow(texttoshow, infoSprite);
        }
        else { Debug.LogWarning($"{name}: Missing InfoHandler reference.", this); }
    }
    private InventoryItem ResolveRewardInventoryItem()
    {
        ...
        if (!RuntimeRewardItems.TryGetValue(rewardName, out InventoryItem rewardItem) || rewardItem == null)
        {
            ...
            rewardObject.hideFlags = HideFlags.HideAndDontSave; DontDestroyOnLoad(rewardObject);
    ...
    private bool TryAddRewardToInventory(InventoryItem rewardItem)
    {
        ...
        SlotManager targetSlotManager = rewardItem.slotManager != null ? rewardItem.slotManager
    ...
    private static void CopyRewardItemData(InventoryItem source, InventoryItem target, string rewardName, Sprite rewardSprite)
    {
        ...
        target.name = rewardName; target.nameofitem = rewardName;
        target.inventorysprite = rewardSprite; target.itemType = InventoryItemType.Usable;
    ...
    private void BreakStoneApart()
    {
        ...
        for (int i = 0; i < breakFragments.Count; i++)
        {
            ...
            Rigidbody rigidbody = fragment.GetComponent<Rigidbody>(); if (rigidbody == null)
            {
                rigidbody = fragment.AddComponent<Rigidbody>();
                addedRigidbodies.Add(rigidbody);
            }
            else if (!cachedExistingRigidbodies.ContainsKey(rigidbody)) { cachedExistingRigidbodies.Add(rigidbody, CaptureRigidbodySnapshot(rigidbody)); }
    ...
    private void RestoreFragmentsFromBreak()
    {
        ...
        {
            Rigidbody rigidbody = entry.Key; if (rigidbody == null) { continue; }
            RigidbodySnapshot snapshot = entry.Value; rigidbody.linearVelocity = Vector3.zero;
            rigidbody.angularVelocity = Vector3.zero; rigidbody.useGravity = snapshot.useGravity;
        ...
        {
            Transform target = entry.Key; if (target == null) { continue; }
            TransformSnapshot snapshot = entry.Value; target.localPosition = snapshot.localPosition;
            target.localRotation = snapshot.localRotation; target.localScale = snapshot.localScale;
        }
        addedRigidbodies.Clear(); addedColliders.Clear();
    }
    ...
    private void CacheFragmentDefaults()
    {
        ...
        initialFragmentTransforms.Clear(); cachedBreakMeshColliderConvex.Clear();
        cachedExistingRigidbodies.Clear(); addedRigidbodies.Clear(); addedColliders.Clear();
        ...
        if (mainstoneparts != null)
        {
            ...
            {
                GameObject fragment = mainstoneparts[i]; if (fragment == null) { continue; }
    ...
    private void EnsureFragmentCollider(GameObject fragment)
    {
        ...
        {
            if (!cachedBreakMeshColliderConvex.ContainsKey(meshCollider)) { cachedBreakMeshColliderConvex.Add(meshCollider, meshCollider.convex); }
            meshCollider.convex = true;
            meshCollider.enabled = true;
        }
        else if (fragmentCollider != null) { fragmentCollider.enabled = true; }
    }
    private Collider CreateFragmentCollider(GameObject fragment)
    {
        ...
        if (meshFilter != null && meshFilter.sharedMesh != null)
        {
            ...
            meshCollider.sharedMesh = meshFilter.sharedMesh; meshCollider.convex = true;
            addedColliders.Add(meshCollider); return meshCollider;
        }
        if (!TryGetLocalBounds(fragment.transform, out Bounds localBounds)) { return null; }
        Vector3 size = localBounds.size; size.x = Mathf.Max(size.x, 0.05f);
        size.y = Mathf.Max(size.y, 0.05f); size.z = Mathf.Max(size.z, 0.05f);
        BoxCollider boxCollider = fragment.AddComponent<BoxCollider>();
        boxCollider.center = localBounds.center; boxCollider.size = size;
        addedColliders.Add(boxCollider); return boxCollider;
    }
    private void ConfigureFragmentRigidbody(Rigidbody rigidbody)
    {
        if (rigidbody == null) { return; }
        rigidbody.linearVelocity = Vector3.zero; rigidbody.angularVelocity = Vector3.zero;
        rigidbody.useGravity = true; rigidbody.isKinematic = false;
        ...
        rigidbody.ResetCenterOfMass(); rigidbody.ResetInertiaTensor();
    }
    ...
    {
        if (ore == null || ore.oreName == "noore") { return DefaultStoneItemName; }
        return ore.oreName;
    }
    private static string GetInventoryItemName(InventoryItem item)
    {
    ...
    {
        if (string.IsNullOrWhiteSpace(itemName)) { return "Stone"; }
        string normalized = itemName.Replace('_', ' ');
        string[] splitParts = normalized.Split(' ');
        for (int i = 0; i < splitParts.Length; i++)
        {
            if (string.IsNullOrWhiteSpace(splitParts[i])) { continue; }
    ...
    {
        if (target == null || mainStonePartTransforms == null || mainStonePartTransforms.Count == 0) { return false; }
        Transform current = target.parent;
        while (current != null)
        {
            if (mainStonePartTransforms.Contains(current)) { return true; }
    ...
    {
        if (string.IsNullOrWhiteSpace(source) || string.IsNullOrWhiteSpace(pattern)) { return false; }
    ...
    private static void CollectFallbackStoneParts(Transform root, Transform[] descendants,
    HashSet<Transform> detectedOreTransforms,
    List<GameObject> detectedStoneParts)
    {
        if (root == null || descendants == null || detectedStoneParts == null) { return; }
    ...
    {
        if (target == null || target == root) { return false; }
    ...
    {
        if (target == null || detectedOreTransforms == null || detectedOreTransforms.Count == 0) { return false; }
        Transform current = target;
        while (current != null)
        {
            if (detectedOreTransforms.Contains(current)) { return true; }
            current = current.parent;
        }
        return false;
    }
    ...
    {
        localBounds = default; if (target == null) { return false; }
        bool hasBounds = false;
        Renderer[] renderers = target.GetComponentsInChildren<Renderer>(true);
        for (int i = 0; i < renderers.Length; i++)
        {
    ...
    {
        Vector3 center = worldBounds.center; Vector3 extents = worldBounds.extents;
        for (int x = -1; x <= 1; x += 2)
        {
            for (int y = -1; y <= 1; y += 2)
            {
